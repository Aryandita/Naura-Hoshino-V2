...
export class NauraHero3DViewer {
  ...
  _initThree() {
    ...
    const height =
      rect.height || this.canvas.parentElement?.clientHeight || 460;
      ...
      this.modelGroup.traverse((node) => {
        ...
        if (node.isMesh || node.isSkinnedMesh) {
          ...
          if (
            node.skeleton &&
            node.skeleton.bones &&
            Object.keys(this.bones).length === 0
          ) {
            node.skeleton.bones.forEach((b) => {
              ...
              if (
                /skirt|dress|cloth/i.test(b.name) &&
                !this.skirtBones.includes(b)
              ) {
  ...
  async switchModel(modelPath) {
    ...
    try {
      ...
      console.error(
        "[NauraHero3DViewer] Gagal beralih model ke " + modelPath + ":",
        err,
      );
      this.isLoadingModel = false;
      if (typeof this.options.onError === "function") {
  ...
  _animate() {
    ...
    const lookYaw =
      this.options.lookAtCursor && !this.isInteracting
        ? -this.mouse.x * 0.3
        : 0;
    const lookPitch =
      this.options.lookAtCursor && !this.isInteracting
    ...
    if (this.options.lookAtCursor && !this.isInteracting) {
      if (this.vrm) {
        ...
        if (this.modelRoot) {
          ...
          this.modelRoot.rotation.y +=
            (-this.mouse.x * 0.06 - this.modelRoot.rotation.y) * lerpSpeed;
          this.modelRoot.rotation.x +=
            (this.mouse.y * 0.04 - this.modelRoot.rotation.x) * lerpSpeed;
          this.modelRoot.rotation.z +=
            (-this.mouse.x * 0.02 - this.modelRoot.rotation.z) * lerpSpeed;
        }
        ...
        this.bones.Head.quaternion
          .copy(this.boneRestQuats.Head)
          .multiply(new THREE.Quaternion().setFromEuler(headEuler));
        ...
        if (this.bones.Neck && this.boneRestQuats.Neck) {
          ...
          this.bones.Neck.quaternion
            .copy(this.boneRestQuats.Neck)
            .multiply(new THREE.Quaternion().setFromEuler(neckEuler));
        }
        ...
        if (this.bones.Chest && this.boneRestQuats.Chest) {
          ...
          this.bones.Chest.quaternion
            .copy(this.boneRestQuats.Chest)
            .multiply(new THREE.Quaternion().setFromEuler(chestEuler));
        }
        ...
        this.modelRoot.rotation.y +=
          (targetRotY - this.modelRoot.rotation.y) * lerpSpeed;
        this.modelRoot.rotation.x +=
          (targetRotX - this.modelRoot.rotation.x) * lerpSpeed;
        this.modelRoot.rotation.z +=
          (targetRotZ - this.modelRoot.rotation.z) * lerpSpeed;
      }
      ...
      if (this.bones.Ponytail && !this.vrm?.springBoneManager) {
        ...
        const targetAngleZ = -lookPitch * 0.2 + Math.cos(elapsed * 1.2) * 0.02;

        const stiffness = 85.0;
        const damping = 9.0;

        const forceY =
          (targetAngleY - this.ponytailPhysics.angleY) * stiffness -
          this.ponytailPhysics.velY * damping;
        this.ponytailPhysics.velY += forceY * delta;
        this.ponytailPhysics.angleY += this.ponytailPhysics.velY * delta;

        const forceZ =
          (targetAngleZ - this.ponytailPhysics.angleZ) * stiffness -
          this.ponytailPhysics.velZ * damping;
        ...
        if (this.boneRestQuats.Ponytail) {
          this.bones.Ponytail.quaternion
            .copy(this.boneRestQuats.Ponytail)
            .multiply(new THREE.Quaternion().setFromEuler(ponyEuler));
        }
      }
    }
  ...
  _updateBlinking(delta) {
    ...
    if (
      ...
      this.blinkState.timer >= this.blinkState.interval
    ) {
  ...
  _updateMorphs(delta, elapsed) {
    ...
    if (this.currentMood === "Talk") {
      this.targetMorphWeights.Talk =
        0.2 + Math.abs(Math.sin(elapsed * 9.0) * Math.cos(elapsed * 4.5)) * 0.7;
    }
    ...
    const activeBlink = Math.max(
      this.morphWeights.Blink,
      this.blinkState.currentWeight || 0,
    );
    const isIdle =
      !this.animController || this.animController.getActiveAnim() === "Idle";
    ...
    if (this.vrm && this.vrm.expressionManager) {
        ...
        for (const [key, exprName] of Object.entries(vrmExprMap)) {
          try {
            this.vrm.expressionManager.setValue(
              exprName,
              Math.max(0, Math.min(1, this.morphWeights[key])),
            );
          } catch (_) {}
        }
      }
      ...
      this.primarySkinnedMesh &&
      this.primarySkinnedMesh.morphTargetDictionary
    ) {
  ...
  playAnimation(name, options = {}) {
    ...
    const resolvedName =
      validNames.find((n) => n.toLowerCase() === (name || "").toLowerCase()) ||
      "Idle";

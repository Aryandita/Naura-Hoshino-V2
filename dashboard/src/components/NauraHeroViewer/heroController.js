...
export function initHeroViewer(targetCanvas = null, customOptions = {}) {
  const canvasElement =
    targetCanvas || document.getElementById("naura-hero-3d-canvas");
  if (!canvasElement) {
    return null;
  }
    ...
    onLoad: () => {
      ...
      if (formatBadge) {
        if (currentActiveModel.includes("NEW")) {
          formatBadge.textContent = currentActiveModel.endsWith(".vrm")
            ? "NEW VRM"
            : "NEW GLB";
        } else {
          formatBadge.textContent = currentActiveModel.endsWith(".vrm")
  ...
  const updateModelButtonsState = (activeModelPath) => {
    ...
    const isVrmOldActive =
      !activeModelPath.includes("NEW") && activeModelPath.endsWith(".vrm");
    ...
    if (formatBadge) {
      if (activeModelPath.includes("NEW")) {
        formatBadge.textContent = activeModelPath.endsWith(".vrm")
          ? "NEW VRM"
          : "NEW GLB";
      } else {
        formatBadge.textContent = activeModelPath.endsWith(".vrm")
  ...
  if (buttonModelNew) {
    buttonModelNew.addEventListener("click", (event) => {
      ...
      handleModelSwitch(
        buttonModelNew.dataset.model || "/models/naura NEW.vrm",
      );
    });
  }
  ...
  if (buttonModelGlb) {
    buttonModelGlb.addEventListener("click", (event) => {
      event.stopPropagation();
      handleModelSwitch(
        buttonModelGlb.dataset.model || "/models/naura NEW.glb",
      );
    });
  }
      ...
      viewerInstance.playAnimation(animationName, {
        ...
        onFinish: () => {
          ...
          animationButtons.forEach((btn) => {
            ...
            btn.style.background = isIdleBtn
              ? "var(--primary-dim)"
              : "transparent";
            btn.style.color = isIdleBtn
              ? "var(--primary)"
              : "var(--text-muted)";
            btn.style.borderColor = isIdleBtn
              ? "var(--border-primary)"
              : "var(--border-subtle)";
  ...
  if (buttonHalo) {
    buttonHalo.addEventListener("click", (event) => {
      ...
      if (typeof window.showToast === "function") {
        window.showToast(
          isVisible ? "✨ Astral Halo Diaktifkan" : "Astral Halo Disembunyikan",
          "info",
          2000,
        );
      }

...
export class Naura3DViewer {
  ...
  async init() {
    ...
    try {
      ...
      console.warn(
        "[Naura3DViewer] Gagal inisialisasi WebGL 3D, menggunakan fallback 2D:",
        err.message,
      );
      return false;
    }
  ...
  handlePointerMove(event, customRect) {
    ...
    if (customRect) {
      ...
      this.mouse.targetX = Math.max(
        -1,
        Math.min(1, (event.clientX - centerX) / (customRect.width * 1.5)),
      );
      this.mouse.targetY = Math.max(
        -1,
        Math.min(1, -(event.clientY - centerY) / (customRect.height * 1.5)),
      );
    } else {
    ...
    if (this.modelGroup && this.options.lookAtCursor) {
      ...
      if (this.vrm) {
        ...
        this.modelGroup.rotation.y +=
          (targetRotY - this.modelGroup.rotation.y) * lerpSpeed;
        this.modelGroup.rotation.x +=
          (targetRotX - this.modelGroup.rotation.x) * lerpSpeed;
        this.modelGroup.rotation.z +=
          (targetRotZ - this.modelGroup.rotation.z) * lerpSpeed;
        ...
        if (this.bones.Ponytail && this.boneRestQuats.Ponytail) {
          ...
          const targetAngleZ =
            -lookPitch * 0.2 + Math.cos(elapsed * 1.2) * 0.02;
  ...
  _updateMorphs(delta, elapsed) {
    ...
    for (const vKey of ["A", "I", "U", "E", "O"]) {
      ...
      this.visemeWeights[vKey] +=
        (vTarget - this.visemeWeights[vKey]) * morphDecay;
    }
    ...
    if (this.vrm && this.vrm.expressionManager) {
      ...
      for (const [vKey, vw] of Object.entries(this.visemeWeights)) {
        if (vw > 0.01) {
          const idx =
            dict[vKey] !== undefined ? dict[vKey] : dict[vKey.toLowerCase()];
          if (idx !== undefined) {
  ...
  setSkin(skinName = "cyberpunk") {
    ...
    const config =
      skinPalettes[skinName.toLowerCase()] || skinPalettes.cyberpunk;

...
const normalizeName = (name) =>
    String(name).toLowerCase().replace(/^mixamorig[:_]?/, "").replace(/[^a-z0-9]/g, "");
...
export function buildHumanoidBones(root) {
    ...
    for (const [vrmKey, glbName] of Object.entries(VRM_TO_GLB_BONE)) {
        ...
        bones[vrmKey] = node;
        if (!node && !OPTIONAL_BONES.has(vrmKey)) missing.push(`${vrmKey} (${glbName})`);
    }

    if (missing.length) {
        console.warn("[NauraAnimation:RigProfile] Bone tidak ditemukan:", missing.join(", "));
    }
    return bones;
}
...
const FALLBACK_TARGET_NAMES = ["Happy", "Thinking", "Sad", "Angry", "Blink", "Talk"];
...
export class GlbExpressionRig {
    ...
    constructor(root) {
        ...
        root.traverse((obj) => {
            if (!obj.isMesh || !obj.morphTargetInfluences || !obj.morphTargetInfluences.length) return;
            let dict = obj.morphTargetDictionary;
            if (!dict || !Object.keys(dict).length) {
                dict = {};
                FALLBACK_TARGET_NAMES.forEach((n, i) => { dict[n] = i; });
            }
            this.meshes.push({ mesh: obj, dict });
        });

        if (!this.meshes.length) {
            console.warn("[NauraAnimation:RigProfile] Tidak ada mesh dengan morph target ditemukan.");
        }
    }
    ...
    _setValue(channelId, name, value) {
        ...
        for (const [target, factor] of mapping) {
            this.entries.set(`${channelId}|${name}|${target}`, { target, value: v * factor });
            touched.add(target);
        }
    ...
    _apply(target) {
        ...
        for (const entry of this.entries.values()) {
            if (entry.target === target && entry.value > influence) influence = entry.value;
        }

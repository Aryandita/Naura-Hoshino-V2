...
export class HairController {
  ...
  init(vrm, modelScene) {
    ...
    try {
      ...
      console.warn(
        "[NauraAnimation:Hair] Failed searching ponytail bone:",
        err.message,
      );
    }
  }
  ...
  update(delta, elapsed, context = {}) {
    try {
      ...
      const targetAngleY =
        (-lookYaw * 0.25 + Math.sin(elapsed * 1.8) * 0.03) * intensity;
      const targetAngleZ =
        (-lookPitch * 0.2 + Math.cos(elapsed * 1.2) * 0.02) * intensity;

      const forceY =
        (targetAngleY - this.physics.angleY) * this.physics.stiffness -
        this.physics.velY * this.physics.damping;
      this.physics.velY += forceY * delta;
      this.physics.angleY += this.physics.velY * delta;

      const forceZ =
        (targetAngleZ - this.physics.angleZ) * this.physics.stiffness -
        this.physics.velZ * this.physics.damping;
      ...
      this.ponytailBone.quaternion
        .copy(this.restQuaternion)
        .multiply(this._quat);
    } catch (err) {
      console.warn(
        "[NauraAnimation:Hair] Error updating hair physics:",
        err.message,
      );
    }
  }

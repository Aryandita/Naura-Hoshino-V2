  ...
  update(delta, elapsed, context = {}) {
    ...
    try {
      ...
      const trackingPitch = enableTracking
        ? THREE.MathUtils.clamp(lookPitch * 0.4, -0.4, 0.4)
        : 0;
      const trackingYaw = enableTracking
        ? THREE.MathUtils.clamp(lookYaw * 0.45, -0.6, 0.6)
        : 0;
      // Kemiringan imut kepala (anime cute head-tilt)
      const trackingRoll = enableTracking
        ? THREE.MathUtils.clamp(headTilt * 0.35, -0.3, 0.3)
        : 0;
      ...
      console.warn(
        "[NauraAnimation:Head] Error updating head kinematics:",
        err.message,
      );
    }
  }

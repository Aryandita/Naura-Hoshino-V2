...
export class NeckController {
  ...
  update(delta, elapsed, context = {}) {
    ...
    try {
      ...
      const trackingPitch = enableTracking
        ? THREE.MathUtils.clamp(lookPitch * 0.2, -0.2, 0.2)
        : 0;
      const trackingYaw = enableTracking
        ? THREE.MathUtils.clamp(lookYaw * 0.25, -0.35, 0.35)
        : 0;
      const trackingRoll = enableTracking
        ? THREE.MathUtils.clamp(headTilt * 0.15, -0.15, 0.15)
        : 0;
      ...
      console.warn(
        "[NauraAnimation:Neck] Error updating neck kinematics:",
        err.message,
      );
    }
  }

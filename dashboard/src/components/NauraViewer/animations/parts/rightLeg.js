    ...
    init(humanoidBones = {}) {
        for (const k of RIGHT_LEG_KEYS) {
            ...
            for (const alias of (RIGHT_LEG_ALIASES[k] || [k])) {
                if (humanoidBones[alias]) { bone = humanoidBones[alias]; break; }
            }
            this.bones[k] = bone;
            this.currentRotations[k] = [0, 0, 0];
        }
            ...
            try {
                ...
                if (!base) {
                    for (const alias of (RIGHT_LEG_ALIASES[key] || [])) {
                        if (targetBones[alias]) { base = targetBones[alias]; break; }
                    }
                }
                base = base || [0, 0, 0];
                ...
                console.warn(`[NauraAnimation:RightLeg] Error updating ${key}:`, err.message);
            }
        }
    }

...
export class ChestController {
    ...
    update(delta, elapsed, context = {}) {
        ...
        try {
            ...
            const extra = vrmToRig([breathPitch - armForwardLean, 0, posturalRoll * 0.55]);

            // Batas dalam sumbu rig: roll (X), yaw (Y), pitch (Z; positif = condong ke belakang)
            const targetX = softClampAngle(base[0] + extra[0], -0.15, 0.15, -0.22, 0.22);
            const targetY = softClampAngle(base[1] + extra[1], -0.18, 0.18, -0.25, 0.25);
            const targetZ = softClampAngle(base[2] + extra[2], -0.28, 0.22, -0.35, 0.30);

            const target = [targetX, targetY, targetZ];

            slerpBoneDirect(this.bone, target, lerpSpeed);

            this.currentRotation[0] += (targetX - this.currentRotation[0]) * lerpSpeed;
            this.currentRotation[1] += (targetY - this.currentRotation[1]) * lerpSpeed;
            this.currentRotation[2] += (targetZ - this.currentRotation[2]) * lerpSpeed;
        } catch (err) {
            console.warn("[NauraAnimation:Chest] Error updating chest kinematics:", err.message);
        }
    }

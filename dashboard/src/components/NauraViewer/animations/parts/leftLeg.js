    ...
    update(delta, elapsed, context = {}) {
        ...
        for (const key of LEFT_LEG_KEYS) {
            try {
                ...
                if (key === "leftUpperLeg") {
                    target[0] = THREE.MathUtils.clamp(target[0], -0.60, 0.80);
                ...
                if (key === "leftLowerLeg") {
                    target[0] = THREE.MathUtils.clamp(target[0], -0.05, 1.60);
                ...
                console.warn(`[NauraAnimation:LeftLeg] Error updating ${key}:`, err.message);
            }
        }
    }

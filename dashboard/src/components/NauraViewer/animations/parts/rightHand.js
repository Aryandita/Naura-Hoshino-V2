...
export class RightHandController {
  ...
  update(delta, elapsed, context = {}) {
    ...
    try {
      ...
      const base = targetBones.rightHand ||
        DEFAULT_REST_BONES.rightHand || [0, 0, 0];
      let wristOffsetZ = 0;
      ...
      if (
        ...
        wristOffsetZ =
          Math.sin(oscTime * (harmonics.freq ?? 2.8) - 0.4) *
          harmonics.rightHandZ;
      }

      const target = [
        THREE.MathUtils.clamp(base[0], -0.6, 0.6),
        THREE.MathUtils.clamp(base[1], -0.5, 0.5),
        THREE.MathUtils.clamp(base[2] + wristOffsetZ, -1.2, 1.2),
      ];
      ...
      console.warn(
        "[NauraAnimation:RightHand] Error updating right hand:",
        err.message,
      );
    }
  }

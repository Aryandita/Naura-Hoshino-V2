...
export class HipsController {
    ...
    update(delta, elapsed, context = {}) {
        ...
        try {
            ...
            const swayWeight = isIdle ? 1.0 : 0.40;
            const swayRoll = Math.sin(elapsed * Math.PI * 2 * 0.15) * 0.018 * swayWeight;

            // Napas menyentuh hips: getaran micro-bounce
            const breathPitch = Math.sin(elapsed * Math.PI * 2 * this.breathingRate) * 0.006 * swayWeight;
            ...
            console.warn("[NauraAnimation:Hips] Error updating hips kinematics:", err.message);
        }
    }

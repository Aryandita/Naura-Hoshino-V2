...
export class UpperChestController {
    ...
    update(delta, elapsed, context = {}) {
        ...
        try {
            ...
            const targetX = softClampAngle(base[0] + extra[0], -0.12, 0.12, -0.2, 0.2);
            const targetY = softClampAngle(base[1] + extra[1], -0.15, 0.15, -0.25, 0.25);
            const targetZ = softClampAngle(base[2] + extra[2], -0.25, 0.20, -0.35, 0.30);
            ...
            console.warn("[NauraAnimation:UpperChest] Error updating upper chest kinematics:", err.message);
        }
    }

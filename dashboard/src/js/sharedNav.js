...
(function () {
      ...
      pages: [
        {
          ...
          icon: "fa-solid fa-gauge-high",
          desc: "Metrik server, CPU/RAM, dan status bot",
        },
        {
          path: "/status",
          label: "Live Telemetri",
          icon: "fa-solid fa-signal",
          desc: "Kesehatan shard & status gateway Discord",
        },
        {
          path: "/feed",
          label: "Activity Feed",
          icon: "fa-solid fa-rss",
          desc: "Log aktivitas realtime & interaksi pengguna",
        },
        {
          path: "/leaderboard",
          label: "Leaderboard Global",
          icon: "fa-solid fa-trophy",
          desc: "Peringkat level EXP dan kekayaan server",
        },
        {
          path: "/activity",
          label: "Activity Hub",
          icon: "fa-solid fa-gamepad",
          desc: "Quest, mini-games & tantangan berkala",
        },
      ],
    },
    map: {
      ...
      pages: [
        {
          path: "/world",
          label: "World Map Aetheria",
          icon: "fa-solid fa-earth-asia",
          desc: "Peta benua, 6 wilayah, POI & komoditas",
        },
        {
          path: "/survival-map",
          label: "Radar Survival",
          icon: "fa-solid fa-map",
          desc: "Radar real-time fasilitas & blip sumber daya",
        },
        {
          path: "/war-room",
          label: "War Room & Siege",
          icon: "fa-solid fa-shield-halved",
          desc: "Operasi klan, wilayah perang & strategi",
        },
      ],
    },
    config: {
      ...
      pages: [
        {
          ...
          desc: "Trigger otomatis, auto-response & scheduler",
        },
        {
          path: "/welcomer",
          label: "Welcomer Card",
          icon: "fa-solid fa-palette",
          desc: "Desain canvas kartu sambutan member baru",
        },
        {
          path: "/builder",
          label: "Embed Builder",
          icon: "fa-solid fa-wand-magic-sparkles",
          desc: "Generator pesan interaktif Discord V2",
        },
        {
          path: "/tickets",
          label: "Ticket System",
          icon: "fa-solid fa-ticket",
          desc: "Manajemen tiket bantuan & antrean support",
        },
      ],
    },
    system: {
      ...
      pages: [
        {
          path: "/music",
          label: "Music Hub Player",
          icon: "fa-solid fa-music",
          desc: "Pemutar audio Poru v5 & cluster Lavalink",
        },
        {
          path: "/jam",
          label: "DJ Jam Session",
          icon: "fa-solid fa-guitar",
          desc: "Studio turntable interaktif & dual crossfader",
        },
        {
          path: "/karaoke",
          label: "Studio Karaoke",
          icon: "fa-solid fa-microphone",
          desc: "Sinkronisasi lirik real-time & vokal",
        },
        {
          path: "/soundboard",
          label: "Soundboard Studio",
          icon: "fa-solid fa-sliders",
          desc: "Papan efek suara instant dengan SFX audio",
        },
        {
          path: "/lounge",
          label: "Cyber Lounge",
          icon: "fa-solid fa-couch",
          desc: "Ruang interaksi komunitas & live companion",
        },
        {
          path: "/topology",
          label: "Guild Topology",
          icon: "fa-solid fa-network-wired",
          desc: "Diagram cluster server & visualisasi node",
        },
        {
          path: "/portfolio",
          label: "Creator Portfolio",
          icon: "fa-solid fa-id-badge",
          desc: "Showcase fitur & profil kreator bot",
        },
      ],
    },
    settings: {
      ...
      pages: [
        {
          ...
          icon: "fa-solid fa-gears",
          desc: "Konfigurasi bot, prefix, role & perizinan",
        },
        {
          path: "/economy",
          label: "Bank & Pasar NC",
          icon: "fa-solid fa-coins",
          desc: "Dompet kas server, bursa valuta & pasar saham",
        },
      ],
    },
  };
  ...
  document
    ...
    .forEach((anchorElement) => {
      anchorElement.addEventListener("click", (navEvent) => {
        ...
        if (
          anchorElement.classList.contains("mobile-nav-item") &&
          anchorElement.getAttribute("data-group") === activeGroupId
        ) {
          return;
        }
  ...
  const applyStagger = () => {
    document
      .querySelectorAll(".animate-in, .animate-in-left, .animate-scale")
      .forEach((animatedElement, index) => {
  ...
  window.showSkeleton = function (target, options = {}) {
    ...
    let skeletonHtml = "";
    if (type === "stat") {
      skeletonHtml = Array.from({ length: count })
        .map(
          () => `
  ...
  window.renderErrorState = function (target, options = {}) {
    ...
    const message =
      options.message ||
      "Terjadi gangguan jaringan atau server tidak merespons. Silakan coba lagi.";
    ...
    if (typeof onRetry === "function") {
      ...
      if (btn) {
        btn.addEventListener("click", () => {
          btn.disabled = true;
          btn.innerHTML =
            '<i class="fa-solid fa-spinner fa-spin"></i><span>Memuat...</span>';
          try {
            onRetry();
          } catch (err) {
  ...
  window.renderEmptyState = function (target, options = {}) {
    ...
    const message =
      options.message || "Tidak ada riwayat atau aktivitas untuk ditampilkan.";
  ...
  function applyTheme(themeId) {
    const validTheme = AVAILABLE_THEMES.some((t) => t.id === themeId)
      ? themeId
      : "midnight";
    ...
    if (pill) {
      const currentObj =
        AVAILABLE_THEMES.find((t) => t.id === validTheme) ||
        AVAILABLE_THEMES[0];
  ...
  function initHeaderQuickControls() {
    ...
    if (!document.getElementById("themeSwitcherPill")) {
      ...
      const curObj =
        AVAILABLE_THEMES.find((t) => t.id === curTheme) || AVAILABLE_THEMES[0];
      ...
      action: () => {
        ...
        if (window.showToast)
          window.showToast({
            ...
            message: `Tema aktif: ${AVAILABLE_THEMES[nextIdx].label}`,
          });
      },
    },
    {
      ...
      action: () => {
        const compBtn =
          document.getElementById("companionToggleBtn") ||
          document.querySelector(".companion-fab");
  ...
  function renderPaletteItems(filterText) {
    ...
    paletteItemsData.forEach((item, idx) => {
      const category =
        item.type === "action" ? "Aksi Cepat" : "Halaman & Navigasi";
  ...
  function navigatePalette(dir) {
    ...
    selectedIndex =
      (selectedIndex + dir + paletteItemsData.length) % paletteItemsData.length;
    updatePaletteSelection();
  }
    ...
    document.addEventListener(
      ...
      (e) => {
        const btn = e.target.closest(
          "button, .btn, .nav-item, .mobile-nav-item, .tab-btn, a[href]",
        );
        if (!btn) return;
        if (
          btn.classList.contains("tab-btn") ||
          btn.classList.contains("mobile-nav-item")
        ) {
    ...
    document.addEventListener(
      ...
      (e) => {
        if (
          e.target &&
          e.target.matches &&
          e.target.matches("button, .btn, .tab-btn, .mobile-nav-item")
        ) {

...
(function (root, factory) {
    ...
    const THEMES = {
        dashboard: {
            ...
            melody: [NOTES.A4, NOTES.C5, NOTES.E5, NOTES.G5, NOTES.E5, NOTES.D5, NOTES.C5, NOTES.A4],
            type: 'warm_rhodes',
            leadType: 'sine'
        },
        map: {
            ...
            melody: [NOTES.E5, NOTES.G5, NOTES.A5, NOTES.C6, NOTES.A5, NOTES.G5, NOTES.E5, NOTES.D5],
            type: 'ambient_pad',
        ...
        config: {
            ...
            melody: [NOTES.D5, NOTES.F5, NOTES.A5, NOTES.D6, NOTES.A5, NOTES.F5, NOTES.E5, NOTES.C5],
            type: 'focus_analog',
            leadType: 'crisp_arp'
        },
        system: {
            ...
            melody: [NOTES.G5, NOTES.B5, NOTES.C6, NOTES.E6, NOTES.D6, NOTES.B5, NOTES.A5, NOTES.G5],
            type: 'neon_pump',
        ...
        settings: {
            ...
            melody: [NOTES.E5, NOTES.G5, NOTES.A5, NOTES.B5, NOTES.A5, NOTES.F5, NOTES.E5, NOTES.D5],
            type: 'velvet_jazz',
    ...
    class SoundManagerClass {
        constructor() {
            ...
            this.bgmVolume = parseFloat(localStorage.getItem('naura_bgm_vol') || '0.35');
            this.sfxVolume = parseFloat(localStorage.getItem('naura_sfx_vol') || '0.50');
        ...
        initContext() {
            ...
            this.bgmGain.gain.setValueAtTime(this.bgmEnabled ? this.bgmVolume : 0.0001, this.ctx.currentTime);
            this.bgmGain.connect(this.masterGain);

            // SFX Gain
            this.sfxGain = this.ctx.createGain();
            this.sfxGain.gain.setValueAtTime(this.sfxEnabled ? this.sfxVolume : 0.0001, this.ctx.currentTime);
        ...
        getCurrentGroupTheme() {
            if (this.playlistMode && this.playlistMode !== 'auto' && THEMES[this.playlistMode]) {
                return this.playlistMode;
            }
            const path = window.location.pathname.replace(/\/$/, '') || '/';
            if (path === '/world' || path.startsWith('/survival') || path === '/war-room') {
                return 'map';
            }
            if (path === '/automations' || path === '/welcomer' || path === '/builder' || path === '/tickets') {
                return 'config';
            }
            if (path === '/music' || path === '/jam' || path === '/karaoke' || path === '/soundboard' || path === '/lounge' || path === '/topology' || path === '/portfolio') {
                return 'system';
            }
        ...
        playTheme(themeId) {
            ...
            this.bgmGain.gain.linearRampToValueAtTime(this.bgmEnabled ? this.bgmVolume : 0.0001, now + 1.2);
        ...
        _renderThemeStep(theme, step) {
            ...
            if (isBarStart || isHalfBar) {
                ...
                chord.forEach((freq, i) => {
                    ...
                    filter.type = 'lowpass';
                    filter.frequency.setValueAtTime(theme.type === 'focus_analog' ? 1400 : 850, now);
        ...
        setBgmEnabled(enabled) {
            ...
            if (this.ctx && this.bgmGain) {
                const now = this.ctx.currentTime;
                this.bgmGain.gain.cancelScheduledValues(now);
                this.bgmGain.gain.linearRampToValueAtTime(this.bgmEnabled ? this.bgmVolume : 0.0001, now + 0.3);
            }
            if (this.ostAudio) {
                this.ostAudio.volume = this.bgmEnabled ? this.bgmVolume : 0;
            }
        ...
        _syncSettingsUi() {
            ...
            if (bgmVolLabel) bgmVolLabel.textContent = Math.round(this.bgmVolume * 100) + '%';
            ...
            this._updateUiWidget(this.isPlaying ? (THEMES[this.currentThemeId]?.title || 'Aktif') : 'Pause');
        }
    }

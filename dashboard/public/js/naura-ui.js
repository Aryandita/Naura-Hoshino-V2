...
var NauraUI = (function () {
    ...
    function normalize(lang) {
        var v = String(lang || '').toLowerCase().slice(0, 2);
    ...
    api.stateHtml = function (kind, options) {
        ...
        var modifier = kind === 'error' ? ' naura-state--error' : kind === 'loading' ? ' naura-state--loading' : '';

        return (
            '<div class="naura-state' + modifier + '">' +
            '<img class="naura-state__figure" alt="" src="' + api.expressionUrl(kind) + '">' +

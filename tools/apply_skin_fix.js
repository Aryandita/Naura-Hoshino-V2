...
function patchSkinningFromFixedGlb(vrmSrc, vrmDst, glbFixedPath) {
  ...
  console.log(
    "[SkinFix] Menyalin data buffer Indices (bv4), JOINTS_0 (bv8), WEIGHTS_0 (bv9), dan IBM (bv7)...",
  );
  ...
  console.log(
    `[SkinFix] Memperbarui translasi dan skala node sendi lengan (${updatedNodesCount} sendi)...`,
  );

  // Tambahkan penanda naura_fix
  if (!vrmJs.asset) vrmJs.asset = {};
  if (!vrmJs.asset.extras) vrmJs.asset.extras = {};
  vrmJs.asset.extras.naura_fix =
    glbJs.asset?.extras?.naura_fix ||
    "naura-fix v4 (bobot+ketiak+lengan-mulus+sendi+tangan1.25)";
  ...
  console.log(
    `✨ [SkinFix] Sukses! Model telah ditambal dengan sendi lengan baru dan bobot vertex diperbaiki.`,
  );
}
...
if (require.main === module) {
  ...
  const fixedGlb =
    args[2] ||
    (fs.existsSync(path.join(__dirname, "../naura_NEW_fixed.glb"))

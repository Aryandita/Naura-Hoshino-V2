...
test("LandEngine - 3D Spatial Voice Proximity & Avatar Coordinates Math", async () => {
  ...
  const pos1 = await landEngine.updatePlayerPosition("g_spatial", "user_a", { x: 3, y: 3, username: "Alice" });
  assert.equal(pos1.x, 3);
  assert.equal(pos1.y, 3);

  // 2. Dekat (1 ubin): volume 100%
  const proxClose = landEngine.calculateSpatialProximity({ x: 3, y: 3 }, { x: 3, y: 4 });
  ...
  const proxFar = landEngine.calculateSpatialProximity({ x: 1, y: 1 }, { x: 7, y: 7 });
  ...
  const proxRight = landEngine.calculateSpatialProximity({ x: 2, y: 2 }, { x: 4, y: 2 });
  ...
  const audioMap = await landEngine.getProximityAudioMap("g_spatial", "user_a", knownPlayers);

...
function buildPassChoiceView(passes) {
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      ...
      .setLabel("Pakai Tiket Biasa")
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId("dungeon_use_special")
      .setLabel("Pakai Tiket Spesial")
      .setStyle(ButtonStyle.Danger),
    new ButtonBuilder()
...
async function buildBattleView({
  ...
  if (!stats.userClass) {
    footer +=
      " \u2022 Naura sarankan pilih kelas lewat /survival class biar skill tempurmu terbuka!";
  }

  const title =
    e("battle", "\u2694\ufe0f") +
    " Infinite Dungeon - Lantai " +
    floor +

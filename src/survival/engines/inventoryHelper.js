...
function countStack(inv, itemId) {
  return safeParseInventory(inv).reduce(
    (total, item) =>
      item && item.id === itemId ? total + (Number(item.amount) || 1) : total,
    0,
  );
}
...
function manager() {
  return require("../../managers/cacheManager");
}
...
async function addItemsAtomic(userId, newItems) {
  ...
  const result = await manager().mutateUserProfileJson(
    userId,
    "inventory",
    (raw) => {
      let inv = safeParseInventory(raw);
      for (const item of list) {
        inv = addOrStackItem(inv, {
          ...item,

...
test("CommodityMarketEngine - volume 0 menghasilkan harga dasar (basePrice) stabil", () => {
  const result = calculateCommodityPrice("celestial_koi", {
    sold: 0,
    bought: 0,
  });
...
test("CommodityMarketEngine - penjualan masif menurunkan harga dan ter-clamp di batas bawah (0.65)", () => {
  const result = calculateCommodityPrice("starlight_ore", {
    sold: 5000,
    bought: 0,
  });
  const expectedMin = Math.round(
    COMMODITY_CATALOG.starlight_ore.basePrice * MIN_PRICE_MULTIPLIER,
  );
...
test("CommodityMarketEngine - pembelian masif menaikkan harga dan ter-clamp di batas atas (1.50)", () => {
  const result = calculateCommodityPrice("cyber_ruby", {
    sold: 0,
    bought: 5000,
  });
  const expectedMax = Math.round(
    COMMODITY_CATALOG.cyber_ruby.basePrice * MAX_PRICE_MULTIPLIER,
  );
...
test("CommodityMarketEngine - timeDecay meregresikan harga menuju baseline seiring waktu", () => {
  const freshBull = calculateCommodityPrice(
    "void_coffee",
    { sold: 0, bought: 300 },
    0,
  );
  const decayedBull = calculateCommodityPrice(
    "void_coffee",
    { sold: 0, bought: 300 },
    12,
  ); // 12 jam kemudian
  ...
  assert.ok(
    decayedBull.currentPrice >= COMMODITY_CATALOG.void_coffee.basePrice,
  );
});

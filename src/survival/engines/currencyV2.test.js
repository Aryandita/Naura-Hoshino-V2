...
test("Currency V2: One-Way Bridge & Dynamic Spread", async (t) => {
  await t.test("convert() menolak penukaran COIN ke FRAGMENT (One-Way Bridge)", () => {
    const result = currencyHelper.convert(10, currencyHelper.COIN, currencyHelper.FRAGMENT);
    assert.strictEqual(result, null, "Penukaran Coin ke NSF harus diblokir (null)");
  });

  await t.test("convert() mengizinkan penukaran FRAGMENT ke COIN", () => {
    const result = currencyHelper.convert(2500, currencyHelper.FRAGMENT, currencyHelper.COIN);
    assert.strictEqual(result, 2, "2500 NSF harus bernilai 2 Coin");
  });

  await t.test("exchange() menolak penukaran COIN ke FRAGMENT dengan alasan one_way_restricted", async () => {
    const mockHolders = {
  ...
  await t.test("getDynamicRateAndFee() menghitung fee progresif dengan tepat", async () => {
    const quoteStandard = await currencyHelper.getDynamicRateAndFee(10000);
    assert.strictEqual(quoteStandard.feePercent, 0.05, "Biaya standar harus 5%");

    const quoteMedium = await currencyHelper.getDynamicRateAndFee(75000);
    assert.strictEqual(quoteMedium.feePercent, 0.1, "Biaya > 50.000 harus 10%");

    const quoteWhale = await currencyHelper.getDynamicRateAndFee(250000);
    assert.strictEqual(quoteWhale.feePercent, 0.15, "Biaya > 200.000 harus 15%");
  });
});

test("Currency V2: Closed-Loop 4-Channel Recycling Pool", async (t) => {
  await t.test("Alokasi 100% tepat: 40% Infra, 25% Undian, 20% Subsidi, 15% Pedagang", () => {
    const total = 10000;
...
test("Currency V2: Durability & Wear & Tear Surcharge", async (t) => {
  await t.test("calculateRepairCost() mengembalikan 0 untuk alat dengan durabilitas penuh", () => {
    const itemFull = { id: "wooden_pickaxe", price: 1000, durability: 100 };
    const cost = durabilityEngine.calculateRepairCost(itemFull);
    assert.strictEqual(cost, 0);
  });

  await t.test("calculateRepairCost() menghitung biaya perbaikan dasar dan surcharge keausan", () => {
    ...
    const itemSeverelyDamaged = { id: "iron_pickaxe", price: 2000, durability: 10 };
    const costSevere = durabilityEngine.calculateRepairCost(itemSeverelyDamaged);
    assert.ok(

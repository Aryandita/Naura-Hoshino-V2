...
const { rollCouponDrop, dropLine } = require("../helpers/couponRewards");
const leveling = require("./survivalLeveling");
const { advanceTime } = require("../helpers/survivalTime");
const combat = require("./dungeonCombat");
const {
  DUNGEON_PASS_ID,
  DUNGEON_SPECIAL_PASS_ID,
  SPECIAL_MULTIPLIER,
} = require("../data/items_dungeon");
...
async function consumePass(userId, passId) {
  ...
  if (!taken.ok) {
    if (taken.reason !== "not_enough") {
      logger.error(
        `[Dungeon] Gagal memotong tiket ${passId} untuk ${userId}: ${taken.reason}`,
      );
    }

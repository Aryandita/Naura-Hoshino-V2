...
async function unlockAchievement(interaction, achievementId) {
  try {
    ...
    if (source) {
      try {
        const survival = await UserSurvival.findOne({
          where: { userId: interaction.user.id },
        });
        if (survival) {
        ...
        logger.error(
          "[AchievementHelper] Gagal menjatuhkan Naura Coupon:",
          err,
        );
      }
    }
    ...
    if (source === "achievement_mythic") {
      lines.push(
        "",
        `${e("shocked", "\uD83D\uDE32")} Ini pencapaian tingkat **Mitos**, lho! Jarang ada yang sampai sini.`,
      );
    } else if (source === "achievement_rare") {
      lines.push(
        "",
        `${e("impressed", "\u2728")} Pencapaian **Langka** \u2014 Naura simpan baik-baik di lemari piala kamu.`,
      );
    }

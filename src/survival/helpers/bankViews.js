...
function mainView(user, snap) {
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId("bank_menu_exchange")
      .setLabel("Tukar Uang")
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId("bank_menu_savings")
      .setLabel("Tabungan")
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId("bank_menu_deposit")
      .setLabel("Deposito")
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId("bank_menu_invest")
      .setLabel("Investasi")
      .setStyle(ButtonStyle.Secondary),
  );
...
function savingsView(snap) {
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId("bank_savings_in")
      .setLabel("Setor ke rekening")
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId("bank_savings_out")
      .setLabel("Tarik ke dompet")
...
function depositView(snap) {
  ...
  return buildContainerV2({
    ...
    description: [
      ...
      `**Status:** ${status}`,
      active
        ? `**Nilai deposito:** \`${n(dep.amount)}\` ${COIN.short} (${dep.termName || "berjangka"})`
        : "",
    ]
      .filter(Boolean)
      .join("\n"),
...
function investView(snap, valuations) {
  ...
  rows.push(
    new ActionRowBuilder().addComponents(backButton("Kembali ke menu utama")),
  );

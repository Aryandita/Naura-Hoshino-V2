...
function getWeather(day, hour) {
  ...
  if (season.name === "Kemarau") {
    if (seed < 70) {
      weatherData = {
        name: "Cerah Berawan",
        emoji: ui.getEmoji("weather_cloudy") || "\uD83C\uDF24\uFE0F",
        staminaDrain: 1.0,
      };
    } else if (seed < 95) {
      weatherData = {
        name: "Gelombang Panas",
        emoji: ui.getEmoji("weather_heat") || "\uD83D\uDD25",
        staminaDrain: 1.5,
      };
    } else {
      weatherData = {
        name: "Hujan Gerimis",
        emoji: ui.getEmoji("weather_drizzle") || "\uD83C\uDF26\uFE0F",
        staminaDrain: 0.9,
      };
    }
  } else if (seed < 40) {
    ...
    weatherData = {
      name: "Badai Petir",
      emoji: ui.getEmoji("weather_storm") || "\u26C8\uFE0F",
      staminaDrain: 1.8,
    };
  }
...
function getShopMultiplier(item, weather, season) {
  ...
  if (item.category === "consumable") {
    if (
      activeSeason.name === "Kemarau" &&
      item.effects &&
      item.effects.thirst
    ) {
      multiplier *= 1.5;
    } else if (
      activeSeason.name === "Penghujan" &&
      item.effects &&
      item.effects.hunger
    ) {
      multiplier *= 1.5;

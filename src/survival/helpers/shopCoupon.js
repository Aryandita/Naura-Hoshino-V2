...
const { COUPON } = require("../engines/currency");
const { COUPON_ITEMS, getCouponItem } = require("../data/items_coupon");
const { pickLine, fill } = require("../data/shopkeepers");
...
const SPAWN_SPOTS = [
  ...
  { key: "hutan", label: "Hutan", spot: "bawah pohon beringin tua" },
  {
    key: "tambang",
    label: "Tambang",
    spot: "mulut lorong yang sudah ditinggalkan",
  },
...
function isOpen(location, day, hour) {
  ...
  return (
    normalizeLocation(location) === schedule.location &&
...
function availableCategories() {
  ...
  Object.keys(CATEGORIES).forEach((key) => {
    if (COUPON_ITEMS.some((it) => it.category === key))
...
function stockByCategory(category) {
  return COUPON_ITEMS.filter(
    (it) => it.category === category && Number(it.couponPrice) > 0,

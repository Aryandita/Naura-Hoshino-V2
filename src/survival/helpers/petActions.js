...
function evaluateEvolution(pet) {
  ...
  if (pet.evolutionStage < expectedStage) {
    const baseType = EVOLUTION_TREE[pet.petType]
      ? pet.petType
      : Object.keys(EVOLUTION_TREE).find((k) => pet.petType.includes(k)) ||
        "default";
...
function getBuffs(pet) {
  ...
  if (pet.mood === "angry" || pet.mood === "sad") {
    return {
      hp: Math.floor(buff.hp / 2),
      dmg: Math.floor(buff.dmg / 2),
      dodge: Math.floor(buff.dodge / 2),
      crit: Math.floor(buff.crit / 2),
    };
  }
  return buff;
}
...
function breedPets(parentA, parentB, userId) {
  ...
  const inheritedPassive =
    Math.random() > 0.5 ? parentA.passiveSkill : parentB.passiveSkill;

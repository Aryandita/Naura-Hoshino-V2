...
async function showGachaBannerShop(interaction, user) {
    ...
    const msg = await interaction.followUp({ ...payload, flags: MessageFlags.Ephemeral }).catch(() => null);
    if (!msg) return;

    const collector = msg.createMessageComponentCollector({
        filter: i => i.user.id === user.id && i.customId.startsWith("gacha_banner_"),
        time: 120000
    });

    collector.on("collect", async (i) => {
        if (i.customId === "gacha_banner_help") {
            const poolText = BANNER_POOL.map(b => `- **${b.name}** (Rarity: ${b.rarity}★)`).join("\n");
            return i.reply({
        ...
        if (i.customId === "gacha_banner_roll") {
            ...
            if (!currentSurvival || (currentSurvival.coupons || 0) < GACHA_COST_COUPON) {
                return i.reply({ content: "Kuponmu tidak cukup!", flags: MessageFlags.Ephemeral });
            }

            // Potong saldo
            await cacheManager.debitUserSurvival(user.id, "coupons", GACHA_COST_COUPON);

            // Roll logic (weighted random)
            const totalWeight = BANNER_POOL.reduce((acc, b) => acc + (10 - b.rarity), 0);

...
function addItem(inventory, id, amount) {
  const existing = inventory.find(
    (entry) => entry && typeof entry !== "string" && entry.id === id,
  );
  if (existing) {
...
async function createCraftingCanvas({
  heading,
  target,
  lines = [],
  ready,
  ...
  for (const line of lines) {
    ...
    ctx.fillText(
      line.name + "   " + line.have + " / " + line.need + suffix,
      36,
      y,
    );
    y += 30;
  ...
  const fallbackNote = ready
    ? "Bahannya lengkap! Ayo Naura kerjakan sekarang."

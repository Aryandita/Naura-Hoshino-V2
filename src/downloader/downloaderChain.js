...
const validateResult = async (data, platform) => {
  ...
  if (data.status === "picker" && Array.isArray(data.picker)) {
    ...
    for (const item of data.picker) {
      ...
      if (core.isBrandingHost(item.url)) {
        logger.info(
          `[Downloader] Menolak aset milik situs downloader: ${item.url.substring(0, 60)}`,
        );
        continue;
      }
      ...
      if (probe.ok) kept.push(item);
        ...
        logger.info(
          `[Downloader] Menolak item non-media (${probe.contentType || "tanpa tipe"}).`,
        );
    }
  ...
  if (typeof data.url === "string" && data.url.startsWith("http")) {
    ...
    if (!probe.ok) {
      logger.info(
        `[Downloader] Menolak hasil non-media (${probe.contentType || "tanpa tipe"}).`,
      );
      return null;
    }
    return data;
...
const runChain = async (url, platform, cleanup) => {
  ...
  logger.info(
    `[Downloader] Rantai (${chain.length} provider): ${chain.map((p) => p.name).join(" -> ")}`,
  );
  ...
  for (const provider of chain) {
    try {
      ...
      if (!clean) {
        logger.info(
          `[Downloader] ${provider.name} memberi hasil yang tidak lolos validasi.`,
        );
        continue;
      }
      ...
      logger.error(
        `[Downloader] ${provider.name} error: ${providerErr.message}`,
      );
    }
  }

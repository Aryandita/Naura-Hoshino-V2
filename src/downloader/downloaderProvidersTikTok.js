...
const tryTikwm = async (url) => {
  ...
  try {
    ...
    const d =
      response.data && response.data.code === 0 ? response.data.data : null;
    if (!d) return null;
...
const trySnapSave = async (url) => {
  ...
  try {
    logger.info(
      `[Downloader] Mencoba SnapSave untuk ${isTikTok ? "TikTok" : "Instagram"}...`,
    );
    ...
    const tokenMatch = String(mainPage.data).match(
      /name="token"\s+value="([^"]+)"/,
    );
    ...
    const videos = keepRealMedia(
      [...html.matchAll(/href="(https:\/\/[^"]+\.mp4[^"]*)"/g)].map(
        (m) => m[1],
      ),
      url,
    );
    ...
    if (videos.length > 0) {
      return videos.length === 1
        ...
        : {
            status: "picker",
            picker: videos.map((u) => ({ url: u, type: "video" })),
          };
    }
    ...
    if (isInstagram && images.length === 0) {
      logger.info(
        "[Downloader] SnapSave tidak memulangkan media Instagram yang sah.",
      );
      return null;
    }
    ...
    if (images.length > 0) {
      return images.length === 1
        ...
        : {
            status: "picker",
            picker: images.map((u) => ({ url: u, type: "photo" })),
          };
    }
  } catch (e) {
...
const trySSSAPI = async (url) => {
  ...
  try {
    ...
    const ttMatch = String(homePage.data).match(
      /id="([a-zA-Z0-9_]+)"\s+value="([a-zA-Z0-9_]+)"/,
    );
    ...
    const noWmMatch = html.match(
      /href="(https:\/\/[^"]+)"[^>]*>\s*(?:Without watermark|HD)/i,
    );

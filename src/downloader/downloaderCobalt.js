...
const fetchCobaltInstances = async () => {
  const now = Date.now();
  if (
    instanceCache.instances.length > 0 &&
    now - instanceCache.lastFetch < INSTANCE_CACHE_TTL
  ) {
  ...
  try {
    ...
    if (Array.isArray(dirRes.data)) {
      ...
      if (dynamicServers.length > 0) {
        logger.info(
          `[Downloader] Menemukan ${dynamicServers.length} server Cobalt dinamis.`,
        );
        const combined = [
          ...new Set([...dynamicServers.slice(0, 12), ...STATIC_INSTANCES]),
        ];
    ...
    logger.info(
      `[Downloader] Gagal ambil daftar instance: ${e.message}, pakai daftar statis.`,
    );
  }
...
const tryCobalt = async (url) => {
  ...
  logger.info(
    `[Downloader] Menjalankan ${instances.length} instance Cobalt secara paralel...`,
  );

...
const tryEmbed = async (shortcode) => {
  try {
    ...
    const isVideoPost =
      /<video[\s>]/i.test(html) || html.includes('"is_video":true');
    ...
    if (isVideoPost) {
      logger.info(
        "[Downloader][IG] Embed mendeteksi video tapi tanpa URL video, lanjut ke provider lain.",
      );
      return null;
    }
    ...
    const imageCandidates = [
      ...html.matchAll(
        /<img[^>]*class="[^"]*EmbeddedMediaImage[^"]*"[^>]*src="([^"]+)"/gi,
      ),
    ]

...
const streamToTempFile = async (link, platform, fallbackExt) => {
  ...
  const finalExt =
    core.mimeToExt(response.headers["content-type"] || "") || fallbackExt;
  const finalPath = path.join(
    os.tmpdir(),
    `naura_media_${Date.now()}.${finalExt}`,
  );
...
const shrink = async ({
  ...
  const targetText = preselectedHeight
    ? `ke resolusi **${preselectedHeight}p**`
    : "biar muat dikirim di Discord";
  ...
  const result = await compressUntilFits(
    filePath,
    limitBytes,
    limitMB,
    3,
 ...
 */
const prepareSingleMedia = async ({
  interaction,
  data,
  platform,
  cleanup,
  ...
  await interaction
    .editReply(render.inspectingCard(interaction.client))
    .catch(() => {});
  ...
  if (isLocalFile && data.url && fs.existsSync(data.url)) {
    ...
    const guessedExt =
      core.mimeToExt(probe.contentType) || extFromUrl(data.url) || "mp4";
    const sizeText =
      probe.sizeBytes > 0
        ? `${mb(probe.sizeBytes)} MB`
        : "ukuran belum diketahui";

    await interaction
      .editReply(render.fetchingCard(interaction.client, sizeText))
      .catch(() => {});

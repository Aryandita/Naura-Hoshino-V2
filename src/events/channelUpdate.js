  ...
  async execute(oldChannel, newChannel, client) {
    ...
    try {
      ...
      if (oldChannel.name !== newChannel.name) {
        changes.push(
          `**Nama:** \`${oldChannel.name}\` ➔ \`${newChannel.name}\``,
        );
      }
      if (oldChannel.topic !== newChannel.topic) {
        changes.push(
          `**Topik:** ${oldChannel.topic ? `\`${oldChannel.topic}\`` : "*Kosong*"} ➔ ${newChannel.topic ? `\`${newChannel.topic}\`` : "*Kosong*"}`,
        );
      }
      if (oldChannel.nsfw !== newChannel.nsfw) {
        changes.push(
          `**NSFW:** \`${oldChannel.nsfw ? "Ya" : "Tidak"}\` ➔ \`${newChannel.nsfw ? "Ya" : "Tidak"}\``,
        );
      }
      ...
      if (oldChannel.parentId !== newChannel.parentId) {
        changes.push(
          `**Kategori:** ${oldChannel.parent ? `\`${oldChannel.parent.name}\`` : "*Tidak ada*"} ➔ ${newChannel.parent ? `\`${newChannel.parent.name}\`` : "*Tidak ada*"}`,
        );
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `>>> Saluran <#${newChannel.id}> (\`${newChannel.name}\`) baru saja mengalami perubahan pengaturan.`,
        )
        .addFields({
          name: `${ui.getEmoji("id") || "📋"} Perincian Perubahan`,
          value: changes.join("\n"),
        })
        .setFooter({
          text: `Diubah oleh: ${executorTag}`,

...
module.exports = {
  ...
  async execute(message, client) {
    ...
    const hasSensitiveData =
      /(password|sandi|sandi=|pw|token|apikey|http|www\.)/gi.test(
        message.content,
      );
    if (!hasSensitiveData) {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `>>> Pesan milik <@${message.author.id}> di <#${message.channel.id}> telah dihapus.`,
        )
        .addFields(
          {
            name: `${ui.getEmoji("info") || "ℹ️"} Dihapus Oleh`,
            value: `\`${executorTag}\``,
            inline: false,
          },
          {
            name: `${ui.getEmoji("desc") || "📋"} Konten Terakhir`,
            value: message.content
              ? `\`\`\`text\n${message.content.substring(0, 1000)}\n\`\`\`${attachmentInfo}`
              : "*Hanya berisi lampiran/embed/stiker*",
          },
        )
        .setFooter({
          text: `User ID: ${message.author.id} • Channel ID: ${message.channel.id}`,
        })
        .setTimestamp();

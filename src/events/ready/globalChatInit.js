    ...
    (async () => {
      try {
        ...
        for (const settings of allSettings) {
          ...
          if (
            parsed.globalChat &&
            parsed.globalChat.enabled &&
            parsed.globalChat.channelId
          ) {
            client.globalChatChannels.set(
              parsed.globalChat.channelId,
              settings.guildId,
            );
          }
        ...
        console.log(
          `\x1b[42m\x1b[30m 🌐 GLOBAL CHAT \x1b[0m \x1b[32mBerhasil memuat ${client.globalChatChannels.size} channel Global Chat.\x1b[0m`,
        );
          ...
          await redisManager.initPubSub("naura:globalchat", async (payload) => {
            ...
            for (const [chanId] of client.globalChatChannels.entries()) {
              ...
              if (targetChannel && targetChannel.isTextBased()) {
                await targetChannel
                  .send({
                    embeds: [payload.embedData],
                    components: [payload.componentsData],
                  })
                  .catch(() => {});
              }
            }

      ...
      async () => {
        try {
          ...
          for (const profile of users) {
            ...
            // Cek cooldown daily
            const dailyTs = cooldowns.daily
              ? new Date(cooldowns.daily).getTime()
              : 0;
            if (now.getTime() - dailyTs >= DAILY_COOLDOWN_MS) {
              reminders.push(
                `${ui.getEmoji("reward") || "🎁"} **Daily** sudah bisa diklaim!`,
              );
            }

            // Cek cooldown work
            const workTs = cooldowns.work
              ? new Date(cooldowns.work).getTime()
              : 0;
            if (now.getTime() - workTs >= WORK_COOLDOWN_MS) {
              reminders.push(
                `${ui.getEmoji("coin") || "💼"} **Work** sudah bisa dikerjakan!`,
              );
            }

            // Cek cooldown mining
            const miningTs = cooldowns.mining
              ? new Date(cooldowns.mining).getTime()
              : 0;
            if (now.getTime() - miningTs >= MINING_COOLDOWN_MS) {
              reminders.push(
                `${ui.getEmoji("lootbox") || "📦"} **Mining** sudah bisa dijalankan!`,
              );
            }
            ...
            try {
              ...
              const embed = new EmbedBuilder()
                ...
                .setTitle(
                  `${ui.getEmoji("reward") || "🎁"} Waktunya Klaim Hadiah!`,
                )
                .setDescription(

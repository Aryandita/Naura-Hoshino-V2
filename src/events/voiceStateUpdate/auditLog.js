...
async function fetchRecentAuditEntry(guild, type) {
  ...
  if (cached && Date.now() - cached.fetchedAt < AUDIT_CACHE_MS)
    return cached.entry;

  const fetchedLogs = await guild
    .fetchAuditLogs({ limit: 1, type })
    .catch(() => null);
        ...
        if (logChannel) {
          if (!oldState.channelId && newState.channelId) {
            ...
            const embed = new EmbedBuilder()
              ...
              .setDescription(
                `>>> <@${member.id}> telah bergabung ke dalam saluran suara.`,
              )
              .addFields({
                name: `${ui.getEmoji("lokasi") || "📍"} Saluran Suara`,
            ...
            const disconnectLog = await fetchRecentAuditEntry(
              guild,
            ...
            const embed = new EmbedBuilder()
              ...
              .setDescription(
                `>>> <@${member.id}> telah meninggalkan saluran suara.`,
              )
              .addFields(
                {
                  name: `${ui.getEmoji("lokasi") || "📍"} Channel Terakhir`,
                  value: `<#${oldState.channelId}>`,
                  inline: true,
                },
                {
                  name: `${ui.getEmoji("info") || "ℹ️"} Keterangan`,
                  value: `\`${executorTag}\``,
                  inline: true,
                },
              )
            ...
            const moveLog = await fetchRecentAuditEntry(
              guild,
            ...
            const embed = new EmbedBuilder()
              ...
              .addFields(
                {
                  name: `${ui.getEmoji("lokasi") || "📍"} Dari`,
                  value: `<#${oldState.channelId}>`,
                  inline: true,
                },
                {
                  name: `${ui.getEmoji("lokasi") || "📍"} Ke`,
                  value: `<#${newState.channelId}>`,
                  inline: true,
                },
                {

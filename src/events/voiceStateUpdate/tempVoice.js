...
module.exports = {
  ...
  async execute(oldState, newState, client) {
    ...
    if (!tempConfig || !tempConfig.enabled || !tempConfig.triggerChannelId)
      return;
    ...
    if (newState.channelId && oldState.channelId !== newState.channelId) {
      ...
      if (channelJoined && channelJoined.name.startsWith("⏳ Wait - ")) {
        const ownerName = channelJoined.name
          .replace("⏳ Wait - ", "")
          .replace(/「🌟」・|「👑」・|🔊 /g, "")
          .replace(/ VIP Voice| Owner Voice| voice/g, "");

        // Cari channel owner yang sebenarnya
        const ownerChannel = guild.channels.cache.find(
          (c) =>
            c.parentId === tempConfig.categoryId &&
            (c.name.includes(`🔊 ${ownerName} voice`) ||
              c.name.includes(`「🌟」・${ownerName} VIP Voice`) ||
              c.name.includes(`「👑」・${ownerName} Owner Voice`)),
        );
          ...
          try {
            ...
            if (ownerMember) {
              ...
              const ownerProfile = await UserProfile.findOne({
                where: { userId: ownerMember.id },
              });
              const isBotOwner =
                env.OWNER_IDS && env.OWNER_IDS.includes(ownerMember.id);
    ...
    if (newState.channelId === tempConfig.triggerChannelId) {
      try {
        ...
        client.trackedTempChannels = new Set([
          ...(client.trackedTempChannels || new Set()),
          ...trackedTempChannels,
        ]);
        ...
        const row1 = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId("tvc_lock")
            .setLabel("Kunci")
            .setEmoji("\ud83d\udd12")
            .setStyle(ButtonStyle.Danger),
          new ButtonBuilder()
            .setCustomId("tvc_unlock")
            .setLabel("Buka")
            .setEmoji("\ud83d\udd13")
            .setStyle(ButtonStyle.Success),
          new ButtonBuilder()
            .setCustomId("tvc_hide")
            .setLabel("Sembunyikan")
            .setEmoji("\ud83d\udc7b")
            .setStyle(ButtonStyle.Secondary),
          new ButtonBuilder()
            .setCustomId("tvc_unhide")
            .setLabel("Tampilkan")
            .setEmoji("\ud83d\udc41\ufe0f")
            .setStyle(ButtonStyle.Secondary),
          new ButtonBuilder()
            .setCustomId("tvc_status")
            .setLabel("Status")
            .setEmoji("\ud83d\udcac")
        ...
        const row2 = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId("tvc_rename")
            .setLabel("Ganti Nama")
            .setEmoji("\u270f\ufe0f")
            .setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId("tvc_limit")
            .setLabel("Limit User")
            .setEmoji("\ud83d\udc65")
            .setStyle(ButtonStyle.Primary),
          new ButtonBuilder()
            .setCustomId("tvc_region")
            .setLabel("Region")
            .setEmoji("\ud83c\udf0d")
            .setStyle(ButtonStyle.Secondary),
          new ButtonBuilder()
            .setCustomId("tvc_stage")
            .setLabel("Stage Mode")
            .setEmoji("\ud83c\udf99\ufe0f")
            .setStyle(ButtonStyle.Danger),
          new ButtonBuilder()
            .setCustomId("tvc_waiting")
        ...
        const row3 = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId("tvc_save")
            .setLabel("Simpan Sesi")
            .setEmoji("\ud83d\udcbe")
            .setStyle(ButtonStyle.Secondary),
          new ButtonBuilder()
            .setCustomId("tvc_move")
            .setLabel("Pindah User")
            .setEmoji("\ud83d\udcf2")
            .setStyle(ButtonStyle.Primary),
        );
        ...
        logger.error(
          "\x1b[31m[VOICE ERROR]\x1b[0m Gagal memproses TempVoice:",
          error,
        );
      }
    }
    ...
    if (oldState.channelId) {
      ...
      const isTracked = oldChannel
        ? trackedTempChannels.has(oldChannel.id) ||
          (await tempVoiceRegistry.isTracked(oldChannel.id))
        : false;

      if (oldChannel && isTracked && oldChannel.members.size === 0) {
        // Menangani penghapusan Waiting Room dengan nama VIP/Owner maupun reguler
        let waitingRoomNameBase = oldChannel.name
          .replace("🔊 ", "")
          .replace(" voice", "");
        if (oldChannel.name.includes("「🌟」・")) {
          waitingRoomNameBase = oldChannel.name
            .replace("「🌟」・", "")
            .replace(" VIP Voice", "");
        ...
        const waitingRoom = oldChannel.guild.channels.cache.find(
          (c) =>
            c.name === waitingRoomName && c.parentId === tempConfig.categoryId,
        );

        await oldChannel.delete().catch(() => {});
        trackedTempChannels.delete(oldChannel.id);
        if (client.trackedTempChannels)
          client.trackedTempChannels.delete(oldChannel.id);
        ...
        if (waitingRoom && waitingRoom.members.size === 0) {
          ...
          if (client.trackedTempChannels)
            client.trackedTempChannels.delete(waitingRoom.id);

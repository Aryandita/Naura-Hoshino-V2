...
module.exports = {
  ...
  async execute(oldMessage, newMessage, client) {
    ...
    try {
      ...
      // ==========================================
      const settingsDB = await cacheManager.getGuildSettings(
        oldMessage.guild.id,
      );
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `>>> Aktivitas edit pesan terdeteksi di <#${oldMessage.channel.id}>.\n[🔗 Lompat ke Lokasi Pesan Saat Ini](${newMessage.url})`,
        )
        .addFields(
          {
            name: `${ui.getEmoji("rename") || "📝"} Diedit Oleh`,
            value: `\`${oldMessage.author.globalName || oldMessage.author.username} (@${oldMessage.author.username})\``,
            inline: false,
          },
          {
            name: `${ui.getEmoji("desc") || "📋"} Konten Original`,
            value: `\`\`\`text\n${oldContent}\n\`\`\``,
            inline: false,
          },
          {
            name: `${ui.getEmoji("desc") || "📋"} Konten Sesudah Diedit`,
            value: `\`\`\`text\n${newContent}\n\`\`\``,
            inline: false,
          },
        )

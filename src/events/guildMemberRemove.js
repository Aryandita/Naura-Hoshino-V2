...
module.exports = {
    ...
    async execute(member) {
        ...
        try {
            ...
            if (greetings && greetings.leave && greetings.leave.enabled && greetings.leave.channelId) {
                ...
                if (channel) {
                    ...
                    if (leaveData.image) {
                        try {
                            ...
                            const imageBuffer = await generateWelcomeImage(member, 'leave', ui, bgUrl);
                    ...
                    await channel.send(payload).catch(err => logger.error('[Leave Send Error]', err));
                }
            }
        } catch (error) {

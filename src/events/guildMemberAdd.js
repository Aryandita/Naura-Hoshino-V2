...
function assignableRoles(member, ids) {
    ...
    return ids.filter(id => {
        ...
        return role && role.position < ceiling && !role.managed && role.id !== member.guild.id;
    });
}
...
async function applyAutoRole(member, settings) {
    ...
    await member.roles.add(valid).catch(err =>
        logger.warn(`[AutoRole] Gagal memberi role ke ${member.user.tag}: ${err.message}`)
    );
}
...
async function sendWelcome(member, settings) {
    ...
    if (welcome.image) {
        try {
            ...
            const buffer = await generateWelcomeImage(member, 'welcome', ui, bgUrl, welcome);

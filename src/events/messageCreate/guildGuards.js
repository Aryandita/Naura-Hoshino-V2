...
async function softbanTrap(message, currentSet) {
    const trapId = currentSet.softbanChannelId
    ...
    await message.member.ban({ reason, deleteMessageSeconds: BAN_PURGE_SECONDS }).catch(async () => {
    ...
    if (logChannel) {
        ...
        await logChannel.send(buildContainerV2({
    ...
    logger.info(`[Scammer Trap] Ban ${message.author.tag} (${message.author.id}) di ${message.guild.name}`);
    return true;
}
...
async function minecraftBridge(message, settings) {
    ...
    if (!mc || !mc.bridgeEnabled || mc.bridgeChannelId !== message.channel.id) return;

...
module.exports = async function handleLegacyAutomod(message, client, ctx) {
  ...
  if (
    ...
    !ALLOWED_HOSTS.some((host) => content.includes(host))
  ) {
    ...
    content.length > 5 &&
    BAD_WORDS.some((word) => content.includes(word))
  ) {
    try {
  ...
  const current =
    profile.mannersPoint !== undefined ? profile.mannersPoint : DEFAULT_MANNERS;
  ...
  if (profile.mannersPoint === 0 && automod.punishRole) {
    try {
      ...
      if (canPunish) {
        ...
        await message.channel.send(
          `${ui.getEmoji("hmph") || "\uD83D\uDEA8"} <@${message.author.id}>, poin tata kramamu habis (0/100). Naura terpaksa mengisolasimu dulu, yaa.`,
        );
      } else {
        await message.channel.send(
          `${ui.getEmoji("akward") || "\uD83D\uDEA8"} <@${message.author.id}>, poin tata kramamu habis (0/100), tapi Naura nggak punya izin yang cukup untuk memberi hukuman.`,
        );
      }
    } catch (err) {
    ...
    if (warning)
      setTimeout(() => warning.delete().catch(() => {}), WARNING_LIFETIME_MS);
  }

...
module.exports = async function handleCounting(message, client, ctx) {
  ...
  const game = ctx.settings.countingGame || {
    currentNumber: 0,
    lastUser: null,
  };
  ...
  const reset = async (reason) => {
    ...
    if (notice)
      setTimeout(() => notice.delete().catch(() => {}), FAIL_NOTICE_MS);
  ...
  if (game.lastUser === message.author.id) {
    return reset(
      `Eh, **${message.author.username}**, nggak boleh menghitung dua kali berturut-turut.`,
    );
  }

  if (input !== expected) {
    return reset(
      `Yah, **${message.author.username}**, harusnya **${expected}**.`,
    );
  }

  await message.react(successEmoji).catch(() => {});
  ctx.settings.countingGame = {
    currentNumber: input,

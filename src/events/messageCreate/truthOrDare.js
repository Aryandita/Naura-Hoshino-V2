...
function choiceRow(playerId) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`tod_chan_truth_${playerId}`)
      .setLabel("Truth")
      .setStyle(ButtonStyle.Success)
      .setEmoji(ui.getEmoji("read") || "\uD83D\uDCDD"),
    new ButtonBuilder()
      .setCustomId(`tod_chan_dare_${playerId}`)
      .setLabel("Dare")
      .setStyle(ButtonStyle.Danger)
      .setEmoji(ui.getEmoji("hmph") || "\uD83D\uDE08"),
    new ButtonBuilder()
      .setCustomId(`tod_chan_spin_${playerId}`)
      .setLabel("Spin")
      .setStyle(ButtonStyle.Primary)
      .setEmoji(ui.getEmoji("chirping") || "\uD83D\uDD04"),
  );
}
...
module.exports = async function handleTruthOrDare(message, client, ctx) {
  ...
  const intro = new EmbedBuilder()
    ...
    .setTitle(
      `${ui.getEmoji("happy") || "\uD83C\uDFAD"} Truth or Dare bareng Naura!`,
    )
    .setDescription(
  ...
  const board = await message.channel.send({
    embeds: [intro],
    components: [choiceRow(currentPlayerId)],
  });
  ...
  collector.on("collect", async (i) => {
    ...
    if (i.customId.startsWith("tod_chan_done_")) {
      const done = new EmbedBuilder()
        ...
        .setTitle(
          `${ui.getEmoji("cheers") || "\uD83C\uDF89"} Tantangan selesai!`,
        )
        .setDescription(
      ...
      return i.editReply({
        ...
        components: [
          new ActionRowBuilder().addComponents(
            new ButtonBuilder()
              .setCustomId(`tod_chan_next_${currentPlayerId}`)
              .setLabel("Main lagi")
              .setStyle(ButtonStyle.Primary),
          ),
        ],
    ...
    if (i.customId.startsWith("tod_chan_spin_")) {
      ...
      try {
        ...
        const candidates = members.filter(
          (m) => !m.user.bot && m.user.id !== currentPlayerId,
        );
      ...
      const spun = new EmbedBuilder()
        .setColor(ui.getColor("primary") || "#FFB6C1")
        .setTitle(
          `${ui.getEmoji("shocked") || "\uD83D\uDD04"} Botolnya berhenti!`,
        )
        .setDescription(
    ...
    if (!challenge)
      challenge = pick(isTruth ? FALLBACK_TRUTHS : FALLBACK_DARES);

    const player = await i.client.users
      .fetch(currentPlayerId)
      .catch(() => i.user);
    const card = new EmbedBuilder()
      .setColor(
        isTruth
          ? ui.getColor("success") || "#2ecc71"
          : ui.getColor("error") || "#e74c3c",
      )
      .setAuthor({
    ...
    return i.editReply({
      ...
      components: [
        new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId(`tod_chan_done_${currentPlayerId}`)
            .setLabel("Selesai")
            .setStyle(ButtonStyle.Success),
          new ButtonBuilder()
            .setCustomId(`tod_chan_next_${currentPlayerId}`)

...
module.exports = async function handleStaffReply(message, client) {
        ...
        if (user) {
            await user.send({
                embeds: [new EmbedBuilder()
                    ...
                    .setTitle(`${ui.getEmoji('sleepy') || '\uD83D\uDD12'} Sesi ditutup`)
                    .setDescription(`Obrolanmu dengan staf **${message.guild.name}** sudah Naura tutup. Kalau butuh bantuan lagi, sapa Naura kapan saja, yaa!`)]
            }).catch(() => {});
        }

        await message.channel.send('Naura rapikan channel ini dalam 5 detik, yaa...');
    ...
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ text: `Staf: ${message.author.username}`, iconURL: message.author.displayAvatarURL() })
            .setTimestamp();

        const files = message.attachments.size > 0
            ? message.attachments.map(a => new AttachmentBuilder(a.url, { name: a.name }))
            : [];
        ...
        await message.channel.send({
            embeds: [new EmbedBuilder()
                .setColor(ui.getColor('error') || '#FF0000')
                .setDescription(`${ui.getEmoji('cry') || '\u274C'} Yah, pesannya nggak sampai. Sepertinya DM pengguna itu tertutup.`)]
        }).catch(() => {});
    }

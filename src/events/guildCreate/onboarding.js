...
module.exports = {
    ...
    async execute(guild, client) {
        logger.info(`[ONBOARDING] Naura bergabung ke server baru: ${guild.name} (${guild.id})`);
        ...
        if (!targetChannel) {
            logger.info(`[ONBOARDING] Tidak ada channel teks yang bisa ditulis di server ${guild.id}.`);
            return;
        }
        ...
        try {
            ...
            logger.warn(`[ONBOARDING] Gagal mengirim pesan wizard ke ${guild.id}:`, error.message);
        }
    }
};

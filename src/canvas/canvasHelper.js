...
async function generateMusicProfileImage(user, stats, clientAvatar) {
  ...
  try {
    userAvatarImg = await canvasRuntime.loadImage(user.displayAvatarURL({ extension: 'png', size: 256 }));
  } catch (_) {
    try { userAvatarImg = await canvasRuntime.loadImage(clientAvatar); } catch (_2) {}
  }
  if (userAvatarImg) {
    ctx.save();
    ctx.beginPath();
  ...
  // Naura avatar circle
  let nauraImg = null;
  try { nauraImg = await canvasRuntime.loadImage(clientAvatar); } catch (_) {}
  if (nauraImg) {
    ctx.save();
    ctx.beginPath();

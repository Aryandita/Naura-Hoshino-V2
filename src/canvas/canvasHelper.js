...
async function generateMusicProfileImage(user, stats, clientAvatar) {
  const canvas = canvasRuntime.createCanvas(1000, 650); // Tinggi diperbesar untuk muat 5 list
  ...
  // ==========================================
  try {
    const nauraLogoImg = await canvasRuntime.loadImage(clientAvatar);
...
async function generateMusicPanelImage(track, currentPos, clientAvatar) {
  const canvas = canvasRuntime.createCanvas(600, 280);
  const ctx = canvas.getContext("2d");
  ...
  else if (source.includes("youtube") || source.includes("ytm"))
    accentColor = "#FF0000";
  ...
  try {
    ...
    try {
      if (
        ...
        const response = await axios.get(clientAvatar, {
          responseType: "arraybuffer",
          timeout: 5000,
        });
        trackThumbImg = await canvasRuntime.loadImage(Buffer.from(response.data));

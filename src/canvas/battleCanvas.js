...
async function drawBattle(playerInfo, enemyInfo, roundLog) {
  ...
  try {
    ...
    console.error(
      "\x1b[41m\x1b[37m 💥 BattleCanvas \x1b[0m \x1b[31mGagal memuat background:",
      err.message,
      "\x1b[0m",
    );
  }
  ...
  ctx.fillText(
    `Class: ${(playerInfo.class || "No Class").toUpperCase()}`,
    160,
    115,
  );
  ...
  const playerHpPercent = Math.max(
    0,
    Math.min(1, playerInfo.hp / playerInfo.maxHp),
  );
  ctx.fillStyle =
    playerHpPercent > 0.5
      ? "#2ecc71"
  ...
  const playerStaminaPercent = Math.max(
    0,
    Math.min(1, playerInfo.stamina / 100),
  );
  ...
  try {
    ...
    console.error(
      "\x1b[41m\x1b[37m 💥 BattleCanvas \x1b[0m \x1b[31mGagal memuat monster image:",
      e.message,
      "\x1b[0m",
    );
  }
  ...
  const enemyHpPercent = Math.max(
    0,
    Math.min(1, enemyInfo.hp / enemyInfo.maxHp),
  );

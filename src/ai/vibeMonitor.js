...
const VIBE_PATTERNS = [
  {
    ...
    emojis: ["🎉", "✨", "🔥", "👑"],
    regex:
      /\b(dapet\s+ur|gacha\s+ur|dapet\s+ssr|gacha\s+ssr|level\s+100|menang\s+duel|win\s+streak|hoki\s+banget|ez\s+dungeon|juara\s+1)\b/i,
  },
  {
    type: "sympathy",
    emojis: ["🥺", "💙", "🫂", "🍵"],
    regex:
      /\b(kalah\s+duel|streak\s+putus|zonk\s+gacha|mati\s+di\s+dungeon|kalah\s+terus|apes\s+banget|sial\s+banget)\b/i,
  },
  {
    type: "affection",
    emojis: ["🌸", "💖", "🥰", "✨"],
    regex:
      /\b(makasih\s+naura|terima\s+kasih\s+naura|love\s+naura|naura\s+cantik|naura\s+lucu|naura\s+terbaik|naura\s+kawaii)\b/i,
  },
];
...
class VibeMonitor {
  ...
  async handleMessage(message) {
    ...
    if (
      now - lastUser < USER_COOLDOWN_MS ||
      now - lastChannel < CHANNEL_COOLDOWN_MS
    ) {
      return;
    }
    ...
    for (const pattern of VIBE_PATTERNS) {
      if (pattern.regex.test(text)) {
        ...
        const emoji =
          pattern.emojis[Math.floor(Math.random() * pattern.emojis.length)];

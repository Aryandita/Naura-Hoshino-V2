...
function stripCustomEmojis(text) {
    if (!text) return '';
    return String(text).replace(/<a?:\w+:\d+>/g, '').trim();
}
...
function progressBar(current, max, length, filledEmoji, emptyEmoji) {
    ...
    return filledEmoji.repeat(filledLength) + emptyEmoji.repeat(span - filledLength);
}
...
async function sendError(interaction, errorMessage, ephemeral = false) {
    ...
    try {
        ...
        if (interaction.deferred || interaction.replied) {
            ...
            msg = await interaction.reply({ ...containerPayload, ephemeral, fetchReply: !ephemeral });
        }

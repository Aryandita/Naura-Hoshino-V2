...
test("UI Footer Configuration - Resolves version dynamically from env.BOT_VERSION", () => {
  const currentVersion = env.BOT_VERSION || "2.1.0";

  const coreFooter = ui.getFooter("core");
  assert.ok(coreFooter.includes(`v${currentVersion}`), `Footer core harus memuat v${currentVersion}`);
  assert.ok(coreFooter.includes("Aryandita"), "Footer core harus memuat pembuat");

  const utilityFooter = ui.getFooter("utility");
  assert.ok(utilityFooter.includes(`v${currentVersion}`), `Footer utility harus memuat v${currentVersion}`);

  const survivalFooter = ui.getFooter("survival");
  assert.ok(survivalFooter.includes(`v${currentVersion}`), `Footer survival harus memuat v${currentVersion}`);

  const musicFooter = ui.getFooter("music");
  assert.ok(musicFooter.includes(`v${currentVersion}`), `Footer music harus memuat v${currentVersion}`);

  const premiumFooter = ui.getFooter("premium");
  assert.ok(premiumFooter.includes(`v${currentVersion}`), `Footer premium harus memuat v${currentVersion}`);
});
...
test("UI Footer - Fallback to core when category is invalid or missing", () => {
  ...
  assert.equal(fallback, core, "Kategori tidak dikenal harus fallback ke footer core");
});

...
function warmCache(userId) {
    ...
    Promise.resolve()
        ...
        .catch(error => logger.warn(`[LocalePatch] Gagal memuat bahasa ${userId}: ${error.message}`))
        .finally(() => warming.delete(userId));
}

function contextUserId(context) {
    if (!context) return null;
    return (context.user && context.user.id) || (context.author && context.author.id) || null;
}
...
function patchPrototype(proto, label) {
    ...
    if (defineIfAbsent(proto, 'localeLang', {
        get() { return resolveLanguageSync(this); }
    })) applied++;

    if (defineIfAbsent(proto, 'lang', {
        get() { return resolveLanguageSync(this); }
    })) applied++;

    if (defineIfAbsent(proto, 't', {
        value: function translate(key, placeholders) {
            return languageManager.translateSync(resolveLanguageSync(this), key, placeholders || {});
        },
        writable: true
    })) applied++;

    if (defineIfAbsent(proto, 'fetchLang', {
    ...
    if (applied > 0) {
        logger.info(`[LocalePatch] ${applied} properti bahasa dipasang pada ${label}.`);
    }
    return applied;
}
...
function applyLocalePatch() {
    ...
    try {
        patchPrototype(BaseInteraction && BaseInteraction.prototype, 'BaseInteraction');
        patchPrototype(Message && Message.prototype, 'Message');
        return true;
    } catch (error) {
        logger.error(`[LocalePatch] Gagal memasang penambal bahasa: ${error.message}`);
        return false;
    }
}

...
test('countComponents ikut menghitung komponen bersarang dan accessory', () => {
    const components = [
        ...
        { type: 12, items: [{ media: { url: 'attachment://b.png' } }, { media: { url: 'attachment://c.png' } }] },
        textDisplay('body')
    ];
...
test('field berlebih dibuang dan diganti catatan, tombol tetap utuh', () => {
    ...
    const buttons = { type: 1, components: [{ type: 2, label: 'Lanjut', custom_id: 'next' }] };
    const fields = Array.from({ length: 45 }, (_, i) => textDisplay(`field ${i}`));
    ...
    const hasNotice = result.components.some(c => c.type === 10 && c.content === '-# Sebagian isi dipotong.');
...
test('teks yang terlalu panjang juga memicu pemotongan', () => {
    ...
    const components = [textDisplay('header'), long, textDisplay('y'.repeat(2500))];

    const result = enforceComponentBudget(components, { droppableIndices: [1, 2], notice: '-# dipotong' });

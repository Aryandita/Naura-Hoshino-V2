...
async function analyzeMessageWithAI(reportedMessage, aiAutomodSettings) {
    ...
    try {
        ...
        if (cached) {
            logger.info(`[AI Automod] Hasil analisis diambil dari cache untuk message ${reportedMessage.id}`);
            return cached;
        }
    } catch (e) {
    ...
    try {
        ...
        if (!jsonMatch) {
            logger.warn('[AI Automod] Respons Gemini tidak mengandung JSON valid:', rawText.substring(0, 200));
            return null;
        }
...
async function handleAIReport(interaction, guildSettings) {
    ...
    if (reportedMessage.author.bot) {
        return interaction.reply({
            ...buildErrorContainerV2({ errorMessage: 'Tidak bisa melaporkan pesan dari bot.' }),
    ...
    if (reportedMessage.author.id === interaction.user.id) {
        return interaction.reply({
            ...buildErrorContainerV2({ errorMessage: 'Tidak bisa melaporkan pesanmu sendiri.' }),
            flags: MessageFlags.Ephemeral
        });
    }
    ...
    if (cooldown.onCooldown) {
        ...
        return interaction.reply({
            ...buildErrorContainerV2({ errorMessage: `Tunggu ${remaining} detik sebelum melaporkan lagi.` }),
            flags: MessageFlags.Ephemeral
        });
    }
    ...
    if (!analysis) {
        return interaction.editReply({
            ...buildErrorContainerV2({ errorMessage: 'AI Automod tidak dapat menganalisis pesan ini saat ini. Coba lagi nanti.' }),
        });
    }
    ...
    if (auditChannelId) {
        ...
        if (auditChannel) {
            ...
            const auditPayload = buildContainerV2({
                accentColorHex: score >= 70 ? '#FF0000' : score >= 30 ? '#FFD700' : '#00FF00',
    ...
    if (isViolation && !isLearningMode) {
        const targetMember = await interaction.guild.members.fetch(reportedMessage.author.id).catch(() => null);

        // Hapus pesan
        await reportedMessage.delete().catch(() => {});

        // Timeout 10 menit jika bot punya permission
        if (targetMember && interaction.guild.members.me?.permissions.has(PermissionFlagsBits.ModerateMembers)) {
            await targetMember.timeout(10 * 60 * 1000, `AI Automod: ${analysis.category} (skor ${score})`).catch(() => {});
        }
...
module.exports = { analyzeMessageWithAI, handleAIReport, checkReportCooldown, setReportCooldown };

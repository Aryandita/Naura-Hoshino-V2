    ...
    static async log(client, guildId, options) {
        ...
        try {
            ...
            await channel.send(container).catch(err => {
                logger.warn(`[AuditLogger] Gagal mengirim log ke channel ${channelId} di guild ${guildId}: ${err.message}`);
            });
        } catch (e) {
            logger.warn(`[AuditLogger] Kesalahan memproses log audit untuk guild ${guildId}: ${e.message}`);
        }
    }
}

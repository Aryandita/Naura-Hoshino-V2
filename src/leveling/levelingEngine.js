  ...
  try {
    ...
    const channelId =
      ...
      (data &&
        ...
        data.settings.channels.levelUp);
...
async function announceLevelUp(profile, user, guild, currentChannel) {
  ...
  try {
    ...
    const buffer = canvas.encodeSync
      ? canvas.encodeSync("webp")
      : canvas.toBuffer();
    ...
    await targetChannel
      .send(
        `Hore! **${user.username}** baru naik ke **Level ${profile.level}**!`,
      )
      .then((msg) =>
        setTimeout(() => msg.delete().catch(() => {}), CONFIG.NOTICE_TTL),
      )
      .catch(() => {});
  }
}
...
async function awardXpDirect(
  ...
  if (redisManager.isReady()) {
    await redisManager.setCache(
      cooldownKey,
      true,
      Math.floor(CONFIG.MSG_COOLDOWN / 1000),
    );
    // 1. Fetch current profile from DB
    let profile = await UserLeveling.findOne({
      where: { userId: user.id, guildId: guild.id },
    });
    if (!profile)
      profile = { xp: 0, level: 1, messageCount: 0, lastActivity: new Date(0) };

    // 2. Retrieve cached known level
    const cachedLevelStr = await redisManager.getCache(
      `user_level_${guild.id}_${user.id}`,
    );
    const currentKnownLevel = cachedLevelStr
      ? parseInt(cachedLevelStr, 10)
      : profile.level;

    // 3. Add XP to Redis buffer
    const bufferedXp = await xpBuffer.addXp(
      guild.id,
      user.id,
      precomputedGain === null ? await rollXp(user.id) : precomputedGain,
    );
  ...
  const lastActivity = profile.lastActivity
    ? new Date(profile.lastActivity).getTime()
    : 0;
  ...
  await profile.save({
    fields: ["xp", "level", "messageCount", "lastActivity"],
  });

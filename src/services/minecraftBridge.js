...
class MinecraftBridgeService {
  ...
  static async generateLinkCode(userId, mcUsername) {
    ...
    await redisManager.set(`mc:link:code:${code}`, JSON.stringify({ userId, mcUsername }), "EX", 900); // 15 menit
  ...
  static async verifyAccount(code) {
    ...
    const link = await MinecraftLink.findOne({ where: { userId: data.userId } });
    if (!link) return null;
    ...
    await currency.reward(data.userId, { starFragments: 500 }, "Minecraft Account Linking Bonus");
  ...
  static async syncRewards(userId) {
    const link = await MinecraftLink.findOne({ where: { userId, isVerified: true } });
    if (!link) {
      return { success: false, message: "Akun Minecraft belum ditautkan atau belum diverifikasi." };
    }

    const now = Date.now();
    const lastSync = link.lastSyncedAt ? new Date(link.lastSyncedAt).getTime() : 0;
    const hoursSince = Math.floor((now - lastSync) / (1000 * 60 * 60));

    if (hoursSince < 1) {
      return {
        success: false,
        message: "Sinkronisasi reward hanya dapat dilakukan setiap 1 jam sekali.",
      };
    }
    ...
    await currency.reward(userId, { starFragments: rewardFrags, coupons: rewardCoupons }, "Minecraft Playtime Realm Sync");

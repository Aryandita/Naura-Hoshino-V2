...
class EconomyGuardEngine {
    ...
    calculateDynamicTax(totalMoneySupply = 0) {
        ...
        const { minTaxRate, maxTaxRate, baseTaxRate, inflationThresholdTier1, inflationThresholdTier2 } = this.config;
        ...
        if (supply < inflationThresholdTier1) {
            ...
            return Number((minTaxRate + (baseTaxRate - minTaxRate) * ratio).toFixed(4));
        }
        ...
        const ratio = (supply - inflationThresholdTier1) / (inflationThresholdTier2 - inflationThresholdTier1);
    ...
    evaluateTransaction(senderId, receiverId, amount, type = 'transfer') {
        if (!senderId || !receiverId) return { allowed: false, reason: 'ID pengguna tidak valid.' };
        if (senderId === receiverId) return { allowed: false, reason: 'Pengirim dan penerima tidak boleh sama.' };
        
        const transferAmount = Number(amount) || 0;
        if (transferAmount <= 0) return { allowed: false, reason: 'Jumlah transfer harus lebih dari 0.' };
        ...
        const currentVolume = history.reduce((sum, t) => sum + t.amount, 0) + transferAmount;

        // 4. Deteksi anomali frekuensi transaksi (Rapid Burst)
        if (currentCount > this.config.maxTransfersPerWindow) {
            this._tripCircuitBreaker(senderId, 'Frekuensi transfer terlalu tinggi dalam waktu singkat (Spam Transfer).');
            this._logSuspiciousEvent(senderId, receiverId, transferAmount, 'EXCESSIVE_TRANSFER_FREQUENCY');
            return {
                allowed: false,
                reason: 'Terdeteksi lonjakan frekuensi transfer berlebihan. Akun diblokir sementara dari transaksi selama 10 menit.',
            };
        }

        // 5. Deteksi anomali volume transaksi (Drain Attack)
        if (currentVolume > this.config.maxVolumePerWindow) {
            this._tripCircuitBreaker(senderId, 'Volume akumulasi transfer melampaui batas keamanan jendela waktu.');
            this._logSuspiciousEvent(senderId, receiverId, transferAmount, 'EXCESSIVE_TRANSFER_VOLUME');
            return {
                allowed: false,
                reason: 'Total volume transfer Anda melampaui batas keamanan. Silakan coba lagi setelah 10 menit.',
            };
        }
    ...
    _tripCircuitBreaker(userId, reason) {
        ...
        logger.warn(`[EconomyGuard] Circuit Breaker diaktifkan untuk user ${userId}: ${reason}`);
    }
    ...
    async _logSuspiciousEvent(senderId, receiverId, amount, alertType) {
        ...
        logger.warn(`[EconomyGuard Audit] ${alertType} - Sender: ${senderId} -> Receiver: ${receiverId}, Amount: ${amount}`);

        try {
            if (mongoManager && mongoManager.isConnected()) {
                const db = mongoManager.getDb();
                if (db) {
                    await db.collection('economy_security_alerts').insertOne(payload).catch(() => {});
                }
            }
        } catch (_e) {

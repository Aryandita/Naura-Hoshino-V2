...
test('WebhookDispatcher - Event Types and validation', async () => {
    ...
    const result = await webhookDispatcher.dispatch('invalid-url', EVENT_TYPES.MEMBER_LEVEL_UP, { userId: '123' });
    assert.equal(result, false);

    const emptyResult = await webhookDispatcher.dispatch('', EVENT_TYPES.TICKET_CREATED, {});

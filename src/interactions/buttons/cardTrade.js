...
module.exports = [
  {
    ...
    async handler(interaction, client) {
      ...
      const tradeId = customId
        .replace("card_trade_accept_", "")
        .replace("card_trade_decline_", "");
      ...
      if (!tradeDataRaw) {
        return interaction.reply({
          ...buildErrorContainerV2({
            ...
            description:
              "Sesi barter kartu ini telah berakhir atau dibatalkan.",
          }),
      ...
      const trade =
        typeof tradeDataRaw === "string"
          ? JSON.parse(tradeDataRaw)
          : tradeDataRaw;
      ...
      try {
        ...
        try {
          ...
          const cardA = await UserCard.findOne({
            where: {
              cardCode: trade.initiatorCardCode,
          ...
          if (trade.starFragmentsOffer > 0) {
            ...
            if (
              !survivalA ||
              (survivalA.starFragments || 0) < trade.starFragmentsOffer
            ) {
              await t.rollback();
              return interaction.reply({
            ...
            survivalB.starFragments =
              (survivalB.starFragments || 0) + trade.starFragmentsOffer;

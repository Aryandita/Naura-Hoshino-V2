...
module.exports = [
...

    {
        ...
        defer: 'reply',
        onError: 'Gagal mengaktifkan Automod. Periksa izin Kelola Role dan Kelola Channel milik bot.',
        async handler(interaction) {
            const existing = await updateGuildSetting(interaction.guild.id, () => {});
            let punishRole = interaction.guild.roles.cache.get(existing.automod?.punishRole);
                    ...
                    for (const channel of interaction.guild.channels.cache.values()) {
                        if (channel.isTextBased() || channel.isVoiceBased()) {
                            await channel.permissionOverwrites
                                .create(punishRole, { ViewChannel: false, SendMessages: false, Connect: false })
                                .catch(() => {});
                        }
                    }
                }

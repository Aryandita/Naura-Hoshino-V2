...
module.exports = [
  {
    ...
    async handler(interaction) {
      ...
      const currentSelect = interaction.message.components?.[0]?.components?.find(
        (c) => c.customId === "help_category_select" || c.data?.custom_id === "help_category_select",
      );
      const currentVal = currentSelect?.options?.find((o) => o.default)?.value || "overview";
      const currentIndex = Math.max(0, HELP_CATEGORY_KEYS.indexOf(currentVal) - 1);

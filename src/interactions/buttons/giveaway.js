...
async function checkRequirements(interaction, requirements) {
  ...
  const { requiredRoleId, minLevel, minAccountAgeDays, mustBeBooster } =
    requirements;
    ...
    async handler(interaction) {
      ...
      try {
        await sequelize.transaction(async (t) => {
          ...
          if (!gw) {
            resultMessage = {
              error: "Giveaway ini sudah tidak ada di database.",
            };
            return;
          }
          ...
          const participants = Array.isArray(gw.participants)
            ? gw.participants
            : [];
          ...
          const rejectionReason = await checkRequirements(
            interaction,
            gw.requirements,
          );
          if (rejectionReason) {

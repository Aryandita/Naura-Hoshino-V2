...
const { UserProfile, UserSurvival, UserCosmetic, UserPet, UserWarn } = require('../../models');
    ...
    async execute(interaction, customId, client) {
        ...
        if (action === 'cancel') {
            await interaction.update({
                content: 'Syukurlah! Naura senang kakak memutuskan untuk tetap menyimpan datanya bersama Naura~ 🥰',
                embeds: [],
                components: []
            });
            return true;
        ...
        if (action === 'confirm') {
            try {
                ...
                await interaction.update({
                    content: '✅ Selesai... Naura sudah menghapus seluruh data kakak. Terima kasih sudah bermain bersama Naura ya, kak! Sampai jumpa lagi... 🥺👋',
                    embeds: [],
                    components: []
                });
            } catch (err) {
                logger.error(`[DATA DELETE] Gagal menghapus data user ${targetUserId}:`, err);
                await interaction.update({
                    content: '❌ Maaf kak, terjadi kesalahan saat mencoba menghapus datamu. Tolong lapor ke developer ya!',
                    embeds: [],
                    components: []
                });
            }

...
const ACTIONS = {
  ...
  async tvc_hide(interaction, ctx) {
    if (!ctx.isPrivileged) {
      return interaction.reply({
        content: PREMIUM_ONLY("Mode Invisible (Ghosting)"),
  ...
  async tvc_stage(interaction, ctx) {
    const overwrite = ctx.channel.permissionOverwrites.cache.get(
    ...
    await ctx.channel.permissionOverwrites.edit(interaction.user.id, {
      Speak: true,
    });
  ...
  tvc_region(interaction, ctx) {
    if (!ctx.isPrivileged) {
      return interaction.reply({
        content: PREMIUM_ONLY("Ubah Region"),
        flags: MessageFlags.Ephemeral,
      });
    }
  ...
  async tvc_save(interaction, ctx) {
    if (!ctx.isPrivileged) {
      return interaction.reply({
        content: PREMIUM_ONLY("Simpan Sesi"),
        flags: MessageFlags.Ephemeral,
      });
    }
...
module.exports = [
  {
    ...
    async handler(interaction) {
      ...
      if (!ctx.isOwner && !ctx.isAdmin)
        return ui.sendError(interaction, "err_sys_75", true);
      ...
      if (!action) {
        return interaction.reply({
          content:
            "\u2753 Tombol ini sudah tidak berlaku. Panggil ulang panelnya ya.",
          flags: MessageFlags.Ephemeral,
        });
      }

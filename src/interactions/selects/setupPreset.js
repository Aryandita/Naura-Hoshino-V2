        ...
        async handler(interaction, client) {
            ...
            if (!interaction.member.permissions.has('Administrator')) {
                const errPayload = buildErrorContainerV2({
                    title: 'Akses Ditolak',
                    description: '❌ | Hanya Administrator server yang bisa menggunakan Setup Wizard ini.',
            ...
            try {
                ...
                if (preset === 'preset_community') {
                    currentSettings.automod = { enabled: true, antiInvite: true, antiCaps: true, massMention: 5, antiSpam: true, badWords: [] };
                    currentSettings.features = { leveling: true, economy: true, music: true };
                } else if (preset === 'preset_gaming') {
                    currentSettings.automod = { enabled: false, antiInvite: false, antiCaps: false, massMention: 10, antiSpam: true, badWords: [] };
                    currentSettings.features = { leveling: true, economy: true, music: true };
                } else if (preset === 'preset_minimal') {
                    currentSettings.automod = { enabled: false, antiInvite: false, antiCaps: false, massMention: 5, antiSpam: false, badWords: [] };
                    currentSettings.features = { leveling: false, economy: false, music: true };
                }
                ...
                logger.error(`[ONBOARDING PRESET] Gagal menyimpan preset untuk ${guildId}:`, error);
                const errPayload = buildErrorContainerV2({
                    title: 'Terjadi Kesalahan',
                    description: '❌ | Gagal menyimpan pengaturan preset. Silakan coba lagi nanti.',
                    footerText: 'Naura Setup Wizard'
                });

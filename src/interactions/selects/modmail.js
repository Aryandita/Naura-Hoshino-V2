...
async function openTicketFromMenu(interaction, client) {
    ...
    await interaction.update({ content: null, embeds: [loadingEmbed], components: [] });

    const fetched = await interaction.channel.messages.fetch({ limit: 5 }).catch(() => null);
    const originalMessage = fetched ? fetched.find((m) => m.author.id === interaction.user.id) : null;
...
module.exports = [
    { id: 'mm_select_server', label: 'modmail-pilih-server', handler: openTicketFromMenu },
    { id: 'modmail_select_guild', label: 'modmail-pilih-guild', handler: openTicketFromMenu }
];

...
function buildBenefitsDescription() {
  const lines = [
    "Ini perbandingan lengkap tiap tier **Naura V.I.P Subscription**, biar kamu gampang memilih.\n",
  ];
  ...
  for (const tier of Object.values(PREMIUM_TIERS)) {
    ...
    lines.push(
      `${dot} **${tier.name}** - \`${tier.price}\` / ${tier.days} hari`,
    );
  ...
  lines.push(
    "-# Pakai `/premium info` untuk berlangganan, atau `/premium check` untuk melihat statusmu.",
  );
  return lines.join("\n");
}

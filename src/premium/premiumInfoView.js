...
function buildSelectRow(disabled) {
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .setPlaceholder(
        disabled
          ? "Menu sudah tidak aktif"
          : "Pilih paket langganan untuk melihat detailnya...",
      )
      .setDisabled(Boolean(disabled))
      .addOptions(
        Object.entries(PREMIUM_TIERS).map(([key, t]) => ({
...
function buildInfoPayload({
  ...
  return buildContainerV2({
    accentColorHex: isPremium
      ? ui.getPremiumColor(tierData.tier)
      : ui.getColor("primary"),
...
function buildConfirmPayload() {
  const owners =
    env.OWNER_IDS && env.OWNER_IDS.length
...
async function runInfo(interaction, profile) {
  ...
  const attachment = await buildTierCard(
    ...
    isPremium,
    daysLeft,
    store.expiryOf(profile),
    "premium-card.png",
  );

  const state = { isPremium, tierData, daysLeft, attachment, disabled: false };
  const files = attachment ? [attachment] : [];
  const messageObj = await interaction.editReply({
    ...buildInfoPayload(state),
    files,
  });
  ...
  collector.on("collect", async (i) => {
    ...
    if (fs.existsSync(QRIS_PATH)) {
      detailFiles.push(
        new AttachmentBuilder(QRIS_PATH, { name: "qris-naura.jpg" }),
      );
    ...
    if (i.user.id !== interaction.user.id) {
      return i.reply(
        ephemeral(
          buildErrorContainerV2({
            description:
              "Tombol ini punya orang yang memanggil perintahnya ya.",
            footerText: ui.getFooter("premium"),
          }),
        ),
      );

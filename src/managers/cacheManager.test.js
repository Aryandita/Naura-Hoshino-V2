...
test("CacheManager - Signature Normalization & Write-Behind Safety", async (t) => {
  ...
  await t.test("incrementUserSurvival normalizes string field and numeric value", async () => {
    ...
    assert.equal(queued.inc["0"], undefined, "Must not contain indexed key '0'");

    cacheManager.survivalQueue.delete(userId);
  });

  await t.test("incrementUserSurvival supports object delta format", async () => {
    cacheManager.survivalQueue.delete(userId);

    // Call with (userId, { starFragments: 100, coupons: 2 })
    await cacheManager.incrementUserSurvival(userId, { starFragments: 100, coupons: 2 });
  ...
  await t.test("incrementUserProfile normalizes string field and numeric value", async () => {
  ...
  await t.test("updateUserSurvival normalizes string field and value", async () => {
    cacheManager.survivalQueue.delete(userId);
    ...
    assert.equal(queued.set["0"], undefined, "Must not contain indexed key '0'");

    cacheManager.survivalQueue.delete(userId);
  });

  await t.test("updateUserProfile normalizes string field and value", async () => {
    cacheManager.writeQueue.delete(userId);
  ...
  await t.test("_flushOne ignores non-attribute keys and does not invoke SQL with column '0'", async () => {
    ...
    try {
      await cacheManager._flushOne(cacheManager.survivalQueue, UserSurvival, "UserSurvival", userId);
  ...
  await t.test("_flushOne handles UserProfile attributes correctly", async () => {
    ...
    try {
      await cacheManager._flushOne(cacheManager.writeQueue, UserProfile, "UserProfile", userId);

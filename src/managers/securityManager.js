...
class SecurityManager {
    ...
    static async handleAuditLog(guild, entry, type) {
        ...
        if (!executor || executor.id === guild.ownerId || executor.bot && executor.id === guild.client.user.id) return;
        ...
        try {
            logger.warn(`[ANTI-NUKE] Triggered on guild ${guild.name} (${guild.id}) by ${executor.tag} (${executor.id}) for action: ${type}`);
            ...
            if (targetMember && targetMember.manageable) {
                await targetMember.roles.set([], '🛡️ Naura Anti-Nuke: Stripping all permissions from attacker').catch(() => {});
            }

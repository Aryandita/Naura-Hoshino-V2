...
class GiveawayManager {
            ...
            for (const gw of activeGiveaways) {
                if (gw.endTime <= now) {
                    await this.endGiveaway(gw).catch((err) =>
                        logger.error(`[Giveaway] Gagal mengakhiri giveaway ${gw.messageId}: ${err.message}`)
                    );
                }
            }
        } catch (err) {
    ...
    async endGiveaway(gwData, manualEnd = false, excludeUserIds = []) {
        try {
            ...
            const allParticipants = Array.isArray(gwData.participants) ? gwData.participants : [];
            // Saring bot dan peserta yang dikecualikan (untuk re-roll)
            const eligible = allParticipants.filter((id) => !excludeUserIds.includes(id));
            ...
            await Promise.allSettled(
                batch.map(async (userId) => {
                    try {
                        ...
                        logger.warn(`[Giveaway] Gagal mengirim DM ke ${userId}: ${err.message}`);
                    }
                })
            );
        }
    ...
    static buildJoinButton(messageId, participantCount = 0) {
        const button = new ButtonBuilder()
            ...
            .setLabel(`🎉 Ikut Giveaway${participantCount > 0 ? ` (${participantCount})` : ""}`)

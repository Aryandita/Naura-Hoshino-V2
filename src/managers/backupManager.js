  ...
  try {
    ...
    if (hasMySQLConfig) {
      ...
      try {
        ...
        logger.success(
          `[BACKUP] MySQL Database successfully backed up to ${backupFile} (Pure JS Mode)`,
        );
      } catch (err) {
        logger.error(
          `[BACKUP] MySQL Backup Failed (Pure JS Mode):`,
          err.message,
        );
      }
    } else {
      ...
      if (fs.existsSync(sqliteSource)) {
        ...
        logger.success(
          `[BACKUP] SQLite Database successfully backed up to ${backupFile}`,
        );
      } else {
        logger.warn(
          `[BACKUP] SQLite database file not found at ${sqliteSource}`,
        );
      }
    }

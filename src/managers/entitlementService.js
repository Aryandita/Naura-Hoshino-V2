...
class EntitlementService {
    ...
    static async isUserPremium(userId) {
        ...
        if (profile.isPremium && profile.premiumUntil && new Date(profile.premiumUntil) > new Date()) {
            return true;
        }
        return false;
    }
    ...
    static async extendUserPremium(userId, durationMs) {
        ...
        await sequelize.transaction(async (t) => {
            ...
            const stillActive = profile.isPremium && profile.premiumUntil && new Date(profile.premiumUntil) > new Date();
            const base = stillActive ? new Date(profile.premiumUntil).getTime() : Date.now();
    ...
    static async isGuildPremium(guildId) {
        ...
        if (premiumData.active && premiumData.until && new Date(premiumData.until) > new Date()) {
            return true;
        }
        return false;
    }

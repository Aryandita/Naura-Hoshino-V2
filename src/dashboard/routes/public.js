...
module.exports = (client) => {
    ...
    const botAvatar = () =>
        client.user ? client.user.displayAvatarURL({ extension: 'png', size: 512 }) : '/assets/dashboard/naura.png';
    ...
    router.get('/api/economy_stats', async (req, res) => {
        try {
            ...
            return res.status(500).json({ error: 'Naura gagal memuat statistik ekonomi.' });
        }
    });
    ...
    router.get('/api/leaderboard', async (req, res) => {
        try {
            ...
            const limitNum = Math.min(Math.max(parseInt(req.query.limit, 10) || 20, 1), 50);
            ...
            const lbData = await Promise.all(
                topProfiles.map(async (u) => {
                    ...
                    const dUser = cachedUser || (await client.users.fetch(u.userId).catch(() => null));
                    if (dUser) {
                        name = dUser.username;

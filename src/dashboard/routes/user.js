...
const { requireApiLogin, requireSelfOrOwner, isOwner } = require('../middleware/auth');
...
module.exports = (client) => {
    ...
    router.get('/api/me', async (req, res) => {
        ...
        try {
            const [profile] = await UserProfile.findOrCreate({ where: { userId: req.user.id } });
            const UserSurvival = require('../../models/UserSurvival');
            const [survival] = await UserSurvival.findOrCreate({ where: { userId: req.user.id } });
            ...
            return res.json({ loggedIn: true, user: req.user, db: null, survival: null, isOwner: false });
        }
    });
    ...
    router.get('/api/me/language', requireApiLogin, async (req, res) => {
        try {
            ...
            res.json({ success: true, language: lang, supported: languageManager.SUPPORTED_LANGUAGES || ['id', 'en'] });
        } catch (e) {
            logger.error('[API LANGUAGE GET] Error:', e);
            res.status(500).json({ success: false, error: 'Naura gagal membaca pilihan bahasamu.' });
        }
    });
    ...
    router.post('/api/me/language', requireApiLogin, async (req, res) => {
        try {
            ...
            if (!supported.includes(lang)) {
                return res.status(400).json({ success: false, error: 'Bahasa itu belum Naura dukung ya.' });
            }

            await languageManager.setUserLanguage(req.user.id, lang);
            res.json({ success: true, language: lang, message: 'Bahasanya sudah Naura ganti!' });
        } catch (e) {
            logger.error('[API LANGUAGE SET] Error:', e);
            res.status(500).json({ success: false, error: 'Naura gagal menyimpan pilihan bahasamu.' });
        }
    });
    ...
    router.get('/api/profile', requireSelfOrOwner, async (req, res) => {
        try {
            ...
            const dUser = cachedUser || (await client.users.fetch(userId).catch(() => null));

            const relationshipNames = ['Kenalan', 'Teman', 'Sahabat', 'Pacar', 'Menikah'];
            ...
            res.json({
                ...
                npcBonds: bonds
                    .map((b) => ({
                        ...
                        relationshipName: relationshipNames[b.relationshipLevel || 0] || 'Kenalan',
            ...
            res.status(500).json({ success: false, error: 'Naura gagal memuat profilmu.' });
        }
    });
    ...
    router.post('/api/inventory/action', requireSelfOrOwner, async (req, res) => {
        try {
            ...
            if (!itemId || itemIdx === undefined || !action) {
                return res.status(400).json({ success: false, error: 'Parameternya belum lengkap ya.' });
            }

            const GameItem = require('../../models/GameItem');
            const profile = await UserProfile.findByPk(req.targetUserId);
            if (!profile) return res.status(404).json({ success: false, error: 'Profil pemain tidak ditemukan.' });

            const inv = profile.inventory || [];
            const idx = Number(itemIdx);
            if (!Number.isInteger(idx) || idx < 0 || idx >= inv.length) {
                return res.status(400).json({ success: false, error: 'Slot item itu tidak valid.' });
            }

            const item = inv[idx];
            if (itemIdOf(item) !== itemId) {
                return res.status(400).json({ success: false, error: 'Itemnya tidak cocok dengan slot itu.' });
            }
            ...
            if (action === 'sell') {
                ...
                return res.status(400).json({ success: false, error: 'Aksi itu tidak dikenali.' });
            }
            ...
            res.status(500).json({ success: false, error: 'Naura gagal memproses isi tasmu.' });
        }
    });
    ...
    router.post('/api/inventory/forge', requireSelfOrOwner, async (req, res) => {
        try {
            ...
            if (!itemId) return res.status(400).json({ success: false, error: 'Item yang mau dilebur belum dipilih.' });
            ...
            const totalAmount = inv.reduce(
                (sum, entry) => (itemIdOf(entry) === itemId ? sum + (entry.amount || 1) : sum),
                0
            );
            ...
            if (totalAmount < 2) {
                return res
                    .status(400)
                    .json({ success: false, error: 'Naura butuh minimal 2 item yang sama untuk melebur ya.' });
            }
            ...
            if (isSuccess) {
                ...
                if (newId) {
                    ...
                    forgedItemName = levelMatch
                        ? name.replace(/\(Lv\.\s*\d+\)/i, `(Lv. ${parseInt(levelMatch[1], 10) + 1})`)
                        : `${name} (Lv. 2)`;
                ...
                message = 'Yah, peleburannya gagal. Satu bahannya hancur. Jangan menyerah ya!';
            }
            ...
            res.status(500).json({ success: false, error: 'Naura gagal melebur itemmu.' });
        }
    });

...
const EVAL_ENABLED = String(process.env.OWNER_EVAL_ENABLED || '').toLowerCase() === 'true';

function broadcast(client, message) {
    if (client.dashboardIo) client.dashboardIo.emit('system_broadcast', { message });
}
...
module.exports = (client) => {
    ...
    router.post('/api/owner/update_user', async (req, res) => {
        ...
        if (!targetId) return res.status(400).json({ error: 'Target User ID wajib diisi.' });
    ...
    router.post('/api/owner/eval', async (req, res) => {
        if (!EVAL_ENABLED) {
            return res.status(403).json({
                success: false,
                error: 'Konsol eval dimatikan. Setel OWNER_EVAL_ENABLED=true di .env kalau memang mau dipakai.'
            });
        }
    ...
    router.post('/api/owner/maintenance', (req, res) => {
        ...
        try {
            if (status) {
                client.user.setPresence({ activities: [{ name: 'Naura lagi dirapikan sebentar' }], status: 'dnd' });
            } else {
                client.user.setPresence({ activities: [{ name: 'Menemani kamu hari ini' }], status: 'online' });
            }
        } catch (e) {
            logger.warn('[OWNER MAINTENANCE] Gagal mengubah presence:', e.message);
        }

        res.json({ success: true, message: `Mode perbaikan sekarang ${status ? 'AKTIF' : 'MATI'}.` });
        broadcast(client, `Mode perbaikan ${status ? 'diaktifkan' : 'dinonaktifkan'}.`);
    });

    // --- Muat ulang satu command ---
    router.post('/api/owner/reload', (req, res) => {
        const commandName = req.body?.command;
        if (!commandName) return res.status(400).json({ error: 'Nama command wajib diisi.' });
        ...
        try {
            ...
            if (!fileLocation) {
                return res.status(404).json({ error: `Berkas untuk ${commandName} tidak bisa dilacak.` });
            }
            ...
            res.json({ success: true, message: `Command ${commandName} sudah dimuat ulang.` });
        } catch (error) {
    ...
    router.post('/api/owner/profile', async (req, res) => {
        ...
        if (!actions[type]) return res.status(400).json({ error: 'Jenis profil tidak dikenali.' });
    ...
    router.post('/api/owner/presence', (req, res) => {
        ...
        if (!type || !text) return res.status(400).json({ error: 'Jenis dan teks wajib diisi.' });

        try {
            const { ActivityType } = require('discord.js');
            if (!(type in ActivityType)) return res.status(400).json({ error: 'Jenis aktivitas tidak dikenali.' });
            client.user.setActivity(text, { type: ActivityType[type] });
            res.json({ success: true, message: `Presence diubah jadi: ${type} ${text}` });
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    });
    ...
    router.post('/api/owner/aivoice', async (req, res) => {
        ...
        if (!guildId) return res.status(400).json({ error: 'Guild ID wajib diisi.' });

        try {
            const [guildData] = await GuildSettings.findOrCreate({ where: { guildId } });
            guildData.aiVoiceEnabled = !!status;
            await guildData.save();
            res.json({ success: true, message: `AI Voice server ${guildId} kini ${status ? 'AKTIF' : 'MATI'}.` });
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    });
    ...
    router.post('/api/owner/eco_reset', async (req, res) => {
        ...
        try {
            ...
            res.json({ success: true, message: `Data finansial, XP, dan inventory ${targetId} sudah dihapus.` });
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    });
    ...
    router.post('/api/owner/reset_bday', async (req, res) => {
        ...
        try {
            ...
            await profile.save();
            res.json({ success: true, message: `Data ulang tahun ${targetId} sudah direset.` });
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    });
    ...
    router.post('/api/owner/guild_setup', async (req, res) => {
        ...
        if (!guildId || !type || !data) return res.status(400).json({ error: 'Parameternya belum lengkap.' });
        ...
        try {
            ...
            if (type === 'minecraft') {
                settings.minecraft = { ...(settings.minecraft || {}), ip: data.ip, port: parseInt(data.port, 10) || 25565 };
            } else if (type === 'sticky') {
                ...
                const existing = settings.autoReplies.findIndex((r) => r.trigger === trigger);
                if (existing !== -1) {
            ...
            await guildData.save();
            res.json({ success: true, message: `Setup [${type}] tersimpan untuk server ${guildId}.` });
        } catch (error) {
            res.status(500).json({ error: error.message });
        }
    });
    ...
    router.post('/api/owner/restart', (req, res) => {
        res.json({ success: true, message: 'Sistem dimatikan dan akan dinyalakan ulang...' });

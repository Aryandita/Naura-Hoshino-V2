...
const { verifyToken, createRateLimiter, claimOnce, fingerprint } = require('../utils/httpGuard');
...
function passesToken(req, res, { tag, envKey, headerNames }) {
    ...
    if (state === 'not_configured') {
        logger.warn(`[WEBHOOK ${tag}] ${envKey} belum diisi. Endpoint ditutup demi keamanan.`);
...
module.exports = (client) => {
    ...
    app.use('/api/webhook', createRateLimiter({ windowMs: 60_000, max: 30, name: 'WEBHOOK' }));
    ...
    app.post('/api/webhook/vote', async (req, res) => {
        if (!passesToken(req, res, { tag: 'VOTE', envKey: 'WEBHOOK_AUTH_VOTE', headerNames: ['authorization'] })) {
            return undefined;
        }
    ...
    function donationHandler({ tag, envKey, headerNames, readAmount, readMessage, readName, readId, label }) {
        return async (req, res) => {
            if (!passesToken(req, res, { tag, envKey, headerNames })) return undefined;
            ...
            if (!(await claimOnce(claimKey, ttl))) {
                logger.info(`[WEBHOOK ${tag}] Kiriman ulang untuk ${userId} diabaikan.`);

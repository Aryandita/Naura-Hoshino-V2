...
module.exports = (client) => {
    ...
    router.post('/api/settings', requireGuildManager, async (req, res) => {
        try {
            const { prefix, automod, twentyFourSeven, verbaSlug1, verbaSlug2, customPersona, serverKnowledge } = req.body;
            ...
            if (twentyFourSeven !== undefined) settings.twentyFourSeven = toBool(twentyFourSeven);
            ...
            if (customPersona !== undefined) settings.ai.customPersona = String(customPersona).slice(0, 500);
            if (serverKnowledge !== undefined) settings.ai.serverKnowledge = String(serverKnowledge).slice(0, 1000);
            ...
            res.status(500).json({ success: false, message: 'Naura gagal menyimpan pengaturan.' });
        }
    });
    ...
    router.get('/api/welcomer', requireGuildManager, async (req, res) => {
        try {
            ...
            res.status(500).json({ error: 'Naura gagal memuat pengaturan welcomer.' });
        }
    });
    ...
    router.post('/api/welcomer', requireGuildManager, async (req, res) => {
        try {
            ...
            for (const key of NUMERIC_WELCOME_KEYS) {
                const raw = body[key];
                welcome[key] = raw !== undefined && raw !== null && raw !== '' ? Number(raw) : WELCOME_DEFAULTS[key];
            ...
            res.json({ success: true, message: 'Tampilan sambutannya sudah Naura simpan!' });
        } catch (e) {
            logger.error('[API WELCOMER SAVE] Error:', e);
            res.status(500).json({ success: false, message: 'Naura gagal menyimpan pengaturan welcomer.' });
        }
    });
    ...
    router.post('/api/minecraft/chat', requireGuildManager, async (req, res) => {
        try {
            ...
            const settings = await GuildSettings.findOne({ where: { guildId: req.guildId } });
            const mc = settings?.settings?.minecraft;
            if (!mc) return res.status(404).json({ error: 'Pengaturan Minecraft belum ada untuk server ini.' });
            if (!mc.bridgeEnabled || !mc.bridgeChannelId) {
                return res.status(400).json({ error: 'Bridge belum diaktifkan atau kanalnya belum diatur.' });
            }
            ...
            res.status(500).json({ error: 'Naura gagal meneruskan pesan ke Discord.' });
        }
    });

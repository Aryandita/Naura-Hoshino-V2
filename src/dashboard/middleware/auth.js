...
function getOwnerIds() {
    const fromConfig = Array.isArray(env.OWNER_IDS) ? env.OWNER_IDS.map(String) : [];
...
function requireApiLogin(req, res, next) {
    if (isLoggedIn(req)) return next();
    return res.status(401).json({ success: false, error: 'Kamu belum login ya. Masuk dulu lewat Discord.' });
}
...
function requireOwner(req, res, next) {
    ...
    if (!isOwner(req.user.id)) {
        return res.status(403).json({ success: false, error: 'Area ini khusus Owner. Maaf ya!' });
    }
    return next();
}
...
function requireGuildManager(req, res, next) {
    ...
    if (!guildId) {
        return res.status(400).json({ success: false, error: 'Guild ID belum dikirim.' });
    }

    if (!canManageGuild(req.user, guildId)) {
        return res.status(403).json({ success: false, error: 'Kamu tidak punya izin Kelola Server di server itu.' });
    }
...
function requireSelfOrOwner(req, res, next) {
    ...
    if (String(targetId) !== String(req.user.id) && !isOwner(req.user.id)) {
        return res.status(403).json({ success: false, error: 'Kamu hanya boleh mengubah datamu sendiri.' });
    }

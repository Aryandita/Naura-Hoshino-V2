    ...
    io.on('connection', (socket) => {
        ...
        socket.on('music_control', async (data) => {
            ...
            if (!(await canControlGuild(client, socket, guildId))) {
                return socket.emit('music_error', { message: 'Kamu belum tergabung di server itu ya.' });
            }
            ...
            io.to(String(guildId)).emit('music_state', snapshotPlayer(client, player));
        });

        socket.on('chat_message', async (data) => {
            const limited = await RateLimiter.isRateLimited(socket.id, 'dashboard_chat', 5, 10);
            if (limited) {
                return socket.emit('chat_response', {
                    reply: 'Pelan-pelan ya, Naura masih mengetik. Tunggu beberapa detik lagi.'
                });
            }
            ...
            try {
                ...
                if (!response.ok) throw new Error(`Verba API error: HTTP ${response.status}`);
                ...
                logger.warn(`[SOCKET CHAT] Verba gagal (${error.message}). Beralih ke cadangan...`);
            }
            ...
            try {
                ...
                return socket.emit('chat_response', {
                    reply: 'Maaf ya, otak Naura lagi ngambek sebentar. Coba lagi nanti, oke?'
                });
            }
        });

...
const { textDisplay, separatorComp } = require('../../src/utils/NauraContainerBuilder');
    ...
    static async renderPanel(manager, player, track, recommendedTracks = []) {
        try {
            ...
            if (!channel && player.textChannel) {
                channel = await manager.client.channels.fetch(player.textChannel).catch(() => null);
            }
            if (!channel) {
                logger.error(`[MusicUI Error] Channel ${player.textChannel} tidak dapat ditemukan.`);
                return;
            }
            ...
            const generatePayload = async (currentPos, isUpdate = false) => {
                ...
                containerComponents.push(textDisplay(`-# ✦  N A U R A  M U S I C  P A N E L  ✦`));
                ...
                const rowFilter = new ActionRowBuilder().addComponents(
                    new StringSelectMenuBuilder()
                        ...
                        .addOptions([
                            ...
                            { label: 'Nightcore Shift', description: 'Peningkatan tempo', value: 'nightcore', emoji: '⚡' },
                            { label: 'Vaporwave Reverb', description: 'Gema ruang', value: 'vaporwave', emoji: '🌊' },
                            { label: '8D Surround', description: 'Audio 360', value: '8d', emoji: '🎧' },
                            { label: 'Pop / Tremolo', description: 'Efek getaran suara', value: 'pop', emoji: '🎸' },
                            { label: 'Karaoke Mode', description: 'Kurangi vokal', value: 'karaoke', emoji: '🎤' }
                        ])
                );
                ...
                const rowMedia = new ActionRowBuilder().addComponents(
                    new ButtonBuilder()
                        ...
                        .setEmoji(ui.parseEmoji(ui.getEmoji('musicPlayPause')) || { name: '⏯️' })
                    ...
                    new ButtonBuilder()
                        ...
                        .setEmoji(ui.parseEmoji(ui.getEmoji('musicAutoplay')) || { name: '🤖' })
                        .setStyle(player.isAutoplayMode ? ButtonStyle.Primary : ButtonStyle.Secondary)
                );

                const rowUtils = new ActionRowBuilder().addComponents(
                    new ButtonBuilder()
                        .setCustomId('music_voldown')
                        .setEmoji(ui.parseEmoji(ui.getEmoji('musicVolDown')) || { name: '🔉' })
                        .setStyle(ButtonStyle.Secondary),
                    new ButtonBuilder()
                        .setCustomId('music_volup')
                        .setEmoji(ui.parseEmoji(ui.getEmoji('musicVolUp')) || { name: '🔊' })
                        .setStyle(ButtonStyle.Secondary),
                    new ButtonBuilder()
                        .setCustomId('music_lyrics')
                        .setEmoji(ui.parseEmoji(ui.getEmoji('musicLyrics')) || { name: '🎤' })
                        .setLabel('Lirik')
                        .setStyle(ButtonStyle.Secondary),
                    new ButtonBuilder()
                ...
                containerComponents.push(separatorComp(true, 1));
                // --- Footer ---
                containerComponents.push(textDisplay(`-# ${ui.stripCustomEmojis(ui.getFooter('music'))}`));

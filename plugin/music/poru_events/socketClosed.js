...
module.exports = {
    ...
    async execute(manager, player, payload) {
        ...
        console.warn(`\x1b[43m\x1b[30m ⚠️ VOICE DISCONNECT \x1b[0m Bot terputus dari Voice (Code: ${payload.code}). Memeriksa mode 24/7...`);
        ...
        if (player.socketRetryCount > 5) {
            console.error(`\x1b[41m\x1b[37m ⚠️ VOICE RECONNECT FAILED \x1b[0m Gagal menghubungkan 24/7 setelah 5 percobaan. Menghentikan player.`);
        ...
        const backoffMs = Math.min(30000, 3000 * Math.pow(2, player.socketRetryCount - 1));
            ...
            if (guildData && guildData.music && guildData.music.twentyFourSeven) {
                ...
                setTimeout(() => {
                    ...
                    if (guild && guild.channels.cache.has(voiceChannelId)) {
                        console.log(`\x1b[42m\x1b[30m ♻️ AUTO RECONNECT \x1b[0m Percobaan #${player.socketRetryCount}: Mengembalikan Naura ke Voice Channel (24/7 Mode)...`);

...
const BLACKLIST_KEYWORDS = [
    'karaoke', 'instrumental', 'cover', 'live performance', 'reaction',
    'nightcore', 'slowed + reverb', 'slowed and reverb', '8d audio',
    'tutorial', 'how to play', 'sped up version'
];
...
function isDurationSane(candidateLen, sourceLen) {
    ...
    if (candidateLen > 15 * 60 * 1000 && candidateLen > sourceLen * 3) return false;
 ...
 */
function rankAutoplayCandidates(tracks = [], sourceTrack, playedHistory = new Set(), limit = 3) {
    if (!Array.isArray(tracks) || tracks.length === 0) return [];
    const sourceInfo = sourceTrack?.info || {};
    const sourceAuthor = (sourceInfo.author || '').toLowerCase();

    const scored = tracks
        .filter(t => t?.info?.identifier && t.info.identifier !== sourceInfo.identifier)
        ...
        .map(t => {
            ...
            const sameArtist = sourceAuthor && (t.info.author || '').toLowerCase() === sourceAuthor;
 ...
 */
function smoothFade(player, fromVolume, toVolume, durationMs = 3000, steps = 15) {
    ...
    try { player.setVolume(Math.round(Math.max(0, Math.min(100, current)))); } catch (e) {}
    ...
    player._fadeInterval = setInterval(() => {
        ...
        if (count >= safeSteps) {
            clearInterval(player._fadeInterval);
            player._fadeInterval = null;
            try { player.setVolume(Math.round(Math.max(0, Math.min(100, toVolume)))); } catch (e) {}
        }
    }, stepTime);
}
...
function startFadeOutWatcher(player, track, baseVolume, fadeOutMs = 6000) {
    ...
    player._endWatcher = setInterval(() => {
        const stillSameTrack = player.currentTrack && player.currentTrack.info.identifier === track.info.identifier;
        ...
        if (!fading && remaining > 0 && remaining <= fadeOutMs) {
            ...
            smoothFade(player, player.volume ?? baseVolume, floorVolume, Math.min(fadeOutMs, remaining));
        }
    }, 1000);
}
 ...
 */
function beginPlaybackTransition(player, track, baseVolume, { fadeInMs = 3000, fadeOutMs = 6000 } = {}) {
    if (!player || typeof player.setVolume !== 'function') return;
...
function clearTransitionTimers(player) {
    if (!player) return;
    if (player._fadeInterval) { clearInterval(player._fadeInterval); player._fadeInterval = null; }
    if (player._endWatcher) { clearInterval(player._endWatcher); player._endWatcher = null; }
}

    ...
    async execute(manager, player, track) {
        try {
            ...
            if (typeof player.baseVolume !== 'number') {
                player.baseVolume = typeof player.volume === 'number' ? player.volume : 65;
            }
            ...
            try {
                ...
                if (guildData && guildData.aiVoiceEnabled && activeTrack.info.requester && !isAutoplayRequest) {
                    isDjActive = true;
                    player.pause(true);
                    ...
                    try {
                        ...
                        const aiResultText = await Promise.race([
                            ...
                            new Promise(resolve => setTimeout(() => resolve(null), AI_DJ_TIMEOUT_MS))
                        ]);

                        if (aiResultText) {
                            djText = aiResultText.replace(/[^\w\s.,?!'a-zA-Z0-9áéíóúÁÉÍÓÚñÑüÜ-]/g, '').trim();
                        } else {
                            console.warn(`\x1b[43m\x1b[30m 🎤 AI DJ \x1b[0m \x1b[33mGemini timeout, menggunakan teks default.\x1b[0m`);
                        }
                    } catch (e) {
                        console.warn(`\x1b[43m\x1b[30m 🎤 AI DJ \x1b[0m \x1b[33mGemini gagal (${e.message}), menggunakan teks default.\x1b[0m`);
                    }
                    ...
                    setTimeout(() => {
                        try {
                            ...
                            logger.error('[DJ ERROR] Gagal mengembalikan volume setelah DJ.', e);
                        }
                    }, DJ_PAUSE_MS);
            ...
            try {
                recommendedTracks = await this.handleAutoplayPrefetch(manager, player, activeTrack);
            } catch (prefetchErr) {
                logger.error('[AutoplayPrefetch Error]', prefetchErr);
            }

            try {
                await MusicUIManager.renderPanel(manager, player, activeTrack, recommendedTracks);
            } catch (uiErr) {
                logger.error('[MusicUI Render Error]', uiErr);
            }

        } catch (error) { logger.error('\x1b[41m\x1b[37m ⚠️ EVENT ERROR \x1b[0m', error); }
    },
    ...
    async handleAutoplayPrefetch(manager, player, activeTrack) {
        ...
        if (player.isAutoplayMode) {
            try {
                ...
                const searchRes = await manager.poru.resolve({ query: searchQuery, requester: manager.client.user });
                if (searchRes && searchRes.tracks && searchRes.tracks.length > 0) {
                    ...
                    const ranked = rankAutoplayCandidates(searchRes.tracks, activeTrack, player.playedHistory, 3);
                    ...
                    if (ranked.length > 0) {
                        ...
                        console.log(`\x1b[42m\x1b[30m 💿 AUTOPLAY NATIVE \x1b[0m \x1b[32mPre-fetch lagu rekomendasi: ${best.info.title}\x1b[0m`);
                    }
                }
            } catch (e) {
                console.error(`\x1b[41m\x1b[37m ⚠️ AUTOPLAY NATIVE ERROR \x1b[0m Alasan: \x1b[33m${e.message || 'Tidak diketahui'}\x1b[0m. Menggunakan fallback...`);
            }
        }

        // Fallback ke Gemini AI jika native Mix gagal mendapatkan lagu baru
        if (player.isAutoplayMode && !player.prefetchedAutoplayTrack && process.env.GEMINI_API_KEY) {
            try {
                const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
                ...
                const searchRes = await manager.poru.resolve({ query: `ytsearch:${aiQuery}`, requester: manager.client.user });
                if (searchRes && searchRes.tracks && searchRes.tracks.length > 0) {
                    const ranked = rankAutoplayCandidates(searchRes.tracks, activeTrack, player.playedHistory, 3);
                    const freshTrack = ranked[0] || searchRes.tracks.find(t => t.info.identifier !== activeTrack.info.identifier);
                ...
                console.error(`\x1b[41m\x1b[37m ⚠️ GEMINI FALLBACK ERROR \x1b[0m Alasan: \x1b[33m${e.message || 'Tidak diketahui'}\x1b[0m.`);
            }
        }
        ...
        try {
            ...
            if (dropdownRes && dropdownRes.tracks) {
                recommendedTracks = rankAutoplayCandidates(dropdownRes.tracks, activeTrack, player.playedHistory, 5);
        ...
        if (player.isAutoplayMode && !player.prefetchedAutoplayTrack) {
            if (recommendedTracks.length > 0) {
                ...
                if (recommendedTracks.length > 1 && (!player.autoplayQueue || player.autoplayQueue.length === 0)) {
            ...
            if (!player.prefetchedAutoplayTrack) {
                try {
                    ...
                    if (extraFallback && extraFallback.tracks && extraFallback.tracks.length > 0) {
                        const ranked = rankAutoplayCandidates(extraFallback.tracks, activeTrack, player.playedHistory, 3);
                        const newTrack = ranked[0] || extraFallback.tracks.find(t => t.info.identifier !== activeTrack.info.identifier);
                        if (newTrack) {

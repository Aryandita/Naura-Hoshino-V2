...
const { rankAutoplayCandidates, clearTransitionTimers } = require('./autoplayUtils');
...
module.exports = {
    async execute(manager, player) {
        ...
        if (player.isAutoplayMode) {
            ...
            if (!trackToPlay && Array.isArray(player.autoplayQueue) && player.autoplayQueue.length > 0) {
                while (player.autoplayQueue.length > 0 && !trackToPlay) {
                    const candidate = player.autoplayQueue.shift();
                    if (candidate && !player.playedHistory?.has(candidate.info.identifier)) {
                        trackToPlay = candidate;
                    }
                }
                if (trackToPlay) {
                    console.log(`\x1b[42m\x1b[30m 💿 AUTOPLAY BUFFER \x1b[0m \x1b[32mMemakai cadangan dari autoplayQueue: ${trackToPlay.info.title}\x1b[0m`);
                }
            }
            ...
            if (!trackToPlay && player.previousTrack) {
                console.log(`\x1b[45m\x1b[37m 💿 AUTOPLAY \x1b[0m \x1b[35mMencari lagu rekomendasi native YouTube Mix...\x1b[0m`);
                try {
                    ...
                    const searchRes = await manager.poru.resolve({ query: mixUrl, requester: manager.client.user });
                    if (searchRes && searchRes.tracks && searchRes.tracks.length > 0) {
                        const ranked = rankAutoplayCandidates(searchRes.tracks, player.previousTrack, player.playedHistory || new Set(), 3);
                        ...
                        if (freshTrack) {
                            ...
                            console.log(`\x1b[42m\x1b[30m 💿 AUTOPLAY NATIVE \x1b[0m \x1b[32mBerhasil memuat lagu rekomendasi YouTube Mix: ${freshTrack.info.title}\x1b[0m`);
                        }
                    }
                } catch (e) {
                    console.error(`\x1b[41m\x1b[37m ⚠️ AUTOPLAY NATIVE ERROR \x1b[0m Gagal fetch: ${e.message}`);
                }
            }
            ...
            if (!trackToPlay && geminiKey && player.previousTrack) {
                console.log(`\x1b[45m\x1b[37m 💿 AUTOPLAY \x1b[0m \x1b[35mMencari lagu rekomendasi AI Fallback...\x1b[0m`);
                try {
...
Berikan 1 rekomendasi lagu selanjutnya yang populer dan memiliki vibe/genre yang sama, tidak harus dari artis yang sama. Balas HANYA dengan format murni: "Judul Lagu - Nama Artis". TANPA KUTIP, TANPA SIMBOL.`;
                    ...
                    if (searchRes && searchRes.tracks && searchRes.tracks.length > 0) {
                        const ranked = rankAutoplayCandidates(searchRes.tracks, player.previousTrack, player.playedHistory || new Set(), 3);
                        const freshTrack = ranked[0] || searchRes.tracks.find(t => t.info.identifier !== player.previousTrack?.info?.identifier);
                    ...
                    logger.error(`\x1b[41m\x1b[37m ⚠️ GEMINI FALLBACK ERROR \x1b[0m Gagal fetch fallback autoplay: ${e.message}`);
                }
            }
            ...
            if (!trackToPlay && player.previousTrack) {
                try {
                    const searchRes = await manager.poru.resolve({ query: `ytsearch:${player.previousTrack.info.title} ${player.previousTrack.info.author} mix`, requester: manager.client.user });
                    if (searchRes && searchRes.tracks && searchRes.tracks.length > 0) {
        ...
        if (oldCache && oldCache.messageId) {
            const channel = manager.client.channels.cache.get(player.textChannel);
            if (channel) {
                channel.messages.fetch(oldCache.messageId)
                    .then(m => m.delete().catch(() => {}))
                    .catch(() => {});
            }
        ...
        if (channel) {
            const exitPayload = buildContainerV2({
                ...
                description: 'Antrean lagu telah habis. Naura pamit dari Voice Channel!',
                footerText: ui.getFooter('music')
            });
            channel.send(exitPayload).then(m => setTimeout(() => m.delete().catch(()=>{}), 15000)).catch(()=>{});
        }
    }
};
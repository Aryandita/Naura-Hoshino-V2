...
const getSourceIcon = (sourceName) => {
    ...
    if (sourceName === 'youtube' || sourceName === 'ytmsearch') return ui.getEmoji('youtube') || '▶️';
...
async function runMusicLogic(client, user, member, guild, channel, subcommand, args, sendReply, isSlash) {
    ...
    if (subcommand === 'profile') {
        ...
        if (target.bot) {
            ...
            const attachment = new AttachmentBuilder(imageBuffer, { name: 'naura-audiophile.webp' });
            ...
            const errPayload = buildErrorContainerV2({ title: 'Gagal Render Profil', description: `${eError} | Gagal merender kartu profil dari server.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
    }

    if (!memberVoice) {
        const errPayload = buildErrorContainerV2({ title: 'Koneksi Gagal', description: `${eError} | Anda harus berada di dalam Voice Channel terlebih dahulu!`, footerText: ui.getFooter('music') });
        return sendReply(errPayload, true);
    }

    let player = poru.players.get(guild.id);
    if (player && player.voiceChannel !== memberVoice.id) {
        const errPayload = buildErrorContainerV2({ title: 'Voice Channel Berbeda', description: `${eError} | Naura sedang aktif di Voice Channel lain.`, footerText: ui.getFooter('music') });
        return sendReply(errPayload, true);
    }
    ...
    if (subcommand === 'lofi' || subcommand === 'radio') {
        ...
        const scanPayload = buildContainerV2({ accentColorHex: stColor, title: 'Radio Stasiun', description: `⏳ Memindai frekuensi stasiun **${subcommand.toUpperCase()}**...`, footerText: ui.getFooter('music') });
        ...
        if (res && res.tracks && res.tracks.length > 0) {
            ...
            const connPayload = buildContainerV2({ accentColorHex: stColor, title: `${stIcon} Stasiun Terhubung`, description: `Mode transmisi 24/7 otomatis aktif.\n${divider}`, footerText: ui.getFooter('music') });
            return sendReply(connPayload);
        }
        const errPayload = buildErrorContainerV2({ title: 'Gagal Kunci Frekuensi', description: `${eError} | Gagal mengunci frekuensi stasiun.`, footerText: ui.getFooter('music') });
        return sendReply(errPayload, true);
    }
    ...
    if (subcommand === 'play') {
        ...
        if (!query) {
            const errPayload = buildErrorContainerV2({ title: 'Query Kosong', description: `${eError} | Harap masukkan judul lagu atau URL yang ingin diputar.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        ...
        const spotifyValidation = SpotifyHelper.validateSpotifyUrl(query);
        ...
        const spotifyBrandEmoji = ui.getEmoji('spotify') || '🎵';
        const searchingPayload = buildContainerV2({ accentColorHex: ui.getColor('primary') || '#FFB6C1', title: 'Mencari Audio', description: `🔍 Menganalisis gelombang suara untuk: **${query}**...`, footerText: ui.getFooter('music') });
        await sendReply(searchingPayload, false);

        if (isSpotifyLink) {
            const spotifyData = await SpotifyHelper.resolveSpotify(query, 'ytm');
            if (spotifyData && spotifyData.queries && spotifyData.queries.length > 0) {
                if (spotifyData.type === 'track') {
                    res = await poru.resolve({ query: spotifyData.queries[0], requester: user });
                } else {
                    const firstQuery = spotifyData.queries[0];
                    res = await poru.resolve({ query: firstQuery, requester: user });
                    
                    if (res && res.tracks && res.tracks.length > 0) {
                        const firstTrack = res.tracks[0];
                        firstTrack.info.requester = user;
                        firstTrack.info.originalSource = 'spotify';
                        player.queue.add(firstTrack);
                        if (!player.isPlaying && !player.isPaused) player.play();

                        const spPayload = buildContainerV2({
                            accentColorHex: '#1DB954',
                            title: `${spotifyBrandEmoji} Playlist/Album Spotify Dimuat`,
                            description: `${ui.getEmoji('musicArtist') || '👤'} **Total:** \`${spotifyData.queries.length} Lagu\`\n⏳ Memasukkan lagu ke antrean...`,
                            iconURL: firstTrack.info.image || client.user.displayAvatarURL(),
                            footerText: ui.getFooter('music')
                        });
                        sendReply(spPayload, true);

                        (async () => {
                            for (let i = 1; i < spotifyData.queries.length; i++) {
                                try {
                                    const r = await poru.resolve({ query: spotifyData.queries[i], requester: user });
                                    if (r && r.tracks && r.tracks.length > 0) {
                                        const t = r.tracks[0];
                                        t.info.requester = user;
                                        t.info.originalSource = 'spotify';
                                        player.queue.add(t);
                                    }
                                } catch (err) {
                                    console.error(`[Spotify background load error for track ${i}]`, err);
                                }
                            }
                        })();
                        return;
        ...
        if (!res || !res.tracks || res.tracks.length === 0 || res.loadType === 'empty' || res.loadType === 'NO_MATCHES') {
            const errPayload = buildErrorContainerV2({ title: 'Audio Tidak Ditemukan', description: `${eError} | Frekuensi audio tidak ditemukan.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        ...
        if (query.includes('spotify.com') || searchSource === 'spsearch') { brandColor = '#1DB954'; brandEmoji = ui.getEmoji('spotify') || '🎵'; }
        else if (searchSource === 'scsearch') { brandColor = '#FF7700'; brandEmoji = ui.getEmoji('soundcloud') || '☁️'; }
    ...
    if (subcommand === 'import') {
        ...
        if (!url || !url.match(/^https?:\/\//)) {
            const errPayload = buildErrorContainerV2({ title: 'URL Tidak Valid', description: `${eError} | Harap masukkan URL Playlist yang valid.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        ...
        const userPlaylistsCount = await UserPlaylist.count({ where: { userId: user.id } });
        if (!isPremium && userPlaylistsCount >= 3) {
            const errPayload = buildErrorContainerV2({ title: 'Batas Playlist Tercapai', description: `${eError} | Pengguna standar hanya dapat menyimpan maksimal **3 Playlist**.\nGunakan \`/premium\` untuk mendapatkan akses simpan tanpa batas!`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }

        const importingPayload = buildContainerV2({ accentColorHex: '#1DB954', title: 'Mengimpor Playlist', description: `⏳ Sedang mengimpor playlist ke database Naura...`, footerText: ui.getFooter('music') });
        ...
        try {
            ...
            if (url.match(/^(https?:\/\/)?(open\.)?spotify\.com\/(playlist|album)\/[a-zA-Z0-9]+/)) {
                const spotifyHelper = require('./spotifyHelper');
                const spotifyData = await spotifyHelper.resolveSpotify(url, 'yt');
                if (spotifyData && spotifyData.queries && spotifyData.queries.length > 0) {
            ...
            if (tracksToSave.length === 0) {
                const errPayload = buildErrorContainerV2({ title: 'Gagal Impor', description: `${eError} | Gagal mengimpor data.`, footerText: ui.getFooter('music') });
                return sendReply(errPayload, true);
            }
            ...
            const errPayload = buildErrorContainerV2({ title: 'Gagal Proses', description: `${eError} | Terjadi kesalahan saat memproses URL.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
    }
    ...
    if (subcommand === 'share') {
        const pid = args.id;
        if (!pid) {
            const errPayload = buildErrorContainerV2({ title: 'ID Diperlukan', description: `${eError} | Harap masukkan ID Playlist untuk dibagikan.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }

        const playlist = await UserPlaylist.findOne({ where: { id: pid, userId: user.id } });
        if (!playlist) {
            const errPayload = buildErrorContainerV2({ title: 'Tidak Ditemukan', description: `${eError} | Playlist tidak ditemukan.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
    ...
    if (subcommand === 'load') {
        ...
        if (!code || !code.startsWith('NRA') || !code.endsWith('URA')) {
            const errPayload = buildErrorContainerV2({ title: 'Kode Salah', description: `${eError} | Kode playlist tidak valid.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }

        try {
            const decodedId = Buffer.from(code.substring(3, code.length - 3), 'base64').toString('ascii');
            const sourcePlaylist = await UserPlaylist.findOne({ where: { id: parseInt(decodedId) } });

            if (!sourcePlaylist) {
                const errPayload = buildErrorContainerV2({ title: 'Tidak Ditemukan', description: `${eError} | Playlist asal tidak ditemukan atau sudah dihapus.`, footerText: ui.getFooter('music') });
                return sendReply(errPayload, true);
            }

            const newName = `${sourcePlaylist.name} (Copy)`;
            await UserPlaylist.create({ userId: user.id, name: newName, tracks: sourcePlaylist.tracks, spotifyUrl: sourcePlaylist.spotifyUrl });
            ...
            const errPayload = buildErrorContainerV2({ title: 'Gagal Menyalin', description: `${eError} | Gagal menyalin playlist.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
    }
    ...
    if (subcommand === 'myplaylist') {
        ...
        if (playlists.length === 0) {
            const errPayload = buildErrorContainerV2({ title: 'Playlist Kosong', description: `${eError} | Kamu belum memiliki playlist tersimpan.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
    ...
    if (subcommand === 'playplaylist') {
        ...
        const pid = args.id;
        if (!pid) {
            const errPayload = buildErrorContainerV2({ title: 'ID Diperlukan', description: `${eError} | Harap masukkan ID Playlist.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        ...
        const loadingPayload = buildContainerV2({ accentColorHex: '#1DB954', title: 'Memuat Playlist', description: `⏳ Memuat \`${playlist.tracks.length}\` lagu dari **${playlist.name}**...`, footerText: ui.getFooter('music') });
        ...
        (async () => {
            for (const query of playlist.tracks) {
                try {
                    const res = await poru.resolve({ query: query.match(/^(?:https?:\/\/|spsearch:|ytmsearch:|ytsearch:|scsearch:|amsearch:)/) ? query : `ytsearch:${query}`, requester: user });
                    if (res && res.tracks && res.tracks.length > 0) {
            ...
            channel.send(syncPayload).then(m => setTimeout(()=> m.delete().catch(()=>{}), 15000)).catch(()=>{});
        })();
        return;
    }
    ...
    if (subcommand === 'pause') {
        if (!player || !player.isPlaying) {
            const errPayload = buildErrorContainerV2({ title: 'Tidak Ada Musik', description: `${eError} | Tidak ada lagu yang sedang diputar.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        if (player.isPaused) {
            const errPayload = buildErrorContainerV2({ title: 'Sudah Dijeda', description: `${eError} | Musik sudah dalam keadaan dijeda.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        player.pause(true);
    ...
    if (subcommand === 'resume') {
        ...
        if (!player.isPaused) {
            const errPayload = buildErrorContainerV2({ title: 'Tidak Dijeda', description: `${eError} | Musik tidak sedang dijeda.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        player.pause(false);
        const payload = buildContainerV2({
    ...
    if (subcommand === 'nowplaying') {
        ...
        try {
            const { drawNowPlayingCard } = require('../../plugin/canvas/nowplayingCanvas');
            const playbackInfo = { position: player.position };
            ...
            console.error('\x1b[41m\x1b[37m 💥 nowplaying \x1b[0m \x1b[31mGagal menggambar visualizer card:', error, '\x1b[0m');
    ...
    if (subcommand === 'queue') {
        if (!player || player.queue.length === 0) {
            const errPayload = buildErrorContainerV2({ title: 'Antrean Kosong', description: `${eError} | Antrean lagu kosong.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        ...
        const renderQueuePage = (page) => {
            ...
            const qList = currentQueue.map((t, i) => {
    ...
    if (subcommand === 'loop') {
        ...
        if (mode === 'track') {
            ...
            const payload = buildContainerV2({ accentColorHex: ui.getColor('primary') || '#FFB6C1', title: 'Loop Track', description: '🔂 | Mode pengulangan **LAGU SAAT INI** diaktifkan.', footerText: ui.getFooter('music') });
            return sendReply(payload, true);
        }
        if (mode === 'queue') {
            player.setLoop('QUEUE');
            const payload = buildContainerV2({ accentColorHex: ui.getColor('primary') || '#FFB6C1', title: 'Loop Queue', description: '🔁 | Mode pengulangan **SELURUH ANTREAN** diaktifkan.', footerText: ui.getFooter('music') });
            return sendReply(payload, true);
        }
        player.setLoop('NONE');
        const payload = buildContainerV2({ accentColorHex: ui.getColor('primary') || '#FFB6C1', title: 'Loop Off', description: '❌ | Mode pengulangan **DIMATIKAN**.', footerText: ui.getFooter('music') });
        return sendReply(payload, true);
    }
    ...
    if (subcommand === 'shuffle') {
        ...
        const payload = buildContainerV2({ accentColorHex: ui.getColor('primary') || '#FFB6C1', title: 'Shuffle', description: '🔀 | Urutan antrean telah diacak.', footerText: ui.getFooter('music') });
        return sendReply(payload, true);
    }
    ...
    if (subcommand === 'clear') {
        ...
        player.queue.clear();
        const payload = buildContainerV2({ accentColorHex: ui.getColor('primary') || '#FFB6C1', title: 'Clear Queue', description: '🗑️ | Seluruh antrean lagu dibersihkan.', footerText: ui.getFooter('music') });
        return sendReply(payload, true);
    }

    if (subcommand === 'stop') {
        if (!player) {
            const errPayload = buildErrorContainerV2({ title: 'Tidak Ada Frekuensi', description: `${eError} | Tidak ada frekuensi aktif.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        player.queue.clear();
        if (player.is247) {
            ...
            const MusicUIManager = require('../../plugin/music/MusicUIManager');
            await MusicUIManager.renderIdle247Panel(client.musicManager, player);
            const payload = buildContainerV2({ accentColorHex: ui.getColor('primary') || '#FFB6C1', title: '24/7 Mode Standby', description: `${ui.getEmoji('music247') || '🌙'} | Sesi audio dihentikan oleh pengguna. Mode 24/7 tetap **Siaga Pasif (0kbps Bandwidth & 0% CPU Load)**.`, footerText: ui.getFooter('music') });
            return sendReply(payload, true);
        } else {
            player.destroy();
            const payload = buildContainerV2({ accentColorHex: ui.getColor('error') || '#ef4444', title: 'Pemutusan Sistem', description: `### ${ui.getEmoji('musicStop') || '⏹️'} Pemutusan Sistem\n${divider}\nSesi audio dihentikan. Naura pamit dari Voice Channel.`, footerText: ui.getFooter('music') });
            return sendReply(payload, true);
        }
    }
    ...
    if (subcommand === 'skip') {
        if (!player || !player.currentTrack) {
            const errPayload = buildErrorContainerV2({ title: 'Tidak Bisa Skip', description: `${eError} | Tidak ada musik yang bisa dilewati.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        if (typeof player.stopTrack === 'function') player.stopTrack();
        else if (player.node && player.node.rest) player.node.rest.updatePlayer({ guildId: player.guildId, data: { track: { encoded: null } } }).catch(()=>{});
        const payload = buildContainerV2({ accentColorHex: ui.getColor('primary') || '#FFB6C1', title: 'Skip Track', description: `### ${ui.getEmoji('musicSkip') || '⏭️'} Melewati Trek\n${divider}\nMemutar urutan selanjutnya...`, footerText: ui.getFooter('music') });
        return sendReply(payload, true);
    }
    ...
    if (subcommand === 'volume') {
        ...
        if (!vol || isNaN(vol) || vol < 10 || vol > 100) {
            const errPayload = buildErrorContainerV2({ title: 'Volume Salah', description: `${eError} | Harap masukkan angka antara 10 - 100.`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        player.setVolume(vol);
        player.baseVolume = vol;
        const payload = buildContainerV2({ accentColorHex: ui.getColor('primary') || '#FFB6C1', title: 'Volume Adjusted', description: `${vol >= 50 ? ui.getEmoji('musicVolUp') || '🔊' : ui.getEmoji('musicVolDown') || '🔉'} | Volume disetel ke **${vol}%**.`, footerText: ui.getFooter('music') });
        return sendReply(payload, true);
    }
    ...
    if (subcommand === '247') {
        ...
        const isUserVip = profile?.isPremium && profile?.premiumUntil && new Date(profile.premiumUntil) > new Date();
        const isGuildVip = guildSettings?.isPremium || guildSettings?.settings?.isPremium;
        const env = require('../../src/config/env');
        const isOwner = env.OWNER_IDS.includes(user.id);

        if (!isUserVip && !isGuildVip && !isOwner) {
            const errPayload = buildErrorContainerV2({ title: 'Fitur VIP Terkunci', description: `${eError} | Akses ditolak! Mode 24/7 khusus untuk VIP Premium / Owner. Gunakan /premium untuk berlangganan!`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        if (!player) {
            const errPayload = buildErrorContainerV2({ title: 'Tidak Ada Musik', description: `${eError} | Berada di dalam Voice Channel terlebih dahulu!`, footerText: ui.getFooter('music') });
            return sendReply(errPayload, true);
        }
        player.is247 = !player.is247;
        try {
            let [guildData] = await GuildSettings.findOrCreate({ where: { guildId: guild.id } });
        ...
        const payload = buildContainerV2({ accentColorHex: player.is247 ? (ui.getColor('primary') || '#FFB6C1') : '#2b2d31', title: '24/7 Mode', description: `${ui.getEmoji('music247') || '🌙'} | Mode Siaga 24/7 **${player.is247 ? 'DIAKTIFKAN' : 'DIMATIKAN'}**.`, footerText: ui.getFooter('music') });
        return sendReply(payload, true);
    }

    const errPayload = buildErrorContainerV2({ title: 'Perintah Salah', description: `${eError} | Perintah tidak dikenali.`, footerText: ui.getFooter('music') });
    return sendReply(errPayload, true);
}
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName('resume').setDescription('Lanjutkan lagu yang dijeda'))
        .addSubcommand(sub => sub.setName('skip').setDescription('Lewati lagu saat ini'))
        .addSubcommand(sub => sub.setName('stop').setDescription('Hentikan musik dan matikan koneksi'))
        .addSubcommand(sub => sub.setName('nowplaying').setDescription('Tampilkan info lagu & visualizer'))
        .addSubcommand(sub => sub.setName('queue').setDescription('Tampilkan antrean lagu'))
        .addSubcommand(sub => sub.setName('volume').setDescription('Atur volume pemutaran (10 - 100)').addIntegerOption(opt => opt.setName('persen').setDescription('Persentase volume (10-100)').setRequired(true).setMinValue(10).setMaxValue(100)))
        .addSubcommand(sub => sub.setName('loop').setDescription('Atur mode pengulangan (Track/Queue/Off)').addStringOption(opt => opt.setName('mode').setDescription('Pilih mode loop').setRequired(true).addChoices({ name: 'Track', value: 'track' }, { name: 'Queue', value: 'queue' }, { name: 'Off', value: 'off' })))
        ...
        .addSubcommand(sub => sub.setName('playplaylist').setDescription('Putar playlist kustom dari ID').addIntegerOption(opt => opt.setName('id').setDescription('ID Playlist').setRequired(true)))
        .addSubcommand(sub => sub.setName('share').setDescription('Dapatkan kode unik untuk membagikan playlist').addIntegerOption(opt => opt.setName('id').setDescription('ID Playlist yang dibagikan').setRequired(true)))
        .addSubcommand(sub => sub.setName('load').setDescription('Salin playlist milik orang lain dari kode').addStringOption(opt => opt.setName('query').setDescription('Kode unik playlist (misal: NRA...URA)').setRequired(true))),

    async autocomplete(interaction) {
        const focusedValue = interaction.options.getFocused();
        if (!focusedValue || focusedValue.trim().length === 0) return interaction.respond([]).catch(()=>{});
        
        try {
            const SpotifyHelper = require('./spotifyHelper');
            if (SpotifyHelper.validateSpotifyUrl(focusedValue).valid) {
                return interaction.respond([{ name: `🎧 [Spotify Link] Click to load track/playlist`, value: focusedValue }]).catch(()=>{});
            }

            const searchEngine = focusedValue.startsWith('sc:') ? 'scsearch' : (focusedValue.startsWith('ytm:') ? 'ytmsearch' : 'ytsearch');
            ...
            const res = await poru.resolve({ query: `${searchEngine}:${cleanQuery}`, requester: interaction.user });

            if (!res || !res.tracks || res.tracks.length === 0) return interaction.respond([]).catch(()=>{});
    ...
    async executePrefix(message, args, client) {
        ...
        const loadingPayload = buildContainerV2({ accentColorHex: ui.getColor('dark') || '#0b0c10', title: 'Audio Processor', description: '⏳ | `Memproses sistem audio...`', footerText: ui.getFooter('music') });

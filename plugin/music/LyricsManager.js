    ...
    async fetchLyricsData(track) {
        ...
        try {
            ...
            if (response && response.data) {
                if (response.data.syncedLyrics)
                    result = { type: 'synced', data: this.parseLRC(response.data.syncedLyrics) };
                else if (response.data.plainLyrics) result = { type: 'plain', data: response.data.plainLyrics };
            }
        } catch (error) {}
        ...
        if (!result && process.env.GEMINI_API_KEY) {
            ...
            try {
                ...
                if (aiText && aiText !== 'NOT_FOUND') result = { type: 'plain', data: aiText };
            } catch (e) {}
        }
            ...
            if (pages.length > 1 && msg) {
                ...
                collector.on('collect', async (btnInteraction) => {
                    if (btnInteraction.user.id !== interaction.user.id) {
                        return btnInteraction.reply({ content: '❌ Hanya pengirim yang bisa menavigasi halaman ini.', flags: 64 });
                    }

                    if (btnInteraction.customId === 'lyrics_prev') current = Math.max(0, current - 1);
                    if (btnInteraction.customId === 'lyrics_next') current = Math.min(pages.length - 1, current + 1);

                    await btnInteraction.update(buildPagePayload(current)).catch(() => {});
                });
    ...
    async startLiveLyrics(interaction, player, track, timedLyrics, isEdit = false) {
        try {
            if (!timedLyrics || timedLyrics.length === 0) {
                return this.sendPagedLyrics(interaction, player, track, 'Lirik sinkron tidak tersedia.');
            }
            ...
            const update = async () => {
                ...
                if (index !== -1) {
                    ...
                    if (timedLyrics[index].text.trim() !== '') {
                        linesText.push(`### ${ui.getEmoji('dot') || '▶️'} **${timedLyrics[index].text}**`);
                    } else {

...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (player && player.isPlaying) {
            const errPayload = buildErrorContainerV2({ title: 'Sistem Sibuk', description: '❌ Naura sedang memutar musik. Hentikan musik terlebih dahulu untuk bermain Trivia.', footerText: ui.getFooter('music') });
        ...
        if (!res || !res.tracks || res.tracks.length === 0) {
            const errPayload = buildErrorContainerV2({ title: 'Gagal Muat Playlist', description: '❌ Gagal memuat playlist untuk Trivia.', footerText: ui.getFooter('music') });
            return interaction.editReply(errPayload);
        }
        ...
        const payload = buildContainerV2({
            ...
            title: '🎵 Music Trivia!',
            description: 'Dengarkan cuplikan lagu ini dan ketik **judul lagunya** di chat secepat mungkin!\nKamu punya waktu **15 detik**.',

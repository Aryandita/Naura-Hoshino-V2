    ...
    async execute(interaction) {
        ...
        if (subcommand === 'add') {
            if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
                const errPayload = buildErrorContainerV2({ title: 'Akses Ditolak', description: '❌ Kamu harus memiliki izin Manage Server untuk menambah soundboard.', footerText: ui.getFooter('music') });
            ...
            if (Object.keys(currentSettings.soundboards).length >= 20) {
                const errPayload = buildErrorContainerV2({ title: 'Batas Maksimum', description: '❌ Batas maksimal soundboard adalah 20 per server.', footerText: ui.getFooter('music') });
                return interaction.editReply(errPayload);
            }
        ...
        if (subcommand === 'hapus') {
            if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild)) {
                const errPayload = buildErrorContainerV2({ title: 'Akses Ditolak', description: '❌ Kamu harus memiliki izin Manage Server untuk menghapus soundboard.', footerText: ui.getFooter('music') });
                return interaction.editReply(errPayload);
            }
            
            const nama = interaction.options.getString('nama').toLowerCase();
            if (!currentSettings.soundboards[nama]) {
                const errPayload = buildErrorContainerV2({ title: 'Tidak Ditemukan', description: '❌ Soundboard tidak ditemukan.', footerText: ui.getFooter('music') });
                return interaction.editReply(errPayload);
            }
        ...
        if (subcommand === 'list') {
            ...
            if (list.length === 0) {
                const emptyPayload = buildContainerV2({ title: 'Daftar Soundboard', description: 'ℹ️ Belum ada soundboard kustom di server ini.', footerText: ui.getFooter('music') });
        ...
        if (subcommand === 'play') {
            ...
            if (!voiceChannel) {
                const errPayload = buildErrorContainerV2({ title: 'Koneksi Gagal', description: '❌ Kamu harus berada di Voice Channel terlebih dahulu.', footerText: ui.getFooter('music') });
                return interaction.editReply(errPayload);
            }
            ...
            if (!url) {
                const errPayload = buildErrorContainerV2({ title: 'Tidak Ditemukan', description: '❌ Soundboard tidak ditemukan di server ini.', footerText: ui.getFooter('music') });
                return interaction.editReply(errPayload);
            }
            ...
            if (!player) {
                ...
                const errPayload = buildErrorContainerV2({ title: 'Voice Channel Berbeda', description: '❌ Bot sedang berada di Voice Channel lain.', footerText: ui.getFooter('music') });
                return interaction.editReply(errPayload);
            }
            ...
            try {
                ...
                if (!res || !res.tracks || res.tracks.length === 0) {
                    const errPayload = buildErrorContainerV2({ title: 'Gagal Muat Audio', description: '❌ Gagal memuat efek suara. Pastikan URL valid.', footerText: ui.getFooter('music') });
                    return interaction.editReply(errPayload);
                }
                ...
                const errPayload = buildErrorContainerV2({ title: 'Gagal Jaringan', description: '❌ Terjadi kesalahan jaringan saat memuat efek suara.', footerText: ui.getFooter('music') });
                return interaction.editReply(errPayload);
            }
        }
    }

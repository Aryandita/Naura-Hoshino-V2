    ...
    async _executeMessage(message, prompt, isOwner, isPremium) {
        ...
        try {
            ...
            // ==========================================
                ...
                let characterSlug = process.env.VERBA_SLUG_GENERAL || process.env.VERBA_CHARACTER_SLUG || 'naura';
                let roleInfo = 'Member Biasa';
                if (isOwner) {
                    characterSlug = process.env.VERBA_SLUG_OWNER || process.env.VERBA_CHARACTER_SLUG || 'naura';
                    roleInfo = 'Owner/Developer';
                } else if (isPremium) {
                    characterSlug = process.env.VERBA_SLUG_PREMIUM || process.env.VERBA_CHARACTER_SLUG || 'naura';
                    ...
                    try {
                        ...
                        if (sessionData.chat && Array.isArray(sessionData.chat._history)) {
                            if (sessionData.chat._history.length > historyLimit) {
                                sessionData.chat._history = sessionData.chat._history.slice(sessionData.chat._history.length - historyLimit);
                            }
                            let totalChars = sessionData.chat._history.reduce((sum, msg) => sum + (msg.parts?.[0]?.text?.length || 0), 0);
                            while (totalChars > 4000 && sessionData.chat._history.length > 2) {
                                sessionData.chat._history.splice(0, 2); // Hapus 1 pasang (user + bot)
                                totalChars = sessionData.chat._history.reduce((sum, msg) => sum + (msg.parts?.[0]?.text?.length || 0), 0);
                            }
                        }

...
async function buildInstruction(guildId) {
    let instruction = 'Kamu adalah Naura Hoshino, gadis asisten virtual yang ceria, perhatian, dan murah senyum. '
    ...
    const [row] = await GuildSettings.findOrCreate({ where: { guildId: guildId || 'DM' } });
...
module.exports = async function chat(interaction) {
    ...
    try {
        ...
        replyText = 'Maaf yaa, pikiran Naura lagi tersendat sedikit. Coba tanya lagi sebentar lagi, ya?';
    }

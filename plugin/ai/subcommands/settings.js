...
function overviewCard(interaction, ai) {
    const description = [
        ...
        asBlock(ai.serverKnowledge, 'Belum diatur, Naura belum punya catatan khusus')
    ].join('\n');
...
module.exports = async function settings(interaction) {
    if (!interaction.guild) {
        return interaction.editReply(failCard(
    ...
    const [row] = await GuildSettings.findOrCreate({ where: { guildId: interaction.guild.id } });
    ...
    if (newPersona !== null) {
        ...
        settingsData.ai.customPersona = newPersona.trim() === '' ? null : newPersona.trim();
        changes.push(`${e('cheers', '\u2705')} Sifat khusus Naura sudah diperbarui.`);
    }
    ...
    if (newKnowledge !== null) {
        ...
        settingsData.ai.serverKnowledge = newKnowledge.trim() === '' ? null : newKnowledge.trim();
        changes.push(`${e('cheers', '\u2705')} Catatan khusus server sudah Naura simpan.`);
    }

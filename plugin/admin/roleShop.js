...
module.exports = {
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addRoleOption((opt) =>
          opt
            .setName("role")
            .setDescription("Role yang disewakan")
            .setRequired(true),
        )
        .addIntegerOption((opt) =>
          opt
            .setName("durasi_hari")
            .setDescription("Durasi sewa (hari)")
            .setRequired(true),
        )
        .addIntegerOption((opt) =>
          opt
            .setName("harga_nsf")
            .setDescription("Harga dalam Naura Star Fragment (NSF)")
            .setRequired(true),
        )
        .addIntegerOption((opt) =>
          opt
            .setName("harga_coin")
            .setDescription("Harga dalam Naura Coin")
            .setRequired(true),
        )
        .addIntegerOption((opt) =>
          opt
            .setName("harga_coupon")
            .setDescription("Harga dalam Naura Coupon")
            .setRequired(true),
        ),
    )
    .addSubcommand((sub) =>
      sub.setName("buy").setDescription("User: Beli/Sewa role yang tersedia."),
    ),
  ...
  async execute(interaction) {
    ...
    if (subCmd === "setup") {
      ...
      return interaction.reply({
        content: `✅ Role ${role.name} berhasil ditambahkan ke Role Shop!`,
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    if (subCmd === "buy") {
      ...
      if (shop.length === 0) {
        return interaction.reply({
          content: "Belum ada role yang disewakan di server ini.",
          flags: MessageFlags.Ephemeral,
        });
      }

    ...
    async execute(interaction) {
        ...
        if (!interaction.guild) {
            return interaction.reply({
                ...buildErrorContainerV2({ errorMessage: 'Fitur report hanya tersedia di dalam server.' }),
        ...
        if (!aiAutomodSettings?.enabled) {
            return interaction.reply({
                ...buildErrorContainerV2({
                    errorMessage: 'Fitur AI Report belum diaktifkan di server ini.\n\nAdmin dapat mengaktifkannya melalui `/setup` → panel **AI Automod**.'
                }),
                flags: MessageFlags.Ephemeral
            });
        }
        ...
        try {
            ...
            logger.error('[Report Command] Error saat proses AI report:', error.message);

            const errPayload = buildErrorContainerV2({ errorMessage: 'Terjadi kesalahan saat memproses laporan. Coba lagi nanti.' });

            if (interaction.deferred || interaction.replied) {
                await interaction.editReply(errPayload).catch(() => {});
            } else {
                await interaction.reply({ ...errPayload, flags: MessageFlags.Ephemeral }).catch(() => {});
            }
        }
    }
};

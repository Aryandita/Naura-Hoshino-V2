const { SlashCommandBuilder, PermissionsBitField, AuditLogEvent, AttachmentBuilder, MessageFlags } = require('discord.js');
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(opt => opt.setName('aksi').setDescription('Aksi yang ingin dicek').setRequired(true).addChoices(
        ...
        .addIntegerOption(opt => opt.setName('limit').setDescription('Jumlah log untuk diambil (Max 50)').setMinValue(1).setMaxValue(50)),
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const descriptionText = entries.map(entry => {
                const executor = entry.executor ? `<@${entry.executor.id}>` : 'Tidak diketahui';
                const target = entry.target ? (entry.target.username || entry.target.name || entry.target.id) : 'Tidak diketahui';
            ...
            const errPayload = buildErrorContainerV2({ title: 'Gagal Audit Log', description: `${ui.getEmoji('error') || '❌'} Gagal mengambil Audit Log. Pastikan bot memiliki izin View Audit Log.`, footerText: ui.getFooter('core') });

...
function rejectionReason(role, me) {
    ...
    if (me && role.position >= me.roles.highest.position) return 'posisinya di atas role Naura';
    return null;
}
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        for (let i = 1; i <= MAX_SLOTS; i += 1) {
            ...
            if (role) slots.push({ role, label: interaction.options.getString(`label${i}`) });
        }
        ...
        try {
            ...
            return interaction.reply(ephemeral(buildErrorContainerV2({
                ...
                description: 'Naura sepertinya belum punya izin mengirim pesan di channel ini. Boleh dicek dulu?',
                footerText: ui.getFooter('core')
            })));
        }

        const catatan = rejected.length > 0
            ? `\n\nTapi ada yang Naura lewati: ${rejected.join(', ')}`
            : '';

        return interaction.reply(ephemeral(buildContainerV2({

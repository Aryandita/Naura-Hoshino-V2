...
class CanvasUtils {
    ...
    // ==========================================
    static async generateRankCard(user, level, currentXp, requiredXp, rank, roleBadge, isPremium = false, customBgUrl = null, customBorderUrl = null) {
        ...
        // DEFAULT BACKGROUND
        if (!hasCustomBg) {
            const isOwner = ['843232230689931284', 'OWNER_ID_DISINI'].includes(user.id);
     ...
     */
    static async generatePremiumTierCard(user, tierData = {}, isPremium = false, daysLeft = 0, premiumUntil = null) {
        const W = 820, H = 280;
        ...
        const tierColorMap = {
            supporter: { primary: '#C0C0C0', glow: 'rgba(192,192,192,0.4)', from: '#1a1a1a', to: '#2a2a2a', badge: 'SUPPORTER' },
            friends:   { primary: '#A855F7', glow: 'rgba(168,85,247,0.4)',   from: '#160a2e', to: '#1e0b3c', badge: 'FRIENDS'   },
            vip:       { primary: '#FFD700', glow: 'rgba(255,215,0,0.4)',    from: '#1c1500', to: '#2a1e00', badge: 'V.I.P'     },
            none:      { primary: '#8e98b0', glow: 'rgba(142,152,176,0.2)',  from: '#0c0c14', to: '#1b1b2f', badge: 'REGULAR'   },
        };
        ...
        try {
            ...
            drawRoundedRect(ctx, avatarX, avatarY, avatarSize, avatarSize, avatarSize / 2, '#333');
        }

        // ── NEON BADGE (pojok kanan atas kartu) ──────────────────
        const badgeText = tc.badge;
        const badgeW = 130, badgeH = 32, badgeX = W - badgeW - 28, badgeY = 30;
        ctx.save();
        ctx.shadowColor = tc.primary;
        ctx.shadowBlur = 20;
        ...
        if (isPremium && premiumUntil) {
            ...
            ctx.fillText(`STATUS: AKTIF  •  Berakhir: ${new Date(premiumUntil).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' })}`, textX, 165);
        } else {
            ctx.fillText('STATUS: TIDAK AKTIF — Beli VIP untuk unlock fitur eksklusif!', textX, 165);
        }
        ...
        if (isPremium && daysLeft > 0) {
            ...
            const barX = textX, barY = 185, barW = W - textX - 45, barH = 18;
            ...
            ctx.fillText('Langganan untuk mengaktifkan bar ini', textX + (W - textX - 45) / 2, 199);
        }
    ...
    static async generatePremiumInfoCard(user, isPremium, daysLeft) {
        const tierData = isPremium
            ? { name: 'V.I.P PREMIUM MEMBER', tier: daysLeft > 90 ? 'vip' : daysLeft > 30 ? 'friends' : 'supporter' }
            : { name: 'Regular Member', tier: 'none' };
        return CanvasUtils.generatePremiumTierCard(user, tierData, isPremium, daysLeft, null);
    }
}

...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('cosmetic')
        .setDescription('\u{1F3A8} Manajemen kosmetik profil canvas kamu (Background & Border)')
        .addSubcommand(subcommand =>
            subcommand
                .setName('shop')
                .setDescription('\u{1F6D2} Beli aset kosmetik baru untuk menghias profilmu')
        )
        .addSubcommand(subcommand =>
            subcommand
                .setName('equip')
...
async function handleShop(interaction, userId) {
    ...
    if (!assets || assets.length === 0) {
        return interaction.editReply('\u{1F6D2} Toko kosmetik saat ini sedang kosong. Harap tunggu update selanjutnya!');
    }
    ...
    const msg = await interaction.editReply({ ...shopPayload, components: [row] });

    const filter = i => i.customId === 'cosmetic_shop_select' && i.user.id === userId;
...
async function handleEquip(interaction, userId) {
    ...
    if (!ownedCosmetics || ownedCosmetics.length === 0) {
        return interaction.editReply('👗 Kamu belum memiliki kosmetik apapun. Kunjungi `/cosmetic shop` terlebih dahulu.');
    }
    ...
    const equipPayload = buildContainerV2({
        ...
        description: 'Pilih kosmetik yang ingin kamu gunakan. Jika kamu memilih tipe yang sama dengan yang sedang aktif, kosmetik lama akan digantikan.',
        footerText: 'Naura Cosmetic System'
    });

    const options = ownedCosmetics.filter(uc => uc.asset).map(uc => {
        return {
    ...
    const msg = await interaction.editReply({ ...equipPayload, components: [row, btnRow] });

    const filter = i => ['cosmetic_equip_select', 'cosmetic_unequip_all'].includes(i.customId) && i.user.id === userId;
    const collector = msg.createMessageComponentCollector({ filter, time: 60000 });
    ...
    collector.on('collect', async i => {
        ...
        if (i.customId === 'cosmetic_equip_select') {
            ...
            const sameTypeCosmetics = ownedCosmetics.filter(uc => uc.asset && uc.asset.type === targetType && uc.id !== selectedUcId);

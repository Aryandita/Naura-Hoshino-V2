...
const { generateMusicPanelImage } = require('./canvasHelper');

const loopBreakers = new Map();
// Cleanup periodic (Rule 1.8 Memory Safety)
setInterval(() => loopBreakers.clear(), 1800000).unref?.();
    ...
    initialize() {
        ...
        this.poru.on('nodeConnect', async node => {
            ...
            try {
                ...
                if (restoredCount > 0)
                    console.log(
                        `\x1b[45m\x1b[37m 🔄 RESURRECT \x1b[0m \x1b[35mBerhasil membangkitkan Naura ke ${restoredCount} Voice Channel!\x1b[0m`
                    );
            } catch (e) {}
        });
        ...
        this.poru.on('nodeDisconnect', node =>
            console.log(`\x1b[43m\x1b[30m ⚠️ WARNING \x1b[0m \x1b[33mKoneksi Lavalink Terputus!\x1b[0m`)
        );
        ...
        this.client.on('interactionCreate', async interaction => {
            ...
            if (!player)
                return interaction
                    .reply({ content: `❌ | Sesi transmisi telah berakhir.`, ephemeral: true })
                    .catch(() => {});
            ...
            if (!memberVoice || memberVoice.id !== player.voiceChannel) {
                return interaction
                    .reply({
                        content: `❌ | Akses ditolak. Anda tidak berada di Voice Channel yang sama.`,
                        ephemeral: true
                    })
                    .catch(() => {});
            }
            ...
            if (interaction.isStringSelectMenu() && interaction.customId === 'music_recommendation') {
                ...
                try {
                    const res = await this.poru.resolve({
                        query: `https://www.youtube.com/watch?v=${trackIdentifier}`,
                        source: 'youtube',
                    ...
                    if (res && res.tracks.length > 0) {
                        ...
                        return interaction
                            .editReply({ content: `✅ | **${res.tracks[0].info.title}** ditambahkan ke antrean.` })
                            .catch(() => {});
                    }
                } catch (e) {
                    return interaction.editReply({ content: `❌ | Gagal memuat trek rekomendasi.` }).catch(() => {});
                }
                return;
            }
            ...
            if (requiresDJ.includes(interaction.customId) && !isRequester && !isDJ) {
                return interaction.editReply({
                    content: `🛡️ | **Akses Ditolak!** Hanya peminta lagu atau Staff yang diizinkan.`
                });
            }
            ...
            if (interaction.isStringSelectMenu() && interaction.customId === 'music_filter') {
                ...
                if (filterType === 'nightcore')
                    applyFilter({ timescale: { speed: 1.2, pitch: 1.2, rate: 1 } }, 'Nightcore Shift');
                if (filterType === 'vaporwave')
                    applyFilter({ timescale: { speed: 0.8, pitch: 0.8, rate: 1 } }, 'Vaporwave Reverb');

                this.updatePanelEmbed(player);
                return interaction.editReply({
                    content: `🎛️ | Filter audio diubah ke: **${player.currentFilterName}**`
                });
            }
            ...
            if (interaction.isButton()) {
                ...
                if (id === 'music_save') {
                    try {
                        ...
                        if (!player.currentTrack || !player.currentTrack.info)
                            return interaction.editReply({ content: `❌ | Tidak ada data trek untuk disimpan.` });
                        ...
                        if (!playlist.includes(savedData)) {
                            ...
                            return interaction.editReply({
                                content: `💖 | Trek ditambahkan ke **Naura Cloud Playlist**!`
                            });
                        } else {
                            return interaction.editReply({
                                content: `⚠️ | Lagu ini sudah ada di daftar favorit Anda.`
                            });
                        }
                    } catch (e) {
                        return interaction.editReply({ content: `❌ | Sinkronisasi database gagal.` });
                    }
                }
                ...
                if (id === 'music_autoplay') {
                    ...
                    this.updatePanelEmbed(player);
                    return interaction.editReply({
                        content: `📻 | Autoplay diatur menjadi: **${player.autoplay ? 'Aktif' : 'Nonaktif'}**`
                    });
                }
                ...
                if (id === 'music_247') {
                    ...
                    return interaction.editReply({
                        content: `🌙 | Mode 24/7 diatur menjadi: **${player.is247 ? 'Aktif' : 'Nonaktif'}**`
                    });
                }
                ...
                if (id === 'music_pause') {
                    ...
                    this.updatePanelEmbed(player);
                    return interaction.editReply({
                        content: player.isPaused ? `⏸️ | Transmisi dijeda.` : `▶️ | Transmisi dilanjutkan.`
                    });
                }
                if (id === 'music_skip') {
                    player.stop();
        ...
        this.poru.on('trackStart', async (player, track) => {
            try {
                ...
                if (now - breaker.lastTime < 2000) {
                    breaker.count++;
                } else {
                    breaker.count = 1;
                    breaker.lastTime = now;
                }
                ...
                if (breaker.count >= 5) {
                    ...
                    if (channel)
                        channel
                            .send('🚨 | **Sistem Anti-Spam:** Lagu diblokir beruntun. Koneksi diputus demi keamanan.')
                            .catch(() => {});
                    player.destroy();
                ...
                if (activeTrack.info.requester && activeTrack.info.length > 0) {
                    try {
                        const [userProfile] = await UserProfile.findOrCreate({
                            where: { userId: activeTrack.info.requester.id }
                        });
                        userProfile.music_tracksListened = (userProfile.music_tracksListened || 0) + 1;
                        userProfile.music_totalDurationMs =
                            BigInt(userProfile.music_totalDurationMs || 0) + BigInt(activeTrack.info.length);
                ...
                try {
                    const searchRes = await this.poru.resolve({
                        query: `https://www.youtube.com/watch?v=${activeTrack.info.identifier}&list=RD${activeTrack.info.identifier}`,
                        source: 'youtube'
                    });
                    if (searchRes && searchRes.tracks) {
                        recommendedTracks = searchRes.tracks
                            .filter(t => t.info.identifier !== activeTrack.info.identifier)
                            .slice(0, 5);
                    }
                } catch (e) {}
                ...
                const generatePanelPayload = async currentPos => {
                    ...
                    const timeStr = activeTrack.info.isStream
                        ? 'LIVE'
                        : `${formatDur(currentPos)} / ${formatDur(activeTrack.info.length)}`;
                    const requesterText = activeTrack.info.requester?.id
                        ? `<@${activeTrack.info.requester.id}>`
                        : `\`📻 Autoplay\``;
                    ...
                    const panelPayload = buildContainerV2({
                        ...
                        authorName: '✦ N A U R A A U D I O P A N E L ✦',
                        iconURL: this.client.user.displayAvatarURL(),
                        description: `### ${e('nowplaying') || '🎵'} [${activeTrack.info.title}](${activeTrack.info.uri})\n` +
                        ...
                        bannerAttachmentName: 'naura-audio-panel.png',
                        footerText: 'Naura Intelligence • Pembaruan waktu nyata (Real-Time)'
                    });
                        ...
                        new StringSelectMenuBuilder()
                            ...
                            .addOptions([
                                ...
                                },
                                {
                                    ...
                                    description: 'Peningkatan tempo & pitch',
                                    value: 'nightcore',
                                    emoji: '🐹'
                                },
                                {
                                    label: 'Vaporwave Reverb',
                                    description: 'Gema ruang (Slowed)',
                                    value: 'vaporwave',
                                    emoji: '🌌'
                                }
                            ])
                    ...
                    const rowMedia = new ActionRowBuilder().addComponents(
                        new ButtonBuilder()
                            ...
                            .setEmoji(fallbackGetUIEmoji('musicPlayPause', '⏯️'))
                            .setStyle(ButtonStyle.Secondary),
                        new ButtonBuilder()
                            .setCustomId('music_stop')
                            .setEmoji(fallbackGetUIEmoji('musicStop', '⏹️'))
                            .setStyle(ButtonStyle.Danger),
                        new ButtonBuilder()
                            .setCustomId('music_skip')
                            .setEmoji(fallbackGetUIEmoji('musicSkip', '⏭️'))
                            .setStyle(ButtonStyle.Secondary),
                        new ButtonBuilder()
                            .setCustomId('music_loop')
                            .setEmoji(fallbackGetUIEmoji('musicLoop', '🔁'))
                            .setStyle(player.loop !== 'NONE' ? ButtonStyle.Primary : ButtonStyle.Secondary),
                        new ButtonBuilder()
                            .setCustomId('music_autoplay')
                            .setEmoji(fallbackGetUIEmoji('musicAutoplay', '📻'))
                    ...
                    const rowUtils = new ActionRowBuilder().addComponents(
                        new ButtonBuilder()
                            ...
                            .setEmoji(fallbackGetUIEmoji('musicVolDown', '🔉'))
                            .setStyle(ButtonStyle.Secondary),
                        new ButtonBuilder()
                            .setCustomId('music_volup')
                            .setEmoji(fallbackGetUIEmoji('musicVolUp', '🔊'))
                            .setStyle(ButtonStyle.Secondary),
                        new ButtonBuilder()
                            .setCustomId('music_shuffle')
                            .setEmoji(fallbackGetUIEmoji('musicShuffle', '🔀'))
                            .setStyle(ButtonStyle.Secondary),
                        new ButtonBuilder()
                            .setCustomId('music_247')
                            .setEmoji(fallbackGetUIEmoji('music247', '🌙'))
                    ...
                    return { ...panelPayload, components: components, files: [attachment] };
                };
                ...
                logError('Poru TrackStart Error', error);
            }
        });
        ...
        this.poru.on('queueEnd', async player => {
            ...
            if (player.autoplay && player.previousTrack && player.previousTrack.info) {
                ...
                if (player.autoplayErrorCount > 5) {
                    ...
                    if (channel)
                        channel
                            .send('⚠️ | **Autoplay Dimatikan:** Algoritma gagal menemukan lagu yang valid.')
                            .catch(() => {});
                ...
                try {
                    ...
                    let res = await this.poru.resolve({
                    ...
                    if (res.loadType === 'playlist' || res.loadType === 'PLAYLIST_LOADED') {
                        ...
                        const fallbackRes = await this.poru.resolve({
                            query: `spsearch:${player.previousTrack.info.author}`,
                            source: 'spotify'
                        });
                        if (fallbackRes && fallbackRes.tracks.length > 0) {
                            player.queue.add(
                                fallbackRes.tracks[Math.floor(Math.random() * Math.min(3, fallbackRes.tracks.length))]
                            );
                            return player.play();
                        }
                    }
            ...
            player.destroy();
            const channel = this.client.channels.cache.get(player.textChannel);
            if (channel && !player.autoplayErrorCount)
                channel
                    .send(`⏹️ | Antrean lagu telah habis. Naura pamit dari Voice!`)

    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("name").setDescription("Nama aset").setRequired(true),
        )
        .addStringOption((option) =>
          option
            .setName("type")
            .setDescription("Tipe aset (background/border)")
            .setRequired(true)
            .addChoices(
        ...
        .addStringOption((option) =>
          option
            .setName("url")
            .setDescription("URL gambar aset (jpg/png)")
            .setRequired(true),
        )
        .addIntegerOption((option) =>
          option
            .setName("price")
            .setDescription("Harga di toko (Naura Coins)")
            .setRequired(true),
        )
        .addBooleanOption((option) =>
          option
            .setName("premium_only")
            .setDescription("Apakah eksklusif VIP?")
            .setRequired(false),
        ),
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addIntegerOption((option) =>
          option
            .setName("id")
            .setDescription("ID dari CanvasAsset")
            .setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
  ...
  async execute(interaction) {
    ...
    if (!ownerIds.includes(interaction.user.id)) {
      return interaction.reply({
        content:
          "❌ Anda tidak memiliki izin. Hanya Bot Owner yang dapat menggunakan command ini.",
    ...
    if (subCmd === "add") {
      ...
      const premiumOnly =
        interaction.options.getBoolean("premium_only") || false;
      ...
      try {
        ...
        if (deleted) {
          return interaction.editReply(
            `✅ Aset dengan ID \`${id}\` berhasil dihapus.`,
          );
        } else {
          return interaction.editReply(
        ...
        return interaction.editReply(
          "❌ Terjadi kesalahan database saat menghapus aset.",
        );
      }
    } else if (subCmd === "list") {
      try {
        ...
        let text = assets
          .map(
            (a) =>
              `**[${a.id}]** ${a.name} (${a.type}) - ${a.price} NC ${a.isPremiumOnly ? "[VIP]" : ""}`,
          )
          .join("\n");
        ...
        return interaction.editReply(
          "❌ Terjadi kesalahan saat mengambil daftar.",
        );
      }
    }
  },

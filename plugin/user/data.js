...
const {
  getUserProfile,
  getUserSurvival,
} = require("../../src/managers/cacheManager");
      ...
      sub
        ...
        .addBooleanOption((opt) =>
          opt
            .setName("konfirmasi")
            .setDescription(
              "Kamu harus yakin 100% karena data TIDAK BISA KEMBALI.",
            )
            .setRequired(true),
        ),
    ),
  ...
  async execute(interaction, client) {
    ...
    if (sub === "export") {
      ...
      try {
        ...
        const cosmetics = await UserCosmetic.findAll({
          where: { userId },
          raw: true,
        });
        ...
        const buffer = Buffer.from(
          JSON.stringify(dataExport, null, 4),
          "utf-8",
        );
        const attachment = new AttachmentBuilder(buffer, {
          name: `naura_data_export_${userId}.json`,
        });
        ...
        return await interaction.editReply({
          content:
            "✅ Ekspor data berhasil! Naura sudah mengirimkan datamu langsung ke DM ya kak~",
        });
      } catch (err) {
        logger.error(`[DATA EXPORT] Gagal ekspor data user ${userId}:`, err);
        return await interaction.editReply({
          content:
            "❌ Uh oh, Naura kesulitan membungkus datamu tadi. Coba lagi nanti ya kak!",
        });
      }
    }
    ...
    if (sub === "delete") {
      ...
      if (!confirmed) {
        return await interaction.reply({
          content:
            "Pilihan konfirmasi harus disetel ke `True` jika kakak benar-benar ingin menghapus data. Hati-hati ya kak, ini tidak bisa dibatalkan! 🥺",

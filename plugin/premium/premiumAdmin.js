...
async function freshCode() {
    for (let attempt = 0; attempt < CODE_ATTEMPTS; attempt += 1) {
        const code = CODE_PREFIX + crypto.randomBytes(5).toString('hex').toUpperCase();
...
async function runAdd(interaction, targetUser, profile) {
    ...
    const delivered = await sendPremiumDM(interaction.client, targetUser.id, 'activated', {
        username: targetUser.username,
        tierName: displayName,
        premiumUntil: newExpiry
    });

    const crown = ui.getEmoji('premium_crown') || ui.getEmoji('premium_badge') || '\ud83d\udc8e';
...
async function runRemove(interaction, targetUser, profile) {
    ...
    const delivered = await sendPremiumDM(interaction.client, targetUser.id, 'removed', {
        username: targetUser.username
    });
...
async function runGenerateVoucher(interaction) {
    ...
    if (!code) {
        return interaction.editReply(buildContainerV2({
            ...
            expression: 'error',
            description: 'Naura sudah mencoba beberapa kali tapi kodenya selalu bentrok. Coba sekali lagi ya.',
            footerText: ui.getFooter('premium')
        }));
    }
...
async function runStats(interaction) {
    ...
    for (const sub of subscribers) {
        ...
        if (rows.length < 10) {
            rows.push(`${rows.length + 1}. <@${sub.userId}> - **${left} hari lagi** (${stamp(expiry, 'R')})`);
        }
    }

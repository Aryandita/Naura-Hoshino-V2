...
async function runCheck(interaction, targetUser, profile) {
    ...
    const tierEmoji = ui.getPremiumEmoji(tierKey) || (ui.getEmoji('premium_badge') || '\ud83d\udc8e');

    const attachment = await buildTierCard(
        targetUser, tierData, isPremium, daysLeft, expiry, 'premium-status.png'
    );

    const payload = buildContainerV2({
        accentColorHex: isPremium ? ui.getPremiumColor(tierKey) : ui.getColor('primary'),
        authorName: 'Naura V.I.P Subscription',
        iconURL: targetUser.displayAvatarURL(),
        title: isPremium ? `${tierEmoji} Status V.I.P aktif` : 'Status V.I.P belum aktif',
        expression: isPremium ? 'celebrate' : 'info',
        description: isPremium && expiry
    ...
    return interaction.editReply({ ...payload, files: attachment ? [attachment] : [] });
}

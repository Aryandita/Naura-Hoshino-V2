...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub => sub.setName('info').setDescription('Lihat paket dan harga langganan V.I.P.'))
        .addSubcommand(sub => sub.setName('benefits').setDescription('Bandingkan fitur lengkap semua tier V.I.P.'))
        ...
        .addSubcommand(sub => sub.setName('generate_voucher').setDescription('[OWNER] Buat kode redeem V.I.P baru.')
            .addIntegerOption(opt => opt.setName('days').setDescription('Lama V.I.P dalam hari').setRequired(true).setMinValue(1))
            .addIntegerOption(opt => opt.setName('expired_in_days').setDescription('Batas waktu klaim kode (opsional)').setRequired(false).setMinValue(1)))
    ...
    async execute(interaction) {
        ...
        if (OWNER_ONLY.includes(subcommand) && !store.isOwner(interaction.user.id)) {
        ...
        switch (subcommand) {
            ...
            case 'redeem': {
                const selfProfile = await cacheManager.getUserProfile(interaction.user.id);
    ...
    async executePrefix(message, args, client) {
        if (!args || args.length === 0) {
            return message.reply('Pakai `/premium` untuk tampilan interaktifnya, atau `n!premium check` kalau mau cepat.');
        }

        const subcommand = args[0].toLowerCase();

        if (['add', 'remove'].includes(subcommand) && !store.isOwner(message.author.id)) {
        ...
        if (subcommand === 'add') {
            if (!mentioned) return message.reply('Formatnya begini ya: `n!premium add @user <hari>`');

            // Argumen numerik dicari eksplisit. Memakai argumen terakhir bisa
            // salah membaca mention yang ditulis di belakang.
            const numeric = args.slice(1).map(a => parseInt(a, 10)).find(n => Number.isInteger(n) && n > 0);
            if (!numeric) return message.reply('Jumlah harinya belum valid. Coba tulis angka lebih dari nol ya.');

            const newExpiry = await store.grantPremium(targetUser.id, profile, numeric);
            const tierInfo = tierByKey(ui.getPremiumTier(numeric, true));

            const delivered = await sendPremiumDM(client, targetUser.id, 'activated', {
            ...
            return message.reply(buildContainerV2({
                ...
                description: [
                    ...
                    delivered ? '' : '-# DM ke user gagal terkirim, tolong kabari manual ya.'
                ].filter(Boolean).join('\n'),
                footerText: ui.getFooter('premium')
            }));
        }
        ...
        if (subcommand === 'remove') {
            ...
            const delivered = await sendPremiumDM(client, targetUser.id, 'removed', { username: targetUser.username });
        ...
        if (subcommand === 'check') {
            ...
            if (isPremium && expiry) {
                return message.reply(buildContainerV2({
                    ...
                    title: `${ui.getPremiumEmoji(tierKey) || ''} Status V.I.P aktif`.trim(),
        ...
        return message.reply('Sub-perintah itu belum Naura kenali. Coba `check`, atau pakai `/premium` untuk tampilan penuhnya.');
    },

...
function start() {
    ...
    flushTimer = setInterval(() => {
        flush().catch(error => logger.error('[XP BUFFER] Flush berkala gagal:', error.message));
    }, FLUSH_INTERVAL_MS);
...
async function readBase(guildId, userId) {
    ...
    if (isEnabled()) {
        ...
        if (cached && typeof cached.xp === 'number' && typeof cached.level === 'number') {
            return cached;
        }
    }
...
async function settleUser(guildId, userId) {
    ...
    try {
        ...
        logger.error('[XP BUFFER] Gagal menyetorkan XP ke database:', error.message);
...
async function flush() {
    ...
    try {
        ...
        for (const [field, value] of Object.entries(pendingXp)) {
            ...
            try {
                await applyToDatabase(parsed.guildId, parsed.userId, value, pendingMsg[field]);
                await invalidateBase(parsed.guildId, parsed.userId);
                written += 1;
            } catch (error) {
                logger.error('[XP BUFFER] Gagal menulis XP batch:', error.message);
                await addXp(parsed.guildId, parsed.userId, toInt(value), toInt(pendingMsg[field]));
            }
        }
        ...
        if (written > 0) {
            logger.info(`[XP BUFFER] Menyetorkan XP ${written} pengguna ke database.`);
        }

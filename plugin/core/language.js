...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const newLangId = await languageManager.getUserLanguage(userId, interaction.guildId);
            
            // Ambil respons sukses berdasarkan bahasa yang baru
            const newStr = languageManager.getString(newLangId, 'core.language') || {};
            const title = newStr.successTitle || '✅ Bahasa Diperbarui!';
            const descId = 'Bahasa kamu berhasil diubah menjadi **Indonesia**.';
            const descEn = 'Your language has been successfully changed to **English**.';
            const descDefault = 'Bahasa kamu sekarang mengikuti pengaturan bawaan server ini.';
            ...
            const errorMsg = str.error || 'Terjadi kesalahan saat menyimpan pengaturan bahasa.';
            const payload = buildContainerV2({
                title: '❌ Gagal',

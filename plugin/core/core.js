...
const LINKS = {
  ...
  DASHBOARD: rawDashboard.startsWith("http")
    ? rawDashboard
    : `http://${rawDashboard}`,
  INVITE:
    ui.invite ||
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("stats")
        .setDescription(
          "Lihat diagnostik spesifikasi server, RAM, dan OS Naura.",
        ),
    )
    .addSubcommand((sub) =>
      sub
        .setName("info")
        .setDescription(
          "Tampilkan info spesifik server saat ini atau info bot secara umum.",
        ),
    )
    .addSubcommand((sub) =>
      sub
        .setName("about")
        .setDescription("Kenalan lebih dekat dengan Naura dan Aryandita!"),
    )
    .addSubcommand((sub) =>
      sub
        .setName("help")
        .setDescription("Buka panduan perintah interaktif Naura.")
        .addStringOption((opt) =>
          opt
            .setName("command")
            .setDescription("Cari nama command spesifik")
            .setRequired(false)
    ...
    .addSubcommand((sub) =>
      sub
        .setName("language")
        .setDescription("Ubah bahasa bot di server ini / Change bot language")
        .addStringOption((opt) =>
          opt
            .setName("lang")
  ...
  async autocomplete(interaction, client) {
    ...
    if (interaction.options.getSubcommand() === "help") {
      ...
      const allChoices = commandList.map((cmd) => ({
        name: `/${cmd}`,
        value: cmd,
      }));
  ...
  async executePrefix(message, args, client) {
    ...
    let subcommand =
      cmdName === "core" ? (args[0] ? args[0].toLowerCase() : null) : cmdName;
...
async function handlePing(interaction, client, lang) {
  ...
  try {
    ...
    /* Lavalink belum siap; status Offline sudah memadai */
  }
  ...
  try {
    ...
    const fetchRes = await fetch("https://api.verba.ink/", {
      method: "HEAD",
  ...
  try {
    ...
    const saweriaRes = await fetch("https://saweria.co", {
      method: "HEAD",
      signal: AbortSignal.timeout(3000),
    }).catch(() => null);
    if (saweriaRes)
      saweriaPing = `${online} \`${Date.now() - saweriaStart}ms\``;
    else saweriaPing = `${offline} Timeout`;
  } catch (err) {
    saweriaPing = `${offline} Error`;
  }

  let trakteerPing = `${offline} Offline`;
  try {
    const trakteerStart = Date.now();
    const trakteerRes = await fetch("https://trakteer.id", {
      method: "HEAD",
      signal: AbortSignal.timeout(3000),
    }).catch(() => null);
    if (trakteerRes)
      trakteerPing = `${online} \`${Date.now() - trakteerStart}ms\``;
    else trakteerPing = `${offline} Timeout`;
  } catch (err) {
    trakteerPing = `${offline} Error`;
  }

  let topggPing = `${offline} Offline`;
  try {
    const topggStart = Date.now();
    const topggRes = await fetch("https://top.gg", {
      method: "HEAD",
      signal: AbortSignal.timeout(3000),
    }).catch(() => null);
  ...
  const payload = buildContainerV2({
    ...
    description:
      lang.PING_DESC ||
      "Real-time monitoring of Naura Hoshino's internal network and API connections.",
    fields: [
      {
        name: `${eCoreSystem} Naura Core Systems`,
      ...
      },
      {
        ...
        value: `${dot} **Google Gemini:** ${geminiPing}\n${dot} **Verba API:** ${verbaPing}`,
      },
      {
        name: `${eThirdPartySystem} Third-Party & Integrations`,
        value: `${dot} **Top.gg Portal:** ${topggPing}\n${dot} **Saweria Gateway:** ${saweriaPing}\n${dot} **Trakteer Support:** ${trakteerPing}`,
      },
      {
        name: `${eAudioSystem} Naura Music & Audio Systems`,
        value: lavalinkStr,
      },
    ],
...
async function handleStats(interaction, client, lang) {
  ...
  const payload = buildContainerV2({
    ...
    fields: [
      ...
      {
        name: `${eRam} ${cleanRam}`,
        value: `${dot} **${lang.STATS_RAM_SERVER}:** \`${usedMem}GB / ${totalMem}GB\`\n${dot} **${lang.STATS_RAM_BOT}:** \`${botMem} MB\``,
      },
      {
  ...
  if (statsBanner) {
    payload.files = [
      new AttachmentBuilder(statsBanner, { name: "banner.png" }),
    ];
  }
...
async function handleInfo(interaction, client, lang) {
  ...
  if (interaction.guild) {
    ...
    const cleanTitle =
      lang.INFO_SERVER_TITLE.replace(/^\u2139\uFE0F?\s*/, "") || "Server Info";
    ...
    fields = [
      ...
      {
        name: `${eClock} Created At`,
        value: `<t:${Math.floor(guild.createdTimestamp / 1000)}:R>`,
      },
      {
        name: `${eUsers} Members [${guild.memberCount}]`,
        value: `Roles: ${roleCount} | Channels: ${channelCount}`,
      },
      {
        name: `${eBooster} Boosts`,
        value: `Level ${guild.premiumTier} (${guild.premiumSubscriptionCount} Boosts)`,
      },
    ];
  } else {
    const cleanTitle =
      lang.INFO_BOT_TITLE.replace(/^\u2139\uFE0F?\s*/, "") || "Bot Info";
  ...
  const payload = buildContainerV2({
    ...
    fields,
    bannerAttachmentName: petAttachment ? "pet.png" : null,
    buttonsRow: createNavButtons(),
    footerText: ui.getFooter("core"),
  });

  if (petAttachment) {
    payload.files = [petAttachment];
  }
...
async function handleHelp(interaction, client, langParam) {
  ...
  if (interaction.deferred || (interaction.editReply && !interaction.reply)) {
    ...
    langResponse = await interaction.reply({
      ...langPayload,
      fetchReply: true,
    });
  }
  ...
  try {
    const langInteraction = await langResponse.awaitMessageComponent({
      ...
      time: 60000,
      filter: (i) =>
        i.user.id === interaction.user.id && i.customId === "help_lang_select",
    });
    ...
    await langInteraction
      .reply({ content: confirmMsg, flags: MessageFlags.Ephemeral })
      .catch(() => {});
  } catch (err) {
...
async function renderHelpMenuV2(
  interaction,
  client,
  lang,
  ...
  try {
    ...
    try {
      ...
      /* pesan sudah tidak bisa disunting */
    }
  }
...
async function handleLanguage(interaction, guildId, newLang, currentLang) {
  ...
  if (interaction.reply && !interaction.deferred)
    return interaction.reply({ content: successMsg });

...
async function buildTranscript(channel, ticket, closer, reason, user) {
    ...
    const body = ordered
        .map(m => `[${new Date(m.createdTimestamp).toUTCString()}] ${m.author.tag}: ${m.content}`)
        .join('\n');
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'reply') {
            ...
            const replyPayload = buildContainerV2({
                ...
                footerText: isAnon ? 'Dibalas oleh: Staf Server' : `Dibalas oleh: ${interaction.user.tag}`
            });
        ...
        try {
            transcriptFile = await buildTranscript(interaction.channel, ticket, interaction.user, reason, user);
        ...
        const successPayload = buildContainerV2({
            ...
            title: 'Tiket Ditutup',
            expression: 'success',
            description: 'Tiketnya sudah Naura tutup. Channel ini akan dihapus sepuluh detik lagi ya!',
            footerText: ui.getFooter('core')
        });
        ...
        try {
            ...
            const logChannelId = settings && settings.settings && settings.settings.modmail
        ...
        setTimeout(() => interaction.channel.delete().catch(() => {}), DELETE_DELAY_MS);
    }
};

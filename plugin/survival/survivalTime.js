...
async function rollSickness(userId, rpgState) {
    ...
    const hasRaincoat = inventory.some(item => item && (item.id === 'raincoat' || item.id === 'rain_coat'));
...
async function advanceTime(userId, hoursAdded) {
    ...
    if (passedOut) {
        const clinicType = survival.currentLocation === 'kota' ? 'RS Kota' : 'Klinik Desa';
    ...
    const finalLocation = survival.currentLocation === 'prison' && advancedDays > 0
        ? HOME_LOCATION

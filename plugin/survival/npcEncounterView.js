...
async function greet(userId, npc) {
    ...
    if (waiting > 0) {
        return { cooled: false, wait: rules.formatCooldown(waiting), affection: row.affection || 0 };
    }
...
function rows({ canGreet, stepsLeft, cost }) {
    ...
    row.addComponents(
        new ButtonBuilder()
            ...
            .setLabel(cost > 0 ? `Jalan lagi (${stepsLeft}, -${cost} stamina)` : `Jalan lagi (${stepsLeft})`)
            .setEmoji(e('run', '\uD83C\uDFC3'))
...
async function runRoam({ interaction, location, hour, luck }) {
    ...
    const render = (target, extra) => {
        ...
        return {
            ...payload,
            components: [...payload.components, ...rows({ canGreet: result.found && !greeted, stepsLeft, cost })]
        };
    };
    ...
    const message = await interaction.followUp(render(null, intro)).catch(() => null);
    ...
    collector.on('collect', async (i) => {
        ...
        if (i.customId === 'roam_greet' && result.found && !greeted) {
            ...
            try {
                ...
                if (!bond.cooled) {
                    notes.push(`${e('sleepy', '\u23F3')} Kalian baru saja mengobrol. **${result.npc.name}** masih sibuk, coba sapa lagi sekitar **${bond.wait}** lagi.`);
                } else {
                    greeted = true;
                    notes.push(`${e('sparkle', '\u2728')} Kedekatanmu dengan **${result.npc.name}** naik jadi **${bond.affection}** \u2014 status **${bond.label}**.`);
                    ...
                    if (gift.kind === 'item') {
                        ...
                        if (hint) notes.push(`${e('read', '\uD83D\uDDE3\uFE0F')} *"${hint}"*`);
                    }
                }
            } catch (error) {
                notes.push(`${e('annoy', '\u26A0\uFE0F')} Sapaanmu tersampaikan, tapi catatannya gagal disimpan. Coba lagi nanti, ya.`);
            }
        ...
        if (i.customId === 'roam_again') {
            const spent = await rules.spendStamina(user.id, location).catch(() => ({ ok: true, cost: 0 }));

            if (!spent.ok) {
                stepsLeft = 0;
                return i.editReply(render(
                    null,

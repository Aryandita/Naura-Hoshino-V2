...
const SKILLS = {
    warrior: { name: 'Iron Slash', staminaCost: 15, multiplier: 1.8, kind: 'physical' },
    mage: { name: 'Fireball', staminaCost: 25, multiplier: 1.1, kind: 'magic' },
    assassin: { name: 'Shadow Strike', staminaCost: 20, multiplier: 2.2, kind: 'sneak' },
    ranger: { name: 'Piercing Arrow', staminaCost: 15, multiplier: 1.5, kind: 'ranged' }
};
...
function buildFighter(user, profile, survival) {
    const className = (survival.rpg_state && survival.rpg_state.class) || null;
    const bonus = bonusFor(className);
    const maxHp = maxHpFor(survival, bonus);

    const strength = (Number(survival.strength) || 1) + (Number(bonus.strength) || 0);
    const agility = (Number(survival.agility) || 1) + (Number(bonus.agility) || 0);
    const intelligence = (Number(survival.intelligence) || 1) + (Number(bonus.intelligence) || 0);
    ...
    return {
        ...
        intelligence,
        luck,
        weaponDmg: (Number(profile.weapon_level) || 1) * 10 + strength * 3
    };
}

function dodgeChance(target) {
    return Math.min(45, target.agility * 1.5);
}

function critChance(active) {
    return Math.min(50, active.luck * 1.5);
}
...
function useSkill(active, target) {
    ...
    let damage;
    if (skill.kind === 'magic') {
        damage = variance(active.intelligence * 4.5 + active.weaponDmg * skill.multiplier, 0.25);
    } else {
        damage = variance(active.weaponDmg * skill.multiplier, skill.kind === 'sneak' ? 0.4 : 0.2);
    }
...
function narrate(active, target, result) {
    if (result.skill) {
        if (result.dodged) return `${target.username} berhasil lolos dari ${result.skill.name} milik ${active.username}!`;
        if (result.missed) return `${result.skill.name} dari ${active.username} meleset, ${target.username} terlalu gesit!`;
        return `${active.username} melepas ${result.skill.name} dan melukai ${target.username} sebesar ${result.damage} HP!`;
    }

    if (result.dodged) return `${target.username} menghindar tepat waktu dari serangan ${active.username}!`;
    if (result.crit) return `KRITIKAL! ${active.username} menghantam ${target.username} sebesar ${result.damage} HP!`;
...
async function settle(p1Survival, p1State, p2Survival, p2State) {

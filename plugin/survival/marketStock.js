...
const ALLOWED_IDS = [
    ...
    'wooden_axe', 'wooden_pickaxe', 'fishing_rod'
];

const EXTRA_STOCK = {
    tools: [{ id: 'veh_bicycle', name: 'Sepeda Kayuh (Kendaraan)', price: 15000, description: 'Bebas ongkos jalan antarwilayah.' }],
    special: [{ id: 'prop_gudang', name: 'Gudang Tua (Properti)', price: 10000, description: 'Tempat berteduh sederhana, tapi cukup untuk tidur.' }]
};
...
function poolFor(category, diffConfig = {}) {
    const base = items.filter(it => it && it.category === category && ALLOWED_IDS.includes(it.id) && Number(it.price) > 0);
...
function decode(value) {
    ...
    return { itemId, value: parseInt(valueStr, 10), amount: parseInt(amountStr, 10) || 0 };
}
...
async function sell(userId, itemId, sellAll) {
    ...
    const nextInv = left > 0
        ? addOrStackItem(kept, { id: itemId, name: item.name, amount: left })
        : kept;

    await cacheManager.updateUserProfile(userId, { inventory: nextInv });
    const balance = await currencyHelper.reward(FRAGMENT, { survival, profile }, earned);

    return { ok: true, itemName: item.name, qty, unitPrice, earned, left, balance, multiplier };
}
...
async function buy(userId, itemId, price) {
    ...
    if (balanceBefore < price) {
        return { ok: false, reason: 'insufficient', shortage: price - balanceBefore };
    }

    const balance = await currencyHelper.charge(FRAGMENT, holders, price);
    if (balance === null) return { ok: false, reason: 'insufficient', shortage: price - balanceBefore };
    ...
    const nextInv = addOrStackItem(safeParseInventory(profile.inventory), { id: itemId, name: itemName, amount: 1 });
    await cacheManager.updateUserProfile(userId, { inventory: nextInv });

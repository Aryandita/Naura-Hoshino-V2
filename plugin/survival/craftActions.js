...
const { getSmeltRecipe, getUpgradePlan } = require('./craftingRecipes');
const { getBlueprint } = require('./craftBlueprints');
...
async function assemble({ userId, survival, profile, blueprintId }) {
    ...
    if (!helpers.takeAll(inventory, blueprint.req)) return { ok: false, reason: 'materials', check };
...
async function smelt({ userId, survival, profile, outputId }) {
    ...
    if (!helpers.takeAll(inventory, recipe.input)) return { ok: false, reason: 'materials', check };

    const balance = await currency.charge(recipe.currency, holders, recipe.fee);
    if (balance === null) return { ok: false, reason: 'money', need: recipe.fee, kind: recipe.currency, balance: 0 };
...
async function upgrade({ userId, survival, profile, fromId }) {
    ...
    const inventory = safeParseInventory(profile.inventory);
    if (helpers.countItem(inventory, fromId) < 1) return { ok: false, reason: 'no_tool' };
    ...
    if (!helpers.takeAll(inventory, plan.materials)) return { ok: false, reason: 'materials', check };
    if (!helpers.takeItem(inventory, fromId, 1)) return { ok: false, reason: 'no_tool' };

    const balance = await currency.charge(plan.currency, holders, plan.cost);
    if (balance === null) return { ok: false, reason: 'money', need: plan.cost, kind: plan.currency, balance: 0 };

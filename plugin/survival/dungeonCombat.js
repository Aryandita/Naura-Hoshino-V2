...
function bonusOf(userClass) {
    return CLASS_BONUS[userClass] || { maxHp: 0, strength: 0, agility: 0, intelligence: 0, luck: 0 };
}
...
function resolveAttack({ useSkill, stats, profile }) {
    ...
    if (!useSkill || !stats.skill) {
        ...
        return { damage, cost: 0, log: 'Kamu menebas musuh dan memberikan **' + damage + '** damage!' };
    }
    ...
    if (stats.userClass === 'warrior') {
        ...
        return { damage, cost, log: '[Iron Slash] Tebasanmu membelah perisai musuh, **' + damage + '** damage!' };
    }

    if (stats.userClass === 'mage') {
        const magic = (profile.weapon_level || 1) * 8 + stats.intelligence * 4;
        const damage = Math.floor(magic * 2.2 * roll);
        return { damage, cost, log: '[Fireball] Bola api raksasa meledak, **' + damage + '** damage!' };
    }

    if (stats.userClass === 'assassin') {
        if (Math.random() * 100 >= 70 + stats.agility) {
            return { damage: 0, cost, log: '[Shadow Strike] Serangan bayanganmu meleset. Sabar, ya!' };
        }
        ...
        return {
            ...
            cost,
            log: isCrit
                ? '[Shadow Strike - KRITIS!] Tebasan mematikan dari balik bayangan, **' + damage + '** damage!'
                : '[Shadow Strike] Tebasan cepat dari balik bayangan, **' + damage + '** damage!'
        };
    }

    if (Math.random() * 100 >= 85 + stats.agility * 2) {
        return { damage: 0, cost, log: '[Piercing Arrow] Musuhnya terlalu gesit, panahmu meleset!' };
    }

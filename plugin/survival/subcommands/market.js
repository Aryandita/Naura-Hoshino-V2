...
const { ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, AttachmentBuilder, MessageFlags } = require('discord.js');
const { buildContainerV2, buildErrorContainerV2 } = require('../../../src/utils/NauraContainerBuilder');
const UserSurvival = require('../../../src/models/UserSurvival');
const cacheManager = require('../../../src/managers/cacheManager');
const ui = require('../../../src/config/ui');
const market = require('../marketStock');
const currencyHelper = require('../currency');
const { SHOPS, say } = require('../shopkeepers');
const { getSeason, getWeather } = require('../survivalTime');
...
module.exports = {
    async execute(interaction) {
        ...
        if (survival.currentLocation === 'prison') return ui.sendError(interaction, 'err_sys_51', true);
        if (!VILLAGE_KEYS.includes(survival.currentLocation)) return ui.sendError(interaction, 'err_sys_52', true);
        ...
        const bannerFiles = banner ? [new AttachmentBuilder(banner, { name: BANNER_NAME })] : [];
        ...
        const walletNow = async () => {
            ...
            return {
                ...
                balance: currencyHelper.balanceOf(FRAGMENT, { survival: freshSurvival, profile: freshProfile })
            };
        };

        function card({ title, description, rows = [], expression = 'Happy', withBanner = false }) {
        ...
        const mainRow = () => new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId('market_buy_init').setLabel('Beli Barang').setStyle(ButtonStyle.Primary),
            new ButtonBuilder().setCustomId('market_sell_init').setLabel('Jual Hasil Panen').setStyle(ButtonStyle.Success)
        );

        const backRow = (target = 'market_back') => new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId(target).setLabel('Kembali').setStyle(ButtonStyle.Secondary)
        );
        ...
        const categoryRow = () => new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder()
                ...
                .addOptions(Object.entries(market.CATEGORIES).map(([value, meta]) => ({
                    label: meta.label,
                    value
                })))
        );
        ...
        const sellCard = async () => {
            const { survival: freshSurvival, profile: freshProfile, balance } = await walletNow();
            const sellable = market.sellableFrom(freshProfile.inventory, freshSurvival);

            if (sellable.length === 0) {
                return card({
                    title: `${e('naura_akward')} Tasmu masih kosong`,
                    description: `> *"Belum ada yang bisa dijual, {nama}. Cari dulu di hutan atau sawah, nanti Pak Damar tunggu di sini."*`.replace('{nama}', vars.nama),
                    rows: [mainRow()],
                    expression: 'Akward'
                });
            }

            const bonusNote = sellable[0].multiplier > 1
            ...
            const sellMenu = new StringSelectMenuBuilder()
                ...
                .addOptions(sellable.slice(0, MAX_OPTIONS).map(it => ({
                    label: `${it.name} (x${it.amount})`.substring(0, 100),
                    description: `${n(it.unitPrice)} ${FRAGMENT.short} per unit`.substring(0, 100),
        ...
        collector.on('collect', async i => {
            ...
            if (i.customId === 'market_category') {
                ...
                const buyMenu = new StringSelectMenuBuilder()
                    ...
                    .addOptions(pool.slice(0, MAX_OPTIONS).map(it => ({
                        ...
                        description: `${n(it.finalPrice)} ${FRAGMENT.short} \u2022 ${it.description || it.desc || 'Kebutuhan desa'}`.substring(0, 100),
                ...
                return i.editReply(card({
                    ...
                    rows: [new ActionRowBuilder().addComponents(buyMenu), categoryRow(), backRow('market_buy_init')]
                }));
            }

            if (i.customId === 'market_buy_pick') {
                const { itemId, value } = market.decode(i.values[0]);
                const result = await market.buy(user.id, itemId, value);

                if (!result.ok) {
                    const kurang = `${n(result.shortage || 0)} ${FRAGMENT.name}`;
                    return i.followUp(hidden(buildErrorContainerV2({
                ...
                await i.followUp(hidden(buildContainerV2({
            ...
            if (i.customId === 'market_sell_pick') {
                ...
                const confirmRow = new ActionRowBuilder().addComponents(
                    new ButtonBuilder().setCustomId(`market_sell_one|${itemId}`).setLabel('Jual 1').setStyle(ButtonStyle.Primary),
                    new ButtonBuilder().setCustomId(`market_sell_all|${itemId}`).setLabel('Jual semua').setStyle(ButtonStyle.Danger),
            ...
            if (i.customId.startsWith('market_sell_one|') || i.customId.startsWith('market_sell_all|')) {
                ...
                if (!result.ok) {
                    return i.followUp(hidden(buildErrorContainerV2({
                        ...
                        description: 'Barangnya sudah tidak ada di tasmu, atau memang tidak ada yang mau membelinya.',
                        footerText: ui.getFooter('survival')
                    })));
                }

                const bonusNote = result.multiplier > 1
                ...
                await i.followUp(hidden(buildContainerV2({
                    ...
                    description: [
                        ...
                        result.left > 0 ? `${e('shop_box')} Sisa di tas: **${result.left} unit**` : ''
                    ].filter(Boolean).join('\n'),
                    footerText: ui.getFooter('survival')
                })));

...
const EFFECT_FAIL = {
    no_bond: 'Kamu belum punya kedekatan sama siapa pun, jadi cincinnya belum ada yang bisa dipakaikan. Coba akrab dulu lewat `/survival npc` ya, Naura bantu doakan!',
    already_married: 'Kamu kan sudah menikah, sayang. Cincinnya Naura simpan balik ke tas, jangan sampai ada yang cemburu!',
    already_owned: 'Khasiat barang ini sudah menempel permanen di kamu, jadi nggak bisa dipakai dua kali. Naura simpan lagi ya biar nggak terbuang.',
    unknown_effect: 'Hmm, Naura belum tahu cara memakai barang ini. Nanti Naura tanya Bagas dulu, ya!'
};
    ...
    async execute(interaction) {
        ...
        if (keys.length === 0) {
            const empty = buildErrorContainerV2({
                ...
                description: 'Duh, nggak ada bekal sama sekali di tasmu. Yuk mancing, nambang, atau mampir ke warung Pak Damar dulu. Naura temani!',
        ...
        const selectMenu = new StringSelectMenuBuilder()
            ...
            .addOptions(keys.map(id => {
                ...
                return new StringSelectMenuOptionBuilder()
                    ...
                    .setDescription((it.effect ? 'Khasiat permanen' : it.description || 'Perbekalan').substring(0, 100))
        ...
        const menuPayload = buildContainerV2({
            ...
            expression: 'info',
            description: 'Ini isi tasmu yang Naura rapikan! Mau makan, minum, atau pakai barang spesial? Pilih saja di bawah, Naura siapkan.',
            footerText: ui.getFooter('survival')
        });
        ...
        collector.on('collect', async i => {
            ...
            if (conf.effect) {
                const result = await applyItemEffect(conf.effect, { survival, userId: user.id });

                if (!result.ok) {
                    const reject = buildErrorContainerV2({
                        title: `${e('akward', '\uD83D\uDE05')} Belum bisa dipakai`,
                        description: EFFECT_FAIL[result.reason] || EFFECT_FAIL.unknown_effect,
                        footerText: ui.getFooter('survival')
                    });
                    return i.editReply({ ...reject, embeds: [], components: reject.components });
                }
                ...
                const lines = [`Kamu memakai **${conf.name}**, dan khasiatnya menempel permanen!`, ''];

                if (conf.effect === 'max_romance') {
                    lines.push(`${e('blowkiss', '\uD83D\uDC8D')} Kamu resmi menikah dengan **${result.npcName}**! Naura ikut menangis bahagia, sungguh...`);
                } else if (conf.effect === 'luck_crown') {
                    lines.push(`${e('impressed', '\uD83D\uDC51')} LUCK kamu naik dari **${result.before}** jadi **${result.after}** (+${result.gained}) selamanya!`);
                } else {
                    lines.push(`${e('cheers', '\u2728')} **${result.label}** sudah aktif buat kamu, permanen tanpa kedaluwarsa.`);
                }
            ...
            const newHunger = Math.min(100, (survival.hunger || 0) + (effects.hunger || 0));
            const newThirst = Math.min(100, (survival.thirst || 0) + (effects.thirst || 0));
            const newStamina = Math.min(100, (survival.stamina || 0) + (effects.stamina || 0));
            ...
            if (effects.hp) {
                lines.push(`> ${e('cheers', '\u2764\uFE0F')} HP: pulih **+${effects.hp}**`);
            }

            lines.push('', `${e('impressed', '\uD83C\uDF1F')} Naura kasih bonus **+1 XP**. Semangat terus, ya!`);
        ...
        collector.on('end', collected => {
            ...
            const timeout = buildErrorContainerV2({
                ...
                description: 'Naura sudah tunggu satu menit, tapi belum ada yang dipilih. Nggak apa-apa, panggil Naura lagi kalau perut kamu sudah keroncongan!',
                footerText: ui.getFooter('survival')
            });

...
module.exports = {
    async execute(interaction) {
        ...
        await interaction.reply({ ...payload, components: [...payload.components, row] });
        ...
        collector.on('collect', async i => {
            ...
            try {
                const buffer = await generateAchievementImage(user, selected.title, selected.description, selected.color);
        ...
        collector.on('end', async (collected, reason) => {
            ...
            const timeout = buildErrorContainerV2({
                ...
                description: 'Naura tunggu agak lama, tapi belum ada gelar yang dipilih. Nggak apa-apa, panggil Naura lagi kapan saja yaa!',
                footerText: ui.getFooter('survival')
            });

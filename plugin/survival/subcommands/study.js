...
module.exports = {
    async execute(interaction) {
        ...
        if ((survival.hunger || 0) <= 15 || (survival.thirst || 0) <= 15) return ui.sendError(interaction, 'err_sys_65', true);
        ...
        collector.on('collect', async i => {
            if (i.customId === 'study_book') {
                ...
                if (Array.isArray(decos) && decos.some(d => d && d.id === 'deco_bookshelf')) intGain += 1;
            ...
            if (hasIjazah) {
                const already = buildErrorContainerV2({
                    ...
                    description: '**Prof. Habibie:** "Nilaimu sudah memuaskan, tidak perlu mengulang ujian lagi." Naura ikut bangga, lho!',
                    footerText: ui.getFooter('survival')
                });
            ...
            if ((rpgState.test_cd || 0) > (survival.inGameDay || 1)) {
                const paid = await currency.charge(currency.COIN, { survival, profile }, REMEDIAL_COST);
                ...
                if (paid === null) {
                    ...
                    return i.reply(ephemeral({ ...cdPayload, embeds: [] })).catch(() => {});
                }
            }
            ...
            if (Math.random() < passChance) {
                inventory.push({ id: 'certificate', name: 'Ijazah Kelulusan', amount: 1, type: 'special' });

...
function relationshipLabel(level) {
    const index = Math.max(0, Math.min(RELATIONSHIP_NAMES.length - 1, Number(level) || 0));
    ...
    async execute(interaction, client) {
        ...
        const selectRow = () => new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder()
                ...
                .addOptions(met.slice(0, MAX_OPTIONS).map(bond => {
                    const npc = npcs[bond.npcId];
                    const mark = album.includes(bond.npcId) ? '\u2B50 ' : '';
                    return {
                        label: `${mark}${npc.name}`.substring(0, 100),
                        value: bond.npcId,
                        description: `${RELATIONSHIP_NAMES[Math.min(4, bond.relationshipLevel || 0)]} \u2022 afeksi ${bond.affection || 0}`.substring(0, 100)
                    };
                }))
        );
        ...
        const photoButtons = bond => new ActionRowBuilder().addComponents(
            new ButtonBuilder()
                .setCustomId('gallery_save')
                .setLabel(album.includes(bond.npcId) ? 'Hapus dari favorit' : 'Simpan ke favorit')
                .setStyle(album.includes(bond.npcId) ? ButtonStyle.Secondary : ButtonStyle.Success),
            new ButtonBuilder()
        ...
        collector.on('collect', async i => {
            ...
            if (i.customId === 'gallery_pick') {
                ...
                return i.editReply({ ...payload, components: [...payload.components, photoButtons(opened)] });
            }

            if (i.customId === 'gallery_back' || !opened) {
                const backPayload = buildIndex();
                return i.editReply({ ...backPayload, components: [...backPayload.components, selectRow()] });
            }
            ...
            if (i.customId === 'gallery_save') {
                ...
                survival.rpg_state = { ...(survival.rpg_state || {}), gallery_album: album };
            ...
            if (i.customId === 'gallery_download') {
                ...
                return i.followUp({
                    ...
                    files: [new AttachmentBuilder(portrait, { name: `naura-${npc.id}${path.extname(portrait)}` })],
                    flags: MessageFlags.Ephemeral
                });
            }
        });

...
const actions = require('../bankActions');
const views = require('../bankViews');
const { COLLECTOR_MS, ANSWER_MS, MIN_EXCHANGE_NSF, MIN_DEPOSIT_COIN, MIN_INVEST_COIN, assetOf } = require('../bankConfig');

const CITY_KEYS = ['kota', 'city'];

const FAIL_MESSAGE = {
    invalid_amount: 'Nominalnya belum terbaca. Tulis angkanya saja ya, misalnya `1500`.',
...
function failText(result) {
    const base = FAIL_MESSAGE[result.reason] || 'Ada yang kurang pas dengan transaksinya.';
...
module.exports = {
    async execute(interaction) {
        ...
        const snapshotNow = async () => actions.snapshot(await actions.freshData(user.id));
        ...
        const response = await interaction.editReply(views.mainView(user, await snapshotNow()));
        ...
        const askAmount = async (i, { title, description, run }) => {
            ...
            chat.on('collect', async m => {
                try {
                    ...
                    await m.reply(result.ok ? views.successView(result.title, result.description) : views.failView(failText(result)));
                } catch (err) {
                    logger.error('[BANK TRANSACTION]', err);
                    await m.reply(views.failView('Ada yang tersangkut di sistem bank. Naura sudah catat, coba lagi sebentar lagi ya.')).catch(() => {});
                } finally {
            ...
            chat.on('end', async collected => {
                if (collected.size === 0) {
                    await interaction.followUp({
                        ...views.failView('Waktunya habis, jadi transaksinya Naura batalkan dulu. Tidak ada saldo yang berkurang, kok!'),
        ...
        collector.on('collect', async i => {
            try {
                ...
                if (id === 'bank_menu_invest') {
                    ...
                    const { payload, rows } = views.investView(snap, actions.valuate(snap.investments, snap.day));
                    return i.editReply({ ...payload, components: [...payload.components, ...rows] });
                }

                // --- Penukaran mata uang ---
                if (id === 'bank_ex_to_coin' || id === 'bank_ex_to_nsf') {
                    const toCoin = id === 'bank_ex_to_coin';
                    const fromKind = toCoin ? currencyHelper.FRAGMENT : currencyHelper.COIN;
                    ...
                    return askAmount(i, {
                        ...
                        run: async (raw) => {
                            const result = await actions.exchangeMoney(user.id, fromKind, toKind, raw);
                            if (!result.ok) return result;

                            const sisa = result.remainder > 0
                ...
                if (id === 'bank_dep_claim') {
                    ...
                    await i.followUp({
                        ...views.successView('Deposito dicairkan!', [
                ...
                if (id.startsWith('bank_inv_buy_')) {
                    ...
                    return askAmount(i, {
                        ...
                        run: async (raw) => {
                            const result = await actions.buyInvestment(user.id, assetKey, raw);
                            if (!result.ok) return result;
                            return {
                                ok: true,
                ...
                if (id.startsWith('bank_inv_sell_')) {
                    ...
                    await i.followUp({
                        ...views.successView('Portofolio dicairkan!', [

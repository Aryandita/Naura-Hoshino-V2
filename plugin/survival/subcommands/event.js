...
module.exports = {
  async execute(interaction) {
    ...
    const mutation = await cacheManager.mutateUserSurvivalJson(user.id, "rpg_state", (state) => {
      if (!state) state = {};
    ...
    if (!mutation.ok) {
      ...
      return interaction.editReply({ ...alreadyClaimedPayload, embeds: [] }).catch(() => {});
    }
    ...
    if (season.name === "new_year") {
      ...
      await cacheManager.incrementUserSurvival(user.id, "coupons", randomCoupon);
      lines.push(`> ${e("coupon", "🎫")} **${randomCoupon}x Naura Coupon**`);
      lines.push("", `${e("happy", "✨")} Selamat Tahun Baru! Semoga petualanganmu tahun ini semakin seru!`);
    } 
    ...
    return interaction.editReply({ ...successPayload, embeds: [] }).catch(() => {});
  }
};

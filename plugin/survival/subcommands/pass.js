    ...
    async execute(interaction) {
        ...
        if (action === 'buy') {
            ...
            const payload = buildContainerV2({
                ...
                title: upgradeRes.success ? '✨ Premium Pass Terbuka!' : '⚠️ Gagal Upgrade Pass',
        ...
        if (action === 'claim') {
            ...
            if (claimRes.totalGold > 0) rewardsSummary.push(`> 🪙 **+${claimRes.totalGold.toLocaleString('id-ID')} Gold**`);
            if (claimRes.totalStarFragments > 0) rewardsSummary.push(`> ⭐ **+${claimRes.totalStarFragments.toLocaleString('id-ID')} Star Fragments**`);
            if (claimRes.totalCoupons > 0) rewardsSummary.push(`> 🎟️ **+${claimRes.totalCoupons} Naura Coupon**`);
            if (claimRes.unlockedLabels && claimRes.unlockedLabels.length > 0) {
                rewardsSummary.push(...claimRes.unlockedLabels.map((l) => `> ✨ **${l}**`));
            }
        ...
        const milestones = [currentTier, currentTier + 1, currentTier + 2].filter((t) => t <= season.maxTier);
        for (const t of milestones) {
            const r = seasonEngine.TIER_REWARDS[t];
            if (r) {
                const freeDesc = r.free ? Object.entries(r.free).map(([k, v]) => `${v} ${k}`).join(', ') : 'Tidak ada';
                const premDesc = r.premium ? Object.entries(r.premium).map(([k, v]) => `${v} ${k}`).join(', ') : 'Tidak ada';
                lines.push(`> 🎯 **Tier ${t}:** Free: \`${freeDesc}\` | Premium: \`${premDesc}\``);
            }
        }

        if (!isPremium) {
            lines.push('', '💡 *Upgrade ke Premium Pass seharga 5 Naura Coupon untuk membuka jalur hadiah premium 2x lipat!*');
        }

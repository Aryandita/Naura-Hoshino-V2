...
const { rollCouponDrop } = require('../couponRewards');
...
const DIFFICULTIES = [
    { value: 'Mudah', label: 'Rebirth Mode MUDAH', emojiKey: 'diff_easy', fallback: '\uD83D\uDFE2', perk: 'EXP +15%, NSF +5%, penurunan stamina -10%' },
    { value: 'Normal', label: 'Rebirth Mode NORMAL', emojiKey: 'diff_normal', fallback: '\uD83D\uDFE1', perk: 'EXP +10%, NSF +3%, penurunan stamina -5%' },
    { value: 'Sulit', label: 'Rebirth Mode SULIT', emojiKey: 'diff_hard', fallback: '\uD83D\uDFE0', perk: 'EXP +5%, NSF +2,5%, penurunan stamina -3%' },
    { value: 'Ekstrim', label: 'Rebirth Mode EKSTRIM', emojiKey: 'diff_extreme', fallback: '\uD83D\uDD34', perk: 'musuh 100% lebih kuat, harga toko melonjak, EXP & NSF -20%. Tamatkan untuk gelar **The Strongest & Smartest Survivor**' }
];
    ...
    async execute(interaction) {
        ...
        const introPayload = buildContainerV2({
            ...
            description: [
                ...
                ...DIFFICULTIES.map(d => `> ${e(d.emojiKey, d.fallback)} **${d.value}:** ${d.perk}`)
            ].join('\n'),
        ...
        collector.on('collect', async i => {
            ...
            const carriedState = {
                ...
                beat_extreme: previous.beat_extreme || previous.difficulty === 'Ekstrim',

...
const { BANNERS, getBannerPool } = require('../gachaBanners');
...
module.exports = {
    async execute(interaction) {
        ...
        if (!bannerData) {
            return ui.sendError(interaction, 'Banner gacha tersebut tidak ditemukan.', true);
        }
        ...
        if (banner.currency === 'coin') {
            debitSuccess = await cacheManager.debitUserProfile(userId, 'economy_wallet', banner.cost);
        } else if (banner.currency === 'coupon') {
            debitSuccess = await cacheManager.debitUserSurvival(userId, 'coupons', banner.cost);
        }

        if (!debitSuccess) {
            return ui.sendError(interaction, `Kamu membutuhkan **${banner.cost} ${banner.currency === 'coin' ? 'Naura Coin' : 'Naura Coupon'}** untuk roll di banner ini!`, true);
        }

        // Ambil data user survival untuk memanipulasi pity counter
        const mutateRes = await cacheManager.mutateUserSurvivalJson(userId, 'rpg_state', (raw) => {
        ...
        if (!mutateRes.ok) {
            return ui.sendError(interaction, 'Terjadi kesalahan sistem saat memproses pity gacha. Koin/Kupon mu tetap aman.', true);
        }
        ...
        if (currentPity >= banner.pityMax) {
            ...
            rarityRolled = bannerId === 'premium' ? 'Mythic' : (Math.random() > 0.8 ? 'Legendary' : 'Epic');
        ...
        if (!possibleItems || possibleItems.length === 0) {
            return ui.sendError(interaction, `Sistem gagal menemukan item dengan kelangkaan ${rarityRolled}.`, true);
        }

        // Ambil 1 barang acak dari pool tersebut
        const itemWon = possibleItems[Math.floor(Math.random() * possibleItems.length)];

        // Masukkan ke inventory secara atomik
        const addResult = await addItemsAtomic(userId, [{ id: itemWon.id, name: itemWon.name, amount: 1 }]);
        if (!addResult.ok) {
            return ui.sendError(interaction, 'Gagal memasukkan hadiah ke inventory. Hubungi admin.', true);
        }
        ...
        if (rarityRolled === 'Langka') { color = '#3B82F6'; starEmoji = '🌟'; }
        if (rarityRolled === 'Epic') { color = '#A855F7'; starEmoji = '✨'; }
        if (rarityRolled === 'Legendary') { color = '#F59E0B'; starEmoji = '🔥'; }
        if (rarityRolled === 'Mythic') { color = '#EF4444'; starEmoji = '💎'; }

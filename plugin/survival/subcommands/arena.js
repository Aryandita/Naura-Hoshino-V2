...
module.exports = {
  async execute(interaction) {
    ...
    if (menu === "coliseum_match") {
      ...
      const { drawColiseumMatch } = require("../../../src/canvas/coliseumCanvas");
      const { AttachmentBuilder } = require("discord.js");

      const user = interaction.user;
      const battleRes = await coliseumEngine.simulate3v3Battle(user.id, user.displayName || user.username);
      const files = [];
      ...
      try {
        const matchBuf = await drawColiseumMatch({
          ...
          eloChange: battleRes.eloChange > 0 ? `+${battleRes.eloChange}` : `${battleRes.eloChange}`,
          elo: battleRes.newElo,
          division: battleRes.newDivision,
        });
        files.push(new AttachmentBuilder(matchBuf, { name: "coliseum_match.png" }));
      } catch (err) {
      ...
      const payload = buildContainerV2({
        ...
        title: battleRes.isVictory ? `🎉 KEMENANGAN ARENA (${battleRes.score})!` : `☠️ KEKALAHAN ARENA (${battleRes.score})!`,
        description: [
          ...
          battleRes.rewardCoins > 0 ? `💰 **Hadiah Kemenangan:** \`+${battleRes.rewardCoins} Star Fragments\`` : "",
        ].filter(Boolean).join("\n"),
        footerText: ui.getFooter("survival"),
      });
    ...
    if (menu === "coliseum_team") {
      const coliseumEngine = require("../../../src/survival/engines/coliseumEngine");
      const user = interaction.user;
      const team = await coliseumEngine.registerOrGetTeam(user.id, `${user.username}'s Vanguard`);

      const fighters = Array.isArray(team.formation) ? team.formation : JSON.parse(team.formation);
      const fighterList = fighters
        .map((f, idx) => `**Slot ${idx + 1}:** **${f.name}** (\`${f.type}\`), ⚔️ ATK: ${f.atk} | 🛡️ DEF: ${f.def} | ❤️ HP: ${f.hp}`)
        .join("\n");
    ...
    if (menu === "coliseum_leaderboard") {
      ...
      const payload = buildContainerV2({
        ...
        description: lbList || "*Belum ada tim yang bertanding di Coliseum musim ini.*",
        footerText: ui.getFooter("survival"),
      });

...
module.exports = {
  async execute(interaction) {
    ...
    if (!targetUser) {
      return reject(
        interaction,
        "Naura belum tahu mau kirim ke siapa. Coba tulis begini ya: `/survival trade user:@teman nsf:100`.",
      );
    }
    ...
    if (targetUser.bot || targetUser.id === sender.id) {
      return reject(
        interaction,
        "Hehe, kamu nggak bisa bertransaksi sama bot atau sama diri sendiri. Pilih teman yang lain ya!",
      );
    }
    ...
    if (amount < MIN_AMOUNT) {
      return reject(
        interaction,
        "Jumlah Star Fragment yang mau dikirim harus lebih dari nol. Naura tunggu angkanya!",
      );
    }

    const [senderSurvival] = await UserSurvival.findOrCreate({
      where: { userId: sender.id },
    });
    const [targetSurvival] = await UserSurvival.findOrCreate({
      where: { userId: targetUser.id },
    });

    const senderBalance = currency.balanceOf(currency.FRAGMENT, {
      survival: senderSurvival,
    });
    ...
    collector.on("collect", async (i) => {
      ...
      if (remaining === null) {
        const failPayload = buildErrorContainerV2({
          ...
          description:
            "Aduh, saldo pengirim sudah tidak mencukupi saat transaksi diproses. Tidak ada yang berpindah kok, aman.",
          footerText: ui.getFooter("survival"),
        });
      ...
      const received = await currency.reward(
        currency.FRAGMENT,
        { survival: targetSurvival },
        amount,
      );
    ...
    collector.on("end", async () => {
      ...
      const expired = buildErrorContainerV2({
        ...
        description:
          "Waktunya habis dan belum ada tanggapan, jadi Naura batalkan penawarannya. Saldomu utuh, nggak ada yang berkurang!",
        footerText: ui.getFooter("survival"),
      });

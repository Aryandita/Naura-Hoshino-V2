...
module.exports = {
    async execute(interaction) {
        ...
        if (!PARK_LOCATIONS.includes(survival.currentLocation)) return ui.sendError(interaction, 'err_sys_40', true);

        const profile = await cacheManager.getUserProfile(user.id);
        const inventory = safeParseInventory(profile.inventory);
        const ticketIndex = inventory.findIndex(item => item && TICKET_IDS.includes(item.id));

        if (ticketIndex === -1) return ui.sendError(interaction, 'err_sys_41', true);

        const romanticNPCs = Object.values(npcConfig).filter(n => n && n.type === 'romansa').slice(0, MAX_OPTIONS);

        if (romanticNPCs.length === 0) {
            return ui.sendError(interaction, 'Duh, belum ada siapa pun di taman yang bisa kamu ajak kencan hari ini. Naura temani jalan-jalan saja, ya!', true);
        }
        ...
        const selectMenu = new StringSelectMenuBuilder()
            ...
            .addOptions(romanticNPCs.map(n => {
                const data = npcMap.get(n.id) || { affection: 0, relationshipLevel: 0 };
                const label = RELATIONSHIP_NAMES[data.relationshipLevel || 0] || 'Kenalan';

                return {
                    label: String(n.name).substring(0, 100),
                    description: `${n.title || 'Penduduk'} \u2022 Afeksi ${data.affection || 0}/100 (${label})`.substring(0, 100),
                    value: n.id
                };
            }));
        ...
        collector.on('collect', async i => {
            ...
            if ((npcData.affection || 0) < MIN_AFFECTION) {
                ...
                handled = true;
                await i.followUp({ ...reject, flags: (reject.flags || 0) | MessageFlags.Ephemeral }).catch(() => {});
                return;
            }
            ...
            npcData.affection = Math.min(100, (npcData.affection || 0) + BONUS_AFFECTION);
            ...
            if (portrait) {
                ...
                files.push(new AttachmentBuilder(portrait.full, { name: bannerAttachmentName }));
            }

            const levelName = RELATIONSHIP_NAMES[npcData.relationshipLevel || 0] || 'Kenalan';
        ...
        collector.on('end', async () => {
            ...
            const timeout = buildContainerV2({
                ...
                iconURL: user.displayAvatarURL(),
                expression: 'fail',
                description: 'Tamannya mulai sepi dan kamu belum memilih siapa pun. Tiketmu Naura simpan utuh kok, jadi bisa dipakai lain waktu!',
                footerText: ui.getFooter('survival')
            });

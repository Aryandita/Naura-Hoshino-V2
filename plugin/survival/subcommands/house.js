...
module.exports = {
  async execute(interaction) {
    ...
    if (survival.currentLocation === "prison")
      return ui.sendError(interaction, "err_sys_49", true);

    const propId = survival.propertyId || "jalanan";
    if (propId === "jalanan")
      return ui.sendError(interaction, "err_sys_50", true);

    const rpgState = survival.rpg_state || {};
    if (!Array.isArray(rpgState.active_decorations))
      rpgState.active_decorations = [];
    ...
    const terpasang =
      rpgState.active_decorations.length > 0
        ? rpgState.active_decorations
    ...
    if (
      ...
      selectMenu.addOptions([
        { label: "Belum ada dekorasi sama sekali", value: "none" },
      ]);
      ...
      rpgState.active_decorations.forEach((deco) => {
        selectMenu.addOptions([
          {
            label: `[LEPAS] ${deco.name}`.substring(0, 100),
            value: `unequip_${deco.id}`,
          },
        ]);
      });
      ...
      availableDecos.forEach((deco) => {
        if (!equippedIds.includes(deco.id)) {
          selectMenu.addOptions([
            {
              label: `[PASANG] ${deco.name}`.substring(0, 100),
              value: `equip_${deco.id}`,
            },
          ]);
        }
      });
    ...
    collector.on("collect", async (i) => {
      ...
      if (isEquip) {
        ...
        if (!decoItem) {
          return i.followUp(
            ephemeral(
              `${e("akward", "\u274C")} Naura nggak menemukan dekorasi itu di tasmu.`,
            ),
          );
        }

        rpgState.active_decorations.push({
          id: decoItem.id,
          name: decoItem.name,
        });
      ...
      const index = rpgState.active_decorations.findIndex(
        (d) => d.id === decoId,
      );
      if (index === -1) {
        return i.followUp(
          ephemeral(
            `${e("akward", "\u274C")} Dekorasi itu memang belum terpasang, kok.`,
          ),
        );
      }

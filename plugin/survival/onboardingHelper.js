...
async function executeOnboarding(interaction, client) {
    ...
    const pages = [
        {
            ...
            imgPath: path.join(__dirname, '..', '..', 'assets', 'dashboard', 'naura.png')
        },
        {
        ...
        },
        {
            ...
            imgPath: path.join(__dirname, '..', '..', 'assets', 'survival', 'characters', 'suster_maya.jpeg')
        },
        {
    ...
    const response = await interaction.reply({ ...firstContainer, components: [row], fetchReply: true });
    ...
    collector.on('collect', async i => {
        ...
        if (i.customId === 'tut_next') {
            ...
            const finishPayload = buildContainerV2({
                ...
                description: 'Kamu sekarang resmi menjadi penduduk Desa Naura. Silakan ulangi perintah `/survival` kamu tadi untuk bermain!',

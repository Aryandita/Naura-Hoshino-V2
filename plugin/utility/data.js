...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('data')
        .setDescription('🛡️ [PRIVACY] Kelola data personal kamu yang disimpan oleh bot.')
        .addSubcommand(sub => 
            sub.setName('export')
            .setDescription('📥 Ekspor seluruh data profil & survival kamu dalam bentuk JSON.')
        )
        .addSubcommand(sub => 
            sub.setName('delete')
            .setDescription('🗑️ Hapus SELURUH data personal kamu dari database (TIDAK BISA KEMBALI).')
        ),
    ...
    async execute(interaction) {
        ...
        if (sub === 'export') {
            ...
            const buffer = Buffer.from(JSON.stringify(exportedData, null, 2), 'utf-8');
            const attachment = new AttachmentBuilder(buffer, { name: `naura_data_${userId}.json` });

            const payload = buildContainerV2({
                accentColorHex: ui.getColor('success'),
                authorName: 'Naura Privacy Center',
                title: '📥 Data Ekspor Selesai',
                description: 'Seluruh data profil dan RPG/Survival kamu berhasil dikumpulkan. Silakan unduh file `.json` yang terlampir pada pesan ini.\n\n*Data ini hanya dikirimkan kepada kamu secara privat (ephemeral).*',
        ...
        if (sub === 'delete') {
            ...
            const payload = buildContainerV2({
                ...
                description: 'Kamu yakin ingin menghapus **SELURUH** datamu?\n\nIni akan menghapus secara permanen:\n- Seluruh level, balance (koin/kupon)\n- Inventaris RPG, pet, farm\n- Pengaturan profil & history premium\n\n**Tindakan ini tidak bisa dibatalkan!**',
                buttonsRow: row,
                footerText: ui.getFooter('core')
            });

            const reply = await interaction.reply({ ...payload, flags: MessageFlags.Ephemeral, fetchReply: true });
            ...
            collector.on('collect', async i => {
                ...
                if (i.customId === 'btn_data_confirm_delete') {
                    ...
                    await i.update(buildContainerV2({
                        ...
                        description: 'Seluruh datamu telah dihapus secara permanen dari database Naura.',
                        footerText: ui.getFooter('core')
                    }));
                } else {
                    await i.update(buildContainerV2({

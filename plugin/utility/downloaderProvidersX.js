...
const tryFxTwitter = async (url) => {
    ...
    try {
        ...
        const tweet = response.data && response.data.code === 200 ? response.data.tweet : null;
        const media = tweet && tweet.media;
        if (!media) return null;

        let items = Array.isArray(media.all) ? media.all : [];
        if (items.length === 0) items = [...(media.videos || []), ...(media.photos || [])];
        if (items.length === 0) return null;

        if (items.length === 1) return { status: 'stream', url: items[0].url };
        return {
            status: 'picker',
            picker: items.map(item => ({ url: item.url, type: item.type === 'video' ? 'video' : 'photo' }))
        };
    } catch (e) {
...
const tryAgatz = async (url) => {
    ...
    else if (url.includes('facebook.com') || url.includes('fb.watch')) kind = 'facebook';
    else if (isTweet(url)) kind = 'twitter';
    if (!kind) return null;

    try {
        logger.info('[Downloader] Mencoba Agatz API...');
        const endpoint = 'https://api.agatz.xyz/api/' + kind + '?url=' + encodeURIComponent(url);
        ...
        if (Array.isArray(r)) {
            ...
            return links.length === 1
                ...
                : { status: 'picker', picker: links.map(link => ({ url: link, type: 'photo' })) };
        }

        if (typeof r === 'string') {
            return keepRealMedia([r], url).length > 0 ? { status: 'stream', url: r } : null;
        }

        if (typeof r === 'object') {
            const candidate = r.hd || r.sd || r.url
                || Object.values(r).find(v => typeof v === 'string' && v.startsWith('http'));
...
const tryPubler = async (url) => {
    try {
        ...
        const payload = response.data && Array.isArray(response.data.payload) ? response.data.payload : [];
        const items = payload
        ...
        return {
            status: 'picker',
            picker: items.map(item => ({ url: item.path, type: item.type === 'video' ? 'video' : 'photo' }))
        };
    } catch (e) {

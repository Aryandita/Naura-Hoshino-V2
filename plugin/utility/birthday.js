  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addIntegerOption((opt) =>
          opt
            .setName("tanggal")
            .setDescription("Tanggal lahir (1-31)")
            .setRequired(true)
            .setMinValue(1)
            .setMaxValue(31),
        )
        .addIntegerOption((opt) =>
          opt
            .setName("bulan")
            .setDescription("Bulan lahir (1-12)")
            .setRequired(true)
            .setMinValue(1)
            .setMaxValue(12),
        )
        .addIntegerOption((opt) =>
          opt
            .setName("tahun")
            .setDescription("Tahun lahir (Opsional)")
            .setRequired(false)
            .setMinValue(1900)
    ...
    .addSubcommand((sub) =>
      sub
        .setName("upcoming")
        .setDescription(
          "Lihat siapa saja yang akan berulang tahun di server ini dalam waktu dekat.",
        ),
    ),
    ...
    if (subcommand === "set") {
      ...
      if (day > daysInMonth[month - 1]) {
        return interaction.reply({
          content: `${ui.getEmoji("cross") || "❌"} Tanggal tidak valid! Bulan ${month} hanya memiliki ${daysInMonth[month - 1]} hari.`,
      ...
      await interaction.reply({
        content: `${ui.getEmoji("success") || "✅"} Berhasil mencatat ulang tahunmu pada **${day} ${monthNames[month - 1]}${year ? ` ${year}` : ""}**!`,
      });
      ...
      if (!bday) {
        return interaction.reply({
          content: `${ui.getEmoji("cross") || "❌"} **${user.username}** belum mengatur tanggal ulang tahunnya.`,
          flags: MessageFlags.Ephemeral,
        });
      }
      ...
      if (bday.year) {
        const age =
          today.getFullYear() -
          bday.year -
          (today < new Date(today.getFullYear(), bday.month - 1, bday.day)
            ? 1
            : 0);
      ...
      if (birthdays.length === 0) {
        return interaction.editReply(
          `${ui.getEmoji("cross") || "❌"} Belum ada member di server ini yang mendaftarkan ulang tahun mereka.`,
        );
      }

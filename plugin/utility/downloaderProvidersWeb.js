...
const tryPinterestScrape = async (url) => {
    ...
    try {
        ...
        if (videoMatch) {
            ...
            if (keepRealMedia([link], url).length > 0) return { status: 'stream', url: link };
        }
...
const tryThreadsEmbed = async (url) => {
    ...
    try {
        ...
        const html = await fetchHtml('https://www.threads.net' + pathname + '/embed');
        if (!html) return null;

        const videos = [...html.matchAll(/<video[^>]*src="([^"]+)"/g)].map(m => unescapeUrl(m[1]));
        const images = [...html.matchAll(/<img[^>]*class="[^"]*BarcelonaImage[^"]*"[^>]*src="([^"]+)"/g)]
            .map(m => unescapeUrl(m[1]));

        const items = [
            ...keepRealMedia(videos, url).map(link => ({ url: link, type: 'video' })),
            ...keepRealMedia(images, url).map(link => ({ url: link, type: 'photo' }))
        ];
...
const tryLinkedInEmbed = async (url) => {
    ...
    try {
        ...
        const html = await fetchHtml(embedUrl, { Referer: 'https://www.linkedin.com/' });
        if (!html) return null;
...
module.exports = { unescapeUrl, tryPinterestScrape, tryRedditJSON, tryThreadsEmbed, tryLinkedInEmbed };

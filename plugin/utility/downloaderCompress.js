...
try {
    if (activeFfmpegPath) ffmpeg.setFfmpegPath(activeFfmpegPath);
} catch {
...
const compressWithFFmpeg = (inputPath, targetSizeMB = 23, forcedHeight = null, cleanup = null) => {
    return new Promise((resolve, reject) => {
        ...
        ffmpeg.ffprobe(inputPath, (err, metadata) => {
            ...
            logger.info(`[Downloader] Two-pass: durasi=${duration.toFixed(1)}s, video=${videoBitrate}k, audio=${audioBitrate}k${forcedHeight ? `, tinggi=${forcedHeight}p` : ''}`);
            ...
            const finishPasslog = () => {
                ...
                for (const ext of ['.log', '.log.mbtree']) {
                    try {
                        if (fs.existsSync(`${passlogPrefix}${ext}`)) fs.unlinkSync(`${passlogPrefix}${ext}`);
                    } catch { /* abaikan */ }
                }
            };
            ...
            const runPassTwo = () => {
                ffmpeg(inputPath)
                    ...
                    .on('progress', (progress) => {
                        if (progress.percent) {
                            logger.info(`[Downloader] Kompresi lintasan 2: ${progress.percent.toFixed(1)}%`);
                        }
                    })
                    .on('end', () => {
                    ...
                    .on('error', (ffmpegErr) => reject(new Error(`FFmpeg lintasan 2 error: ${ffmpegErr.message}`)))
                    .run();
            };
            ...
            ffmpeg(inputPath)
                ...
                .on('error', (ffmpegErr) => reject(new Error(`FFmpeg lintasan 1 error: ${ffmpegErr.message}`)))
                .run();
        });
    });
};
...
const compressUntilFits = async (inputPath, limitBytes, limitMB, maxAttempts = 3, forcedHeight = null, cleanup = null) => {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        logger.info(`[Downloader] Percobaan ${attempt}/${maxAttempts}: ${(stats.size / (1024 * 1024)).toFixed(1)}MB melebihi ${limitMB.toFixed(1)}MB.`);
        targetMB = Math.max(Math.floor(targetMB * (limitBytes / stats.size) * 0.9), 8);
    }

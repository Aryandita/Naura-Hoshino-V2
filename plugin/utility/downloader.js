...
const { getUploadLimitBytes, getUploadLimitMB } = require('./downloaderCompress');

module.exports = {
    data: new SlashCommandBuilder()
        .setName('downloader')
        .setDescription('Unduh video & foto kualitas terbaik (YouTube, IG, TikTok, X, dan lainnya).')
        .addStringOption(opt =>
            opt.setName('url')
    ...
    async execute(interaction) {
        const limited = await RateLimiter.isRateLimited(interaction.user.id, 'downloader', 4, 60);
        if (limited) return ui.sendError(interaction, 'err_sys_30', true);

        await interaction.deferReply();

        const url = String(interaction.options.getString('url')).replace(/[<>]/g, '').trim();
        ...
        try {
            ...
            if (data.status === 'picker' && Array.isArray(data.picker)) {
                ...
                if (resolutionPickerSource) cleanup.files.delete(resolutionPickerSource);

                if (!data.isLocalFile && typeof data.url === 'string' && data.url.startsWith('http')) {
            ...
            if (primaryAttachmentName) {
                ...
                const visual = ['mp4', 'webm', 'mov', 'avi', 'mkv', 'jpg', 'jpeg', 'png', 'webp', 'gif', 'bmp', 'heic'];
                if (visual.includes(ext)) mediaGalleryRefs.push(primaryAttachmentName);
                else otherFileRefs.push(primaryAttachmentName);
            }

            const rows = [actionRow];
            if (resolutionPickerSource) rows.push(picker.buildResolutionRow(interaction.id));
            const usableRows = rows.filter(row => row && row.components.length > 0);

            const qualityLabel = preselectedHeight ? ` | pilihan kamu: **${preselectedHeight}p**` : '';
            const resolutionText = primaryAttachmentName && primaryAttachmentName.includes('compressed')

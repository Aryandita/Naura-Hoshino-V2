...
const FAILURE_HINT = {
    instagram: 'Pastikan postingannya **Publik**, ya. Kalau akunnya digembok, Naura tidak bisa mengintip ke dalam.',
    twitter: 'Servernya lagi rewel. Coba lagi beberapa menit lagi, Naura tunggu bareng kamu.',
    ...
    facebook: 'Pastikan postingannya **Publik** dulu, baru Naura bisa mengambilnya.',
    ...
    linkedin: 'Pastikan postingannya **Publik** dan memang berisi video atau gambar.',
    vk: 'Pastikan videonya **Publik**, bukan yang privat.',
    douyin: 'Kontennya mungkin dibatasi region atau butuh login.',
    bilibili: 'Sebagian konten Bilibili butuh akun premium atau hanya tersedia di China.'
};

const platformEmojiOf = (platform) => e(PLATFORM_EMOJI[platform] || 'globe_with_meridians', PLATFORM_FALLBACK_EMOJI[platform] || '\uD83C\uDF10');

const platformNameOf = (platform) => platform.charAt(0).toUpperCase() + platform.slice(1);

/** Kartu ketika tautan yang dikirim bukan http/https. */
const invalidUrlCard = () => buildErrorContainerV2({
...
const failureCard = (platform) => buildErrorContainerV2({
...
const unsupportedCard = () => buildErrorContainerV2({
...
const loadingCard = ({ client, title, description }) => buildLoadingContainerV2({
...
const inspectingCard = (client) => loadingCard({
    client,
 ...
 */
const resultCard = ({ client, platform, sourceUrl, resolutionText, extraText = '', mediaNames = [], fileNames = [], rows = null }) => buildContainerV2({
...
const compressFailedCard = ({ client, title, description }) => buildContainerV2({

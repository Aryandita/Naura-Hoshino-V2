...
setInterval(() => {
    ...
    for (const [key, value] of mojangCache.entries()) {
        if (value.cachedAt && now - value.cachedAt > 3600000) { // 1 jam TTL
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('minecraft')
        .setDescription('Alat utilitas untuk game Minecraft (Server Ping, Skin Render, Sinkronisasi Akun)')
        .addSubcommand(sub =>
            sub.setName('ping')
            .setDescription('Periksa status dan ping dari server Minecraft (Java Edition)')
            .addStringOption(opt => opt.setName('ip').setDescription('Alamat IP Server Minecraft').setRequired(true))
        )
        .addSubcommand(sub =>
            sub.setName('player')
            .setDescription('Render kulit 3D (skin) & kepala (head) pemain Minecraft')
            .addStringOption(opt => opt.setName('username').setDescription('Username Minecraft (Premium)').setRequired(true))
        )
        .addSubcommand(sub =>
            sub.setName('link')
            .setDescription('Tautkan akun Discord-mu dengan username Minecraft asli')
            .addStringOption(opt => opt.setName('username').setDescription('Username Minecraft Premium milikmu').setRequired(true))
        )
        .addSubcommand(sub =>
            sub.setName('stats')
        ...
        .addSubcommand(sub =>
            ...
            .addChannelOption(opt => opt.setName('channel').setDescription('Channel Discord untuk jembatan chat').setRequired(true))
            .addStringOption(opt => opt.setName('rcon_ip').setDescription('Alamat IP RCON Server Minecraft').setRequired(false))
            .addIntegerOption(opt => opt.setName('rcon_port').setDescription('Port RCON Server Minecraft (Default: 25575)').setRequired(false))
            .addStringOption(opt => opt.setName('rcon_password').setDescription('Kata Sandi RCON Server Minecraft').setRequired(false))
            .addBooleanOption(opt => opt.setName('enabled').setDescription('Aktifkan atau nonaktifkan jembatan chat').setRequired(false))
        ),
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'ping') {
            ...
            try {
                ...
                const fields = [
                    { name: `${ui.getEmoji('radar') || '📡'} Latensi / Ping`, value: `\`${response.ping}ms\`` },
                    { name: `${ui.getEmoji('users') || '👥'} Populasi Pemain`, value: `\`${response.players.online} / ${response.players.max}\`` },
                    { name: `${ui.getEmoji('gear') || '⚙️'} Versi Enjin`, value: `\`${response.version.name}\`` },
                    { name: `${ui.getEmoji('scroll') || '📜'} MOTD (Pesan Hari Ini)`, value: `\`\`\`${response.motd.clean}\`\`\`` }
                ];

                if (response.players.sample && response.players.sample.length > 0) {
                    const playerList = response.players.sample.map(p => p.name).join(', ');
                    fields.push({ name: `${ui.getEmoji('users') || '🧑‍🤝‍🧑'} Pemain Online (Sampel)`, value: `\`\`\`${playerList}\`\`\`` });
                }
                ...
                return interaction.editReply(`${ui.getEmoji('cross')} Gagal mencapai **${ip}**. Server mungkin *offline*, memblokir port (TCP 25565), atau menolak koneksi eksternal.`);
            }
        }
            ...
            try {
                ...
                await cacheManager.updateUserProfile(interaction.user.id, { minecraft_ign: profileData.name });
            ...
            if (!interaction.member.permissions.has(PermissionFlagsBits.ManageGuild) && !interaction.member.permissions.has(PermissionFlagsBits.Administrator)) {
                const errPayload = buildErrorContainerV2({ title: 'Akses Ditolak', description: `${ui.getEmoji('cross') || '❌'} Kamu memerlukan izin \`Manage Server\` atau \`Administrator\` untuk menyetel jembatan chat ini.`, footerText: ui.getFooter('core') });
                return interaction.editReply(errPayload);
            }
            ...
            let [settingsModel] = await GuildSettings.findOrCreate({ where: { guildId: interaction.guild.id } });
            ...
            else if (enabled === null && mc.bridgeEnabled === undefined) mc.bridgeEnabled = true;

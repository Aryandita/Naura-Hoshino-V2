...
const buildResolutionRow = (interactionId) => new ActionRowBuilder().addComponents(
 ...
 * @param {object} args
 */
const attachResolutionCollector = ({ interaction, message, sourcePath, platform, sourceUrl, limitBytes, limitMB, extraRows = [] }) => {
    ...
    collector.on('collect', async (i) => {
        ...
        try {
            ...
            await i.editReply(render.compressingCard(
                interaction.client,
                'ukuran aslinya',
            ...
            if (!fs.existsSync(sourcePath)) {
                throw new Error('Berkas sumber sudah tidak tersedia (mungkin server sempat restart).');
            }

            const picked = await compressUntilFits(sourcePath, limitBytes, limitMB, 3, chosenHeight, manualCleanup);
            ...
            if (picked.success) {
                ...
                payload.files = [new AttachmentBuilder(picked.path, { name: finalName })];
                await i.editReply(payload);
                return;
            }

            await i.editReply(render.compressFailedCard({
                client: interaction.client,
            ...
            logger.error(`[Downloader] Gagal memproses pilihan resolusi: ${pickErr.message}`);

            const isFfmpegIssue = /ENOENT|FFmpeg|tidak terdeteksi/i.test(pickErr.message);
    ...
    collector.on('end', (collected) => {
        ...
        interaction.editReply(render.resultCard({
            client: interaction.client,
            platform,
            sourceUrl,
...
module.exports = { PICK_TIMEOUT_MS, buildResolutionRow, attachResolutionCollector };

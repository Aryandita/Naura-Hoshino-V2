const { SlashCommandBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ModalBuilder, TextInputBuilder, TextInputStyle, AttachmentBuilder, MessageFlags } = require('discord.js');
...
async function createBusinessCard(user, profile, topFriend, streak) {
    ...
    ctx.fillText(`Saldo: ${profile.economy_wallet.toLocaleString('id-ID')} ${ui.currencySymbol || 'NC'}`, 230, 210);
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub =>
            sub.setName('view')
                ...
                .addUserOption(opt => opt.setName('user').setDescription('Pilih user').setRequired(false))
        )
        .addSubcommand(sub =>
            sub.setName('setup')
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'setup') {
            ...
            const ytInput = new TextInputBuilder().setCustomId('social_yt').setLabel('Username YouTube (Kosongkan jika tidak ada)').setStyle(TextInputStyle.Short).setRequired(false).setValue(profile.social_youtube || '');
            const igInput = new TextInputBuilder().setCustomId('social_ig').setLabel('Username Instagram').setStyle(TextInputStyle.Short).setRequired(false).setValue(profile.social_instagram || '');
            const xInput = new TextInputBuilder().setCustomId('social_x').setLabel('Username X / Twitter').setStyle(TextInputStyle.Short).setRequired(false).setValue(profile.social_x || '');
            const fbInput = new TextInputBuilder().setCustomId('social_fb').setLabel('Username Facebook').setStyle(TextInputStyle.Short).setRequired(false).setValue(profile.social_facebook || '');
            ...
            // Wait for submit
            try {
                const submit = await interaction.awaitModalSubmit({ filter: i => i.customId === 'profile_social_modal' && i.user.id === interaction.user.id, time: 300000 });

                const updates = {
                    social_youtube: submit.fields.getTextInputValue('social_yt') || null,
                    social_instagram: submit.fields.getTextInputValue('social_ig') || null,
            ...
            const isVIP = (profile.isPremium && profile.premiumUntil && new Date(profile.premiumUntil) > new Date()) || isBotOwner;
            ...
            if (friends.length > 0) {
                ...
                const topFriendId = topF.user1Id === targetUser.id ? topF.user2Id : topF.user1Id;
                const topFriendUser = await interaction.client.users.fetch(topFriendId).catch(() => null);
                if (topFriendUser) {
            ...
            if (isVIP) {
                const buffer = await createBusinessCard(targetUser, profile, topFriendData, topStreak);
            ...
            if (profile.social_youtube) row.addComponents(new ButtonBuilder().setLabel(profile.social_youtube).setStyle(ButtonStyle.Link).setURL(`https://youtube.com/@${profile.social_youtube.replace('@', '')}`));
            if (profile.social_instagram) row.addComponents(new ButtonBuilder().setLabel(profile.social_instagram).setStyle(ButtonStyle.Link).setURL(`https://instagram.com/${profile.social_instagram}`));
            if (profile.social_x) row.addComponents(new ButtonBuilder().setLabel(profile.social_x).setStyle(ButtonStyle.Link).setURL(`https://x.com/${profile.social_x}`));
            if (profile.social_facebook) row.addComponents(new ButtonBuilder().setLabel(profile.social_facebook).setStyle(ButtonStyle.Link).setURL(`https://facebook.com/${profile.social_facebook}`));
            ...
            const payload = buildContainerV2({
                ...
                iconURL: targetUser.displayAvatarURL(),
                description: hasSocial ? undefined : '*User ini belum menautkan akun sosial media.*',
                fields: [
                    { name: '💳 Ekonomi', value: `**Saldo Tunai:** ${ui.getEmoji('coin') || '🪙'} ${profile.economy_wallet.toLocaleString('id-ID')} ${ui.currencyName || 'Naura Coin'}` },
                    { name: `${ui.getEmoji('handshake') || '🤝'} Sosial`, value: `**Total Teman:** ${friends.length}\n**${ui.getEmoji('fire') || '🔥'} Top Streak:** ${topFriendData ? `${topFriendData.username} (${topStreak} hari)` : 'Belum ada'}` }
                ],

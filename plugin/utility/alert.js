    ...
    async execute(interaction) {
        ...
        const generateDashboard = () => {
            const rowBtns = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId('sa_add_yt').setLabel('Tambah YouTube').setEmoji(ui.getEmoji('red_circle') || '🔴').setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId('sa_add_tt').setLabel('Tambah TikTok').setEmoji(ui.getEmoji('music_note') || '🎵').setStyle(ButtonStyle.Primary)
            );
            ...
            const payload = buildContainerV2({
                ...
                description: 'Sistem ini akan mendeteksi video YouTube atau TikTok terbaru secara otomatis dan mengirimkannya langsung ke channel servermu!\n\n**Daftar Notifikasi Aktif:**\n' +
                    (alerts.length > 0 
                        ? alerts.map((a, i) => `**${i + 1}.** ${a.platform === 'youtube' ? (ui.getEmoji('red_circle') || '🔴') : (ui.getEmoji('music_note') || '🎵')} **${a.name}**\n↳ ${ui.getEmoji('round_pushpin') || '📍'} Dikirim ke: <#${a.discordChannelId}>`).join('\n\n')
        ...
        collector.on('collect', async i => {
            if (i.user.id !== interaction.user.id) return ui.sendError(i, 'err_sys_29', true);
            ...
            if (i.isButton() && (i.customId === 'sa_add_yt' || i.customId === 'sa_add_tt')) {
                ...
                try {
                    ...
                    await submitted.update({ ...channelPayload, components: [channelRow] });

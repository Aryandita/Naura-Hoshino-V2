...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("alert")
    .setDescription(
      "📡 [ADMIN] Setup Notifikasi Sosial Media (YouTube/TikTok) Interaktif.",
    )
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageWebhooks),

  async execute(interaction) {
    let alerts = await SocialAlert.findAll({
    ...
    const generateDashboard = () => {
      const rowBtns = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId("sa_add_yt")
          .setLabel("Tambah YouTube")
          .setEmoji(ui.getEmoji("red_circle") || "🔴")
          .setStyle(ButtonStyle.Danger),
        new ButtonBuilder()
          .setCustomId("sa_add_tt")
          .setLabel("Tambah TikTok")
          .setEmoji(ui.getEmoji("music_note") || "🎵")
      ...
      if (alerts.length > 0) {
        const removeSelect = new StringSelectMenuBuilder()
          .setCustomId("sa_remove")
          .setPlaceholder("🗑️ Hapus Notifikasi (Pilih untuk menghapus)");
        alerts.forEach((a) => {
          removeSelect.addOptions({
            label: `Hapus ${a.platform.toUpperCase()}: ${a.name.substring(0, 50)}`,
            value: a.id.toString(),
          });
        });
      ...
      const payload = buildContainerV2({
        ...
        title: "📡 Social Media Alert Dashboard",
        description:
          "Sistem ini akan mendeteksi video YouTube atau TikTok terbaru secara otomatis dan mengirimkannya langsung ke channel servermu!\n\n**Daftar Notifikasi Aktif:**\n" +
          (alerts.length > 0
            ? alerts
                .map(
                  (a, i) =>
                    `**${i + 1}.** ${a.platform === "youtube" ? ui.getEmoji("red_circle") || "🔴" : ui.getEmoji("music_note") || "🎵"} **${a.name}**\n↳ ${ui.getEmoji("round_pushpin") || "📍"} Dikirim ke: <#${a.discordChannelId}>`,
                )
                .join("\n\n")
    ...
    collector.on("collect", async (i) => {
      if (i.user.id !== interaction.user.id)
        return ui.sendError(i, "err_sys_29", true);

      if (
        i.isButton() &&
        (i.customId === "sa_add_yt" || i.customId === "sa_add_tt")
      ) {
        const platform = i.customId === "sa_add_yt" ? "YouTube" : "TikTok";
        const inputLabel =
          i.customId === "sa_add_yt"
            ? "Channel ID (UC...)"
            : "Username (Tanpa @)";

        const modal = new ModalBuilder()
          .setCustomId(`modal_${i.customId}`)
          .setTitle(`Setup ${platform} Alert`);
        modal.addComponents(
        ...
        try {
          ...
          const channelRow = new ActionRowBuilder().addComponents(
            new ChannelSelectMenuBuilder()
              ...
              .setPlaceholder("Pilih Channel Target")
          ...
          const filterChannel = (m) =>
            m.user.id === interaction.user.id &&
            m.customId === "select_alert_channel";
          ...
          alerts = await SocialAlert.findAll({
            where: { guildId: interaction.guild.id },
          });

...
async function handleVoiceState(oldState, newState) {
    ...
    const stateChanged = oldState.selfMute !== newState.selfMute || oldState.selfDeaf !== newState.selfDeaf;

    // User is AFK if they are muted AND deafened, or server muted/deafened
    const isAfk = newState.selfDeaf || newState.serverDeaf || (newState.selfMute && newState.serverMute); // Just a simple heuristic

    if (joinedChannel || (switchedChannel && !isAfk) || (stateChanged && !isAfk)) {
        ...
        if (joinTime) {
            ...
            if (minutes >= 1) {
                ...
                await cacheManager.incrementUserSurvival(userId, 'starFragments', nsfReward);
            }
        }
    }
}

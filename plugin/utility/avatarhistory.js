...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(sub =>
            ...
            .addUserOption(opt => opt.setName('user').setDescription('Pilih user').setRequired(false))
            .addBooleanOption(opt => opt.setName('server_avatar').setDescription('Gunakan foto profil server? (Jika ada)').setRequired(false))
        )
        .addSubcommand(sub =>
            sub.setName('banner')
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'info') {
            ...
            if (member) {
                ...
                boosterStr = member.premiumSince ? `Ya, sejak <t:${Math.floor(member.premiumSinceTimestamp / 1000)}:R>` : 'Tidak';
            }
            ...
            const avatarUrl = (useServerAvatar && member && member.avatar) ?

...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(opt => opt.setName('teks').setDescription('Teks pencapaian (maks 25 karakter)').setRequired(true))
        .addIntegerOption(opt => opt.setName('icon').setDescription('Pilih ID icon (1-39)').setRequired(false).setMinValue(1).setMaxValue(39)),

    async execute(interaction) {
        let text = interaction.options.getString('teks');
        let icon = interaction.options.getInteger('icon') || Math.floor(Math.random() * 39) + 1;

...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('akinator')
        .setDescription('🧞‍♂️ Akinator akan mencoba menebak karakter yang sedang kamu pikirkan!'),
        ...
        try {
            ...
            const createPayload = () => {
                return buildContainerV2({
                    ...
                    iconURL: 'https://en.akinator.com/bundles/elokencesite/images/akinator.png',
                    ...
                    buttonsRow: [
                        ...
                        new ActionRowBuilder().addComponents(
                            new ButtonBuilder().setCustomId('aki_3').setLabel('Mungkin Ya').setStyle(ButtonStyle.Primary),
                            new ButtonBuilder().setCustomId('aki_4').setLabel('Mungkin Tidak').setStyle(ButtonStyle.Primary),
                            new ButtonBuilder().setCustomId('aki_back').setLabel('« Kembali').setStyle(ButtonStyle.Secondary)
                        )
                    ],
            ...
            collector.on('collect', async i => {
                ...
                if (aki.progress >= 80 || aki.currentStep >= 79) {
                    ...
                    if (guess) {
                        ...
                        await i.update({ content: '🧞‍♂️ Aku menyerah! Aku tidak bisa menebak karaktermu.', embeds: [], components: [] });
                        collector.stop('lost');
                    }
                } else {
            ...
            collector.on('end', (collected, reason) => {
                if (reason === 'time') {
                    interaction.editReply({ content: '⏰ Waktu habis! Game Akinator dihentikan.', embeds: [], components: [] }).catch(() => {});
                }
            });

        } catch (error) {
            logger.error('[Akinator Error]', error);
            await interaction.editReply('❌ Gagal memulai Akinator. Coba beberapa saat lagi.');
        }
    }
};

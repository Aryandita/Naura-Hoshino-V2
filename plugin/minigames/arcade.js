...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("arcade")
    .setDescription("🕹️ Holo-Arcade: Mainkan mini-game interaktif dan kumpulkan Star Fragments!")
    .addSubcommand((sub) =>
      sub
        .setName("trivia")
        .setDescription("🧠 Cyber Trivia Royale: Jawab kuis anime/gaming dengan cepat!")
    ...
    .addSubcommand((sub) =>
      sub
        .setName("rhythm")
        .setDescription("🎶 Rhythm Tap: Ikuti urutan tombol warna cyber yang muncul!")
        .addIntegerOption((opt) =>
          opt
            .setName("bet")
            .setDescription("Taruhan Star Fragments (Opsional)")
    ...
    .addSubcommand((sub) =>
      sub
        .setName("roulette")
        .setDescription("🎰 Cyber Roulette: Pasang taruhan pada warna atau angka keberuntungan!")
        .addStringOption((opt) =>
          opt
            .setName("choice")
            .setDescription("Pilihan taruhan (RED, BLACK, GREEN, atau angka 0-36)")
            .setRequired(true),
        )
        .addIntegerOption((opt) =>
          opt
  ...
  async execute(interaction) {
    ...
    if (subcommand === "roulette") {
      const choice = interaction.options.getString("choice").trim().toUpperCase();
      ...
      if (!validColors.includes(choice) && !isValidNum) {
        return interaction.reply({
          ...buildErrorContainerV2({
            ...
            description: "Pilihan harus berupa `RED`, `BLACK`, `GREEN`, atau angka antara `0` sampai `36`.",
      ...
      const canDebit = await currency.charge(userId, { starFragments: bet }, "Arcade Roulette Bet");
      if (!canDebit) {
      ...
      if (result.isWon) {
        await currency.reward(userId, { starFragments: result.payout }, "Arcade Roulette Win");
      }
    ...
    if (subcommand === "trivia") {
      ...
      if (bet > 0) {
        const canDebit = await currency.charge(userId, { starFragments: bet }, "Arcade Trivia Bet Escrow");
        if (!canDebit) {
          return interaction.editReply({
            ...buildErrorContainerV2({
              title: "Saldo Kurang",
      ...
      await redisManager.set(`arcade:trivia:${sessionId}`, JSON.stringify(sessionData), "EX", 60);
    ...
    if (subcommand === "rhythm") {
      ...
      if (bet > 0) {
        const canDebit = await currency.charge(userId, { starFragments: bet }, "Arcade Rhythm Bet Escrow");
        if (!canDebit) {
          return interaction.editReply({
            ...buildErrorContainerV2({
              title: "Saldo Kurang",
      ...
      await redisManager.set(`arcade:rhythm:${sessionId}`, JSON.stringify(sessionData), "EX", 60);

...
async function runMigration() {
    ...
    for (const file of files) {
        ...
        content = content.replace(sendErrorRegex, (fullMatch, p1, quote, text, p4) => {
            // Skip jika sudah diterjemahkan atau ada template literal kompleks
            if (text.includes('languageManager') || text.includes('${') || text.startsWith('err_sys_')) {
                return fullMatch;
            }
    ...
    console.log(`✅ Berhasil menemukan dan mengganti key di ${modifiedFilesCount} file.`);
    console.log(`⏳ Menyalin string ke en.json (Menunggu terjemahan manual atau batch)...`);

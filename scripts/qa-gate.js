...
function runStep(name, command, args) {
  return new Promise((resolve) => {
    ...
    proc.on("close", (code) => {
      ...
      if (success) {
        console.log(
          `${C.green}✔ ${name} LOLOS (${(durationMs / 1000).toFixed(2)}s)${C.reset}`,
        );
      } else {
        console.log(
          `${C.red}✖ ${name} GAGAL dengan exit code ${code} (${(durationMs / 1000).toFixed(2)}s)${C.reset}`,
        );
      }
...
async function run() {
  ...
  console.log(
    `${C.bold}${C.cyan}  NAURA HOSHINO V2 - MASTER QA GATE RUNNER          ${C.reset}`,
  );
  console.log(
    `${C.bold}${C.cyan}====================================================${C.reset}`,
  );
  ...
  // 1. ESLint
  results.push(
    await runStep("1/5 ESLint & Standar Kode", "npm", ["run", "lint"]),
  );

  // 2. Em dash check
  results.push(
    await runStep("2/5 Cek Larangan Karakter Em Dash", "node", [
  ...
  results.push(
    await runStep("3/5 Paritas Kamus Bahasa (Strict)", "node", [
  ...
  // 4. Resolusi Require
  results.push(
    await runStep("4/5 Integritas Resolusi Require", "node", [
      "scripts/check-requires.js",
    ]),
  );

  // 5. Automated Unit Tests
  results.push(
    await runStep("5/5 Automated Unit Test Suite", "node", [
      "--test",
      "src/**/*.test.js",
    ]),
  );
  ...
  console.log(
    `${C.bold}${C.cyan}                 RINGKASAN QA GATE                  ${C.reset}`,
  );
  console.log(
    `${C.bold}${C.cyan}====================================================${C.reset}\n`,
  );
  ...
  for (const r of results) {
    ...
    console.log(
      `  ${status} ${r.name.padEnd(40)} : ${C.gray}${time}${C.reset}`,
    );
  }

...
function readJson(file) {
    ...
    try {
        ...
        console.error(`  [X] JSON tidak valid: ${path.relative(ROOT, file)} -> ${error.message}`);
...
for (const dir of findLocaleDirs(ROOT)) {
    ...
    if (missingInEn.length) {
        console.log(`      Hilang di en.json (${missingInEn.length}): ${missingInEn.slice(0, 15).join(', ')}${missingInEn.length > 15 ? ', ...' : ''}`);
    }
    if (missingInId.length) {
        console.log(`      Hilang di id.json (${missingInId.length}): ${missingInId.slice(0, 15).join(', ')}${missingInId.length > 15 ? ', ...' : ''}`);
    }

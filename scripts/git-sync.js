...
function main() {
  console.log("🚀 [GitSync] Memulai pemeriksaan sinkronisasi repositori GitHub...");

  // 1. Dapatkan status perubahan
  const status = runGit("git status --porcelain");
  if (!status) {
    console.log("✅ [GitSync] Repositori sudah bersih (working tree clean). Tidak ada perubahan untuk di-commit.");
    return;
  }
  ...
  const customMessage = process.argv.slice(2).filter((arg) => !arg.startsWith("--")).join(" ").trim();
  const commitMessage = customMessage || `chore(update): sync repository updates and milestone progress [${new Date().toISOString().split("T")[0]}]`;

  console.log(`📦 [GitSync] Perubahan terdeteksi pada branch "${currentBranch}". Menyiapkan commit...`);
  ...
  // 5. Commit perubahan
  const commitOutput = runGit(`git commit -m "${commitMessage.replace(/"/g, '\\"')}"`);
  console.log(`💾 [GitSync] Commit berhasil dibuat:\n${commitOutput}`);

  // 6. Push ke remote origin
  console.log(`🌐 [GitSync] Mengirimkan perubahan ke remote origin/${currentBranch}...`);
  try {
    const pushOutput = runGit(`git push origin ${currentBranch}`);
    if (pushOutput) {
      console.log(pushOutput);
    }
    console.log(`🎉 [GitSync] Sinkronisasi ke GitHub selesai dengan sukses! Branch: ${currentBranch}`);
  } catch (pushErr) {
    console.warn(`⚠️ [GitSync] Peringatan: Push gagal atau jaringan bermasalah:\n${pushErr.message}`);
